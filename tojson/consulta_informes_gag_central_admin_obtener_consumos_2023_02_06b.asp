...
<%
	...
	'response.write("<br>nombre establecimiento: " & nombre_establecimiento)
		...
		cadena_sql = cadena_sql & " FROM (SELECT A.ID AS ID_ARTICULO , A.CODIGO_SAP as CODIGO_SAP, REPLACE(A.DESCRIPCION, '""', '\""') as ARTICULO, A.UNIDADES_DE_PEDIDO"
		...
		cadena_sql = cadena_sql & ", F.EMPRESA AS NOMBRE_EMPRESA"
		...
		cadena_sql = cadena_sql & " GROUP BY A.ID, A.CODIGO_SAP, A.DESCRIPCION, A.UNIDADES_DE_PEDIDO, F.EMPRESA"
		...
		cadena_sql = cadena_sql & " LEFT JOIN (SELECT Z.ID_ARTICULO"
		...
		cadena_sql = cadena_sql & ", V.EMPRESA, SUM(UNIDADES_ACEPTADAS) AS UNIDADES_DEVUELTAS"
		...
		cadena_sql = cadena_sql & " GROUP BY Z.ID_ARTICULO, V.EMPRESA"
		...
		cadena_sql = cadena_sql & " ) DEVOLUCIONES ON ARTICULOS.ID_ARTICULO=DEVOLUCIONES.ID_ARTICULO AND ARTICULOS.NOMBRE_EMPRESA=DEVOLUCIONES.EMPRESA"
		...
		cadena_sql = cadena_sql & " ORDER BY ARTICULOS.ARTICULO, ARTICULOS.NOMBRE_EMPRESA"
