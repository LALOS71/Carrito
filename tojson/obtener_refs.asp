...
<%
	...
	solicitados = "" & request.QueryString("p_solicitados")
	...
	'response.write("<br>nombre establecimiento: " & nombre_establecimiento)
		...
		'PRIMERO MARCAMOS AUTOMATICAMENTE SOLICITADO_AL_PROVEEDOR =SI, a aquellos articulos que requieren hoja de ruta
		cadena_sql = "UPDATE ARTICULOS"
		cadena_sql=cadena_sql & " SET SOLICITADO_AL_PROVEEDOR='SI'"
		cadena_sql=cadena_sql & " FROM ARTICULOS"
		cadena_sql=cadena_sql & " WHERE ID IN ("
		cadena_sql=cadena_sql & " SELECT A.ID_ARTICULO"
		...
		cadena_sql=cadena_sql & " left join (select final.articulo , sum(final.cantidad) as cantidad , sum(final.cantidad_pendiente) as cantidad_pendiente"
		cadena_sql=cadena_sql & " from ( select articulo as articulo, sum(cantidad) as cantidad, sum(cantidad) as cantidad_pendiente from pedidos_detalles"
		...
		cadena_sql=cadena_sql & " select tabla.articulo , sum(tabla.cantidad) as cantidad, sum(tabla.cantidad_pendiente) as cantidad_pendiente"
		cadena_sql=cadena_sql & " from (select a.articulo , a.cantidad ,(a.cantidad - (select sum(cantidad_enviada)"
		...
		cadena_sql=cadena_sql & " group by articulo )as final"
		cadena_sql=cadena_sql & " group by final.articulo ) consultita"
		...
		cadena_sql=cadena_sql & " LEFT JOIN ( SELECT FAMILII.*, FAMILIAS.DESCRIPCION"
		cadena_sql=cadena_sql & " FROM (SELECT ARTICULOS_EMPRESAS.ID_ARTICULO, COUNT(*) AS CUENTA"
		...
		cadena_sql=cadena_sql & " FROM ARTICULOS_EMPRESAS"
		cadena_sql=cadena_sql & " LEFT JOIN V_EMPRESAS"
		...
		cadena_sql=cadena_sql & " ON FAMILIAS.ID=FAMILII.CODIGO_FAMILIA ) AS TABLA2"
		...
		cadena_sql=cadena_sql & " and ((SOLICITADO_AL_PROVEEDOR IS NULL) OR SOLICITADO_AL_PROVEEDOR='NO')"
		cadena_sql=cadena_sql & " and requiere_hoja_ruta='SI'"
		...
		cadena_sql=cadena_sql & " , REPLACE(B.DESCRIPCION, '""', '____') as DESCRIPCION"
		...
		if ucase(solicitados)="FALSE" then
			cadena_sql=cadena_sql & " and (solicitado_al_proveedor = 'N0' or solicitado_al_proveedor is null)"
		end if
	...
	if ver_cadena="SI" then
		response.write("<br><br>Cadena SQL: " & cadena_sql)
	end if
