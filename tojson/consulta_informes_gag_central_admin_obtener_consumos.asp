...
<%
	...
	'response.write("<br>nombre establecimiento: " & nombre_establecimiento)
		...
		if diferenciar_articulos = "SI" then
			cadena_sql = cadena_sql & ", A.ID AS ID_ARTICULO , A.CODIGO_SAP as CODIGO_SAP, REPLACE(A.DESCRIPCION, '""', '____') as ARTICULO, A.UNIDADES_DE_PEDIDO"
		end if
		...
		cadena_sql = cadena_sql & " AND (F.ID = " & codigo_empresa & ")" 'SOLO PARA GLS
		if id_articulo<>"" then
		...
		cadena_sql = cadena_sql & " AND (W.EMPRESA = " & codigo_empresa & ")"
		if id_articulo<>"" then
	...
	cadena_devoluciones=REPLACE(cadena_devoluciones,"____", "\""")
