...
<%
	...
	grupo_empleado=request.querystring("grupo_empleado")
	...
	for i=1 to Session("numero_articulos")
		...
		with articulos
			...
			.Source="SELECT A.ID, A.CODIGO_SAP, A.DESCRIPCION, B.ID_GRUPO_ROPA"
			.Source= .Source & " FROM ARTICULOS A"
			.Source= .Source & " LEFT JOIN GRUPOS_ROPA_ARTICULOS_EMPLEADOS_GLS B"
			.Source= .Source & " ON A.ID=B.ID_ARTICULO"
			.Source= .Source & " WHERE A.ID=" & articulo
			.Source= .Source & " AND B.GRUPO=" & grupo_empleado
			.Open
		end with
		...
		tabla=tabla & " style='cursor:pointer;cursor:hand'>" & referencia 
		tabla=tabla & "<hidden class='resumen_articulos_grupo_ropa' id='oculto_resumen_grupo_ropa_'" & id_grupo_ropa & "' value='" & cantidad & "'>"
		tabla=tabla & "</td>"
