...
<%
	...
	'response.write("<br>nombre establecimiento: " & nombre_establecimiento)
		...
		cadena_sql= cadena_sql & ", round( (isnull(Tot.total,0 ) + isnull(gastos_envio,0)  - isnull(total_devoluciones,0)) * 0.21, 2)  as TotIva"
		...
		cadena_sql= cadena_sql & ", round( ((isnull(Tot.total,0 ) + isnull(gastos_envio,0) - isnull(TOTAL_SALDOS,0) - isnull(total_devoluciones,0)) + ((isnull(Tot.total,0 ) + isnull(gastos_envio,0) - isnull(total_devoluciones,0)) * 0.21)), 2)  as TotalEnvio"
		if session("usuario_admin")="CARLOS" then
			...
			cadena_sql= cadena_sql & ", Tot.total as TotalSinIVA"
			cadena_sql= cadena_sql & ", Tot.TotalCoste  as TotalCoste"
		end if	
		...
		cadena_sql= cadena_sql & " LEFT JOIN (SELECT ID_Pedido, sum(total) Total, SUM(isnull(CANTIDAD,0) * isnull(PRECIO_COSTE,0)) as TotalCoste, Sum(1) NReg FROM Pedidos_Detalles"
