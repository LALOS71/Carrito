...
<%
	...
	set empresas=Server.CreateObject("ADODB.Recordset")
		...
		with empresas
			...
			'RESPONSE.WRITE("<BR>CONSULTA EMPRESAS: " & .SOURCE)
			.Open
		...
		with proveedores
			...
			'RESPONSE.WRITE("<BR>CONSULTA PROVEEDORES: " & .SOURCE)
			.Open
		...
		with articulos
			...
			'RESPONSE.WRITE("<BR>CONSULTA ARTICULO: " & .SOURCE)
			.Open
		end with
		campo_codigo_sap=""
		campo_descripcion=""
		...
		with stocks_articulo
			.ActiveConnection=connimprenta
			.CursorType=3 'adOpenStatic
			'RESPONSE.WRITE("<BR>CONSULTA STOCKS ARTICULO: " & sql)
			.Source=sql
			.Open
		end with
...
function comprobar_articulo()
{
	...
	//console.log('pesta�a vuelta: ' + pestanna_vuelta)	
				...
				bootbox.confirm({
					...
					callback: function (result) {
						if (result)
							...
							//console.log('metemos el valor de la pesta�a: ' + pestanna_vuelta)
...
<!--*********** FINAL DE LAS PESTA�AS ********************************-->										  
								  
								  
								</main><!--final de las pesta�as-->
...
//se pulsa la pesta�a de entradas/salidas de material
...
//se pulsa la pesta�a de cantidades/precios
