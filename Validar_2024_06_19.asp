...
'****************************************************************************
	...
	if hotel_seleccionado="" then
		hotel_seleccionado="" & Request.Form("cmbclientes")
	end if
	...
	'validar.asp?empresa=AVORIS	
	'etc....
	
	if empresa_query="BELIVE" then
		hotel_seleccionado=4671 'BE LIVE ADULTS ONLY MARIVENT
		contrasenna_seleccionada="globalia/marivent"
		empresa_entrada=2
	end if
	
	if empresa_query="BELIVE_AD" then
		hotel_seleccionado=1429 'BE LIVE ADMINISTRADORA
		contrasenna_seleccionada="globalia-centralbelive"
		empresa_entrada=2
	end if
	...
	if empresa_query="ATESA" then
		hotel_seleccionado=3687 'E5CG TALAVERA DE LA REINA
		contrasenna_seleccionada="atesa.111"
		empresa_entrada=5
	end if
	if empresa_query="ATESA_AD" then
		hotel_seleccionado=623 'ATESA ADMINISTRADORA
		contrasenna_seleccionada="central.atesa"
		empresa_entrada=5
	end if
	
	if empresa_query="ABBA" then
		hotel_seleccionado=7480 'ABBA FONSECA
		contrasenna_seleccionada="abba-fonseca"
		empresa_entrada=7
	end if
	if empresa_query="ABBA_AD" then
		hotel_seleccionado=3814 'ABBA ADMINISTRADORA
		contrasenna_seleccionada="central-abbahoteles"
		empresa_entrada=7
	end if
	...
	if empresa_query="HALCON" and tipo_query="PROPIA" then
		...
		hotel_seleccionado=6352 '217-VALENCIA - PAZ
		contrasenna_seleccionada="HALCON_217"
	...
	if empresa_query="GROUNDFORCE" then
		if oficina_query<>"" then
				if oficina_query=1460 then
					hotel_seleccionado=1460 'GROUNDFORCE LAS PALMAS UTE (CANARIAS - ISLAS MAYORES)
					contrasenna_seleccionada="GROUNDFORCE_LPA"
					empresa_entrada=30
				end if
				if oficina_query=6622 then
					hotel_seleccionado=6622 'GROUNDFORCE IBIZA 2015 UTE (BALEARES - ISLAS MENORES)
					contrasenna_seleccionada="GROUNDFORCE_IBZ"
					empresa_entrada=30
				end if
				if oficina_query=6625 then
					hotel_seleccionado=6625 'GROUNDFORCE PMI 2015 UTE (BALEAREA - ISLAS MAYORES)
					contrasenna_seleccionada="GROUNDFORCE_PMI"
					empresa_entrada=30
				end if
			else
				hotel_seleccionado=1465 'GROUNDFORCE SEVILLA UTE (PENINSULA)
				contrasenna_seleccionada="GROUNDFORCE_SVQ"
				empresa_entrada=30
		end if
	end if
	if empresa_query="GROUNDFORCE_AD" then
		hotel_seleccionado=846 'GLOBALIA HANDLING
		contrasenna_seleccionada="AEREA"
		empresa_entrada=30
	end if
	
	if empresa_query="AIR_EUROPA" then
		hotel_seleccionado=6653 'AIR EUROPA MTO-ALMACEN MADRID
		contrasenna_seleccionada="AEA-MTO-ALM"
		empresa_entrada=40
	end if
	if empresa_query="AIR_EUROPA_AD" then
		hotel_seleccionado=543 'AIR EUROPA LINEAS AEREAS, S.A.U.
		contrasenna_seleccionada="AEREA"
		empresa_entrada=40
	end if
	
	if empresa_query="CALDERON" then
		hotel_seleccionado=6981 'GROUNDFORCE CARGO IBIZA
		contrasenna_seleccionada="CALDERON_DCAL"
		empresa_entrada=50
	end if
	if empresa_query="CALDERON_AD" then
		hotel_seleccionado=1530 'GROUNDFORCE CARGO 
		contrasenna_seleccionada="AEREA"
		empresa_entrada=50
	end if

	if empresa_query="PORTUGAL" then
		hotel_seleccionado=5188 '952-LISBOA-AFONSO COSTA
		contrasenna_seleccionada="PORTUGAL_952"
		empresa_entrada=80
	end if
	if empresa_query="TRAVELPLAN" then
		hotel_seleccionado=6444 'WELCOME - IGEN DIRECCI�N GENERAL
		contrasenna_seleccionada="WELCOME_IGEN"
		empresa_entrada=90
	end if
	
	if empresa_query="TUBILLETE" then
		hotel_seleccionado=3872 'TUBILLETE
		contrasenna_seleccionada="TUBILLETE"
		empresa_entrada=100
	end if
	if empresa_query="TUBILLETE_AD" then
		hotel_seleccionado=6541 'TUBILLETE ADMINISTRADORA
		contrasenna_seleccionada="MINORISTA"
		empresa_entrada=100
	end if
	
	if empresa_query="GLOBALIA" then
		hotel_seleccionado=6645 'GSC - ADMON RECURSOS HUMANOS
		contrasenna_seleccionada="GSC_RHH"
		empresa_entrada=110
	end if
	if empresa_query="GLOBALIA_AD" then
		hotel_seleccionado=477 'GLOBALIA CORPORACION EMPRESARIAL
		contrasenna_seleccionada="GSC_ADMIN"
		empresa_entrada=110
	end if
	
	if empresa_query="GEOMOON" then
		hotel_seleccionado=7168 '4872 ENTRE DESTINOS (Y68)
		contrasenna_seleccionada="geomoonw72"
		empresa_entrada=130
	end if
	
	if empresa_query="GEOMOON_AD" then
		hotel_seleccionado=2497 'GEOMOON ADMINISTRADORA
		contrasenna_seleccionada="geomoon00"
		empresa_entrada=130
	end if
	
	if empresa_query="CORPORATE" then
		hotel_seleccionado=8205 'U39-EVENTOS MALAGA
		contrasenna_seleccionada="gct.U39"
		empresa_entrada=170
	end if
	
	if empresa_query="MARSOL" then
		hotel_seleccionado=9098 'MARSOL BARCELONA
		contrasenna_seleccionada="MARSOL.BARCELONA"
		empresa_entrada=210
	end if
	
	if empresa_query="MARSOL_AD" then
		hotel_seleccionado=1929 'MARSOL ADMINISTRADORA
		contrasenna_seleccionada="MARSOL.CENTRAL"
		empresa_entrada=210
	end if
	
	if empresa_query="IMPRENTA" then
		hotel_seleccionado=8932 'IMPRENTA
		contrasenna_seleccionada="imprenta"
		empresa_entrada=220
	end if
		
	if empresa_query="AVORIS" then
		hotel_seleccionado=7697 'AUTOCARES IBEROBUS - PALMA
		contrasenna_seleccionada="AUT-PMI"
		empresa_entrada=230
	end if
	if empresa_query="AVORIS_AD" then
		hotel_seleccionado=9873 'AVORIS ADMINISTRADORA
		contrasenna_seleccionada=""
		empresa_entrada=230
	end if
	
	if empresa_query="FHALCON" then
		hotel_seleccionado=10040 'Q12 HALCON franquicia
		contrasenna_seleccionada="HALCON.Q12"
		empresa_entrada=240
	end if
	
	if empresa_query="FECUADOR" then
		hotel_seleccionado=9922 'C05 BILBAO
		contrasenna_seleccionada="ECUADOR.C05"
		empresa_entrada=250
	end if
	
	if empresa_query="GENERAL" then
		hotel_seleccionado=10867 'PRUEBAS - MANUEL ALBA - GENERAL 1
		contrasenna_seleccionada="1"
		empresa_entrada=260
	end if
	...
	'para poder entrar como cualquer oficina
	id_cliente_seleccionado = "" & Request.QueryString("id_cliente_seleccionado")	
	if id_cliente_seleccionado<>"" then
		hotel_seleccionado = id_cliente_seleccionado
	end if	
	...
	'response.write("<br>consulta datos cliente: " & sql)
	...
	if not hoteles.eof then
		...
		if contrasenna_hotel=contrasenna_seleccionada or id_cliente_seleccionado<>"" then
			valido="SI"
			...
			tipo_precio=hoteles("tipo_precio")
			...
			'response.write("<br>consulta pedidos realizados: " & sql)
			cantidad_pedidos=0	
			with primer_pedido
				.ActiveConnection=connimprenta
				.Source=sql_primer
...
'CADE	170	GLOBALIA CORPORATE TRAVEL	GLOBALIA_CORPORATE_TRAVEL	1	#8232#
			...
			'response.write("<br>consulta admin" & sql)
			...
			valido="NO"
		end if ' del if contrasenna_hotel=contrasenna_seleccionada
	  else
	  	response.write("<br><br><b>EL CLIENTE " & hotel_seleccionado & " NO EXISTE O ESTÁ DADO DE BAJA...</b><br><br>")
	end if 'del hoteles.eof
	...
	'response.write("<br><br>final de la pagina.... sucursal: " & codigo_sucursal & "<br>Usuario: " & usuario & "<br>Contraseña: " & contrasenna)
%>
...
function moverse_mantenimiento(sino, administrador_central, carpeta, empresa, mostrar_aviso_popup, usuario) {

	$("#pantalla_popup_en_mantenimiento").modal("show");
	
	// Almacenar los par�metros para usarlos despu�s
    var params = {
        sino: sino,
        administrador_central: administrador_central,
        carpeta: carpeta,
        empresa: empresa,
        mostrar_aviso_popup: mostrar_aviso_popup,
        usuario: usuario
    };
	
	$('#pantalla_popup_en_mantenimiento').on('hidden.bs.modal', function () {
        moverse(params.sino, params.administrador_central, params.carpeta, params.empresa, params.mostrar_aviso_popup, params.usuario);
    });
...
function moverse(sino, administrador_central, carpeta, empresa, mostrar_aviso_popup, usuario)	{
	...
	//  globalia (los gsc)(110), geomoon(130), glovalia corporate travel(170), marsol(210), GAG(220), AVORIS (230), FRANQUICIAS HALCON (240)
	// FRANQUICIAS ECUADOR (250)
	if ((empresa==2)||(empresa==10)||(empresa==20)||(empresa==30)||(empresa==40)||(empresa==50)||(empresa==80)||(empresa==90)||
				(empresa==100)||(empresa==110)||(empresa==130)||(empresa==170)||(empresa==210)||(empresa==220)||(empresa==230)||(empresa==240)||(empresa==250))	{
	    cadena_carpeta='GAG/'
	    cadena_empresa='_Gag'			
	}		
	...
	if (sino=='SI')
		...
		if (administrador_central=='SI')
			{
		    ruta_redireccion= cadena_carpeta + 'Lista_Articulos' + cadena_empresa + '_Central_AD.asp'
			}
		  else 
			{
		...
		if (empresa==4)  
			...
			if ('<%=session("usuario_pais")%>' != 'PORTUGAL')
				{
				if ('<%=session("usuario_tipo")%>' != 'GLS PROPIA')
					{
					$("#pantalla_popup_gls_navidad").modal("show")
					//$("#pantalla_popup_gls_impresoras").modal("show")
					}
				  else
					{
					$("#pantalla_popup_gls_new_rotulacion").modal("show");
			...
			if (empresa==2)
				...
				<%if tipo_precio="BALEARES" then%>
					$("#pantalla_popup_be_live_amenities_ecologicos").modal("show");
				<%else%>
					$("#pantalla_popup_be_live_oferta").modal("show");
				<%end if%>
				}
			...
			if (empresa==5)
				...
				//$("#pantalla_popup_semana_santa_2021").modal("show"); 
				}
				
			//los clientes de Abba hoteles
			if (empresa==7)
				{
				document.getElementById('ocultoruta').value=ruta_redireccion	
				$("#pantalla_popup_abba_documentacion").modal("show"); 
			...
			if ((empresa==10)||(empresa==20)||(empresa==240)||(empresa==250))	
				{
					...
					if ((empresa==10)||(empresa==240))
						{
						cadena_imagen='popup/Tarjeta_de_Visita_Halcon.jpg'
						}
					  else
					  	{
						cadena_imagen='popup/Tarjeta_de_Visita_Ecuador.jpg'
						}
						
					<%if session("usuario_tipo")="FRANQUICIA" then%>	
						$("#imgtarjetas_hal_ecu").attr("src", cadena_imagen);
						$("#pantalla_popup_tarjetas_hal_ecu").modal("show");  		
					<%else%>
						$("#pantalla_popup_hal-ecu_globalbag_merchan").modal("show"); 
					<%end if%>
				...
				{
				if (empresa!=2 && empresa!=5 && empresa!=7 && empresa!=130)
					{
					location.href=ruta_redireccion
					}
				}
...
<div class="modal fade" id="pantalla_popup_en_mantenimiento" data-keyboard="false" data-backdrop="static">	
    <div class="modal-dialog modal-md" style="width:850px;">	  
      <div class="modal-content">	    
        <div class="container-fluid">
			<div class="col-md-12" align="center">
				<img class="img-responsive" src="popup/En_Mantenimiento_r.jpg" border="0">
				<br />
			</div>
		</div>	
...
<div class="modal fade" id="pantalla_popup_abba_documentacion" data-keyboard="false" data-backdrop="static">	
    <div class="modal-dialog modal-md">	  
      <div class="modal-content">	    
        <div class="container-fluid">
			<div class="col-md-12" align="center">
				<img class="img-responsive" src="popup/Abba_Documentacion_r.png" border="0">
			</div>
		</div>	
        <div class="modal-footer">                  
...
<div class="modal fade" id="pantalla_popup_tarjetas_hal_ecu" data-keyboard="false" data-backdrop="static">	
    <div class="modal-dialog modal-md">	  
      <div class="modal-content">	    
        <div class="container-fluid">
			<div class="col-md-12" align="center">
				<img class="img-responsive" src="" id="imgtarjetas_hal_ecu" border="0">
			</div>
		</div>	
        <div class="modal-footer">                  
          <p><button type="button" class="btn btn-default" data-dismiss="modal"><%=validar_ventana_mensajes_boton_cerrar%></button></p>                
...
<div class="modal fade" id="pantalla_popup_be_live_amenities_ecologicos" data-keyboard="false" data-backdrop="static">	
    <div class="modal-dialog modal-md">	  
      <div class="modal-content">	    
        <div class="container-fluid">
			<div class="col-md-12" align="center">
				<img class="img-responsive" src="popup/BE_LIVE_Nueva_Familia_Amenities_Ecologicos_r.jpg" border="0">
			</div>
		</div>	
        <div class="modal-footer">                  
          <p><button type="button" class="btn btn-default" data-dismiss="modal"><%=validar_ventana_mensajes_boton_cerrar%></button></p>                
...
<div class="modal fade" id="pantalla_popup_gls_new_rotulacion" data-keyboard="false" data-backdrop="static">	
    <div class="modal-dialog modal-md">	  
      <div class="modal-content">	    
        <div class="container-fluid">
			<div class="col-md-12" align="center">
				<img class="img-responsive" src="popup/GLS_Nuevas_Familias_Rotulacion.jpg" border="0">
			</div>
		</div>	
        <div class="modal-footer">                  
          <p><button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button></p>                
...
<div class="modal fade" id="pantalla_popup_gls_impresoras" data-keyboard="false" data-backdrop="static">	
    <div class="modal-dialog modal-md">	  
      <div class="modal-content">	    
        <div class="container-fluid">
			<div class="col-md-12" align="center">
				<img class="img-responsive" src="popup/GLS_Impresoras_Reparto_r.jpg" border="0">
			</div>
		</div>	
        <div class="modal-footer">                  
          <p><button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button></p>                
...
<div class="modal fade" id="pantalla_popup_gls_navidad" data-keyboard="false" data-backdrop="static">	
    <div class="modal-dialog modal-md">	  
      <div class="modal-content">	    
        <div class="container-fluid">
			<div class="col-md-12" align="center">
				<img class="img-responsive" src="popup/GLS_Reserva_Productos_Navidad_2024.jpg" border="0">
			</div>
		</div>	
        <div class="modal-footer">                  
          <p><button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button></p>                
...
$('#pantalla_popup_gls_new_rotulacion').on('hidden.bs.modal', function (e) {
	location.href=document.getElementById('ocultoruta').value
})

$('#pantalla_popup_gls_impresoras').on('hidden.bs.modal', function (e) {
	//location.href=document.getElementById('ocultoruta').value
	$("#pantalla_popup_gls_new_rotulacion").modal("show");
})
$('#pantalla_popup_gls_navidad').on('hidden.bs.modal', function (e) {
	$("#pantalla_popup_gls_impresoras").modal("show");
...
$('#pantalla_popup_be_live_amenities_ecologicos').on('hidden.bs.modal', function (e) {
  $("#pantalla_popup_be_live_oferta").modal("show"); 
})
...
$('#pantalla_popup_tarjetas_hal_ecu').on('hidden.bs.modal', function (e) {
  $("#pantalla_popup_hal-ecu_globalbag_merchan").modal("show");  
})
...
$('#pantalla_popup_abba_documentacion').on('hidden.bs.modal', function (e) {
  location.href=document.getElementById('ocultoruta').value	//para que se dirija a la pagina despues de cerrar el popup
  //$("#pantalla_popup_semana_santa_2021").modal("show");
})
