...
<%
	...
	set empresas=Server.CreateObject("ADODB.Recordset")
		...
		with empresas
			...
			'RESPONSE.WRITE("<BR><BR>" & now &  " - CONSULTA EMPRESAS: " & .SOURCE)
			.Open
			'RESPONSE.WRITE("<BR>" & now &  " - DESPUES DE CONSULTAR EMPRESAS")
		...
		with proveedores
			...
			'RESPONSE.WRITE("<BR><BR>" & now &  " - CONSULTA PROVEEDORES: " & .SOURCE)
			.Open
			'RESPONSE.WRITE("<BR>" & now &  " - DESPUES DE CONSULTAR PROVEEDORES")
		...
		with articulos
			...
			.Source= .Source & " ARTICULOS.SOLICITADO_AL_PROVEEDOR, ARTICULOS.PESO, ARTICULOS.PERMITE_DEVOLUCION"
			.Source= .Source & ", ARTICULOS.EN_AVORIS_SOLO_VER, ARTICULOS.CODIGO_ARANCELARIO"
			.Source= .Source & " FROM ARTICULOS WHERE ARTICULOS.ID=" & articulo_seleccionado
			
			'RESPONSE.WRITE("<BR><BR>" & now &  " - CONSULTA ARTICULO: " & .SOURCE)
			.Open
			'RESPONSE.WRITE("<BR>" & now &  " - DESPUES DE CONSULTAR ARTICULO")
		...
		campo_peso=""
		campo_devolucion=""
		campo_en_avoris_solo_ver=""
		campo_codigo_arancelario=""
		
		if not articulos.eof then
			campo_codigo_sap=articulos("codigo_sap")
			campo_descripcion=REPLACE(articulos("descripcion"), """", "&quot;")
			...
			campo_devolucion=articulos("permite_devolucion")
			campo_en_avoris_solo_ver=articulos("en_avoris_solo_ver")
			campo_codigo_arancelario = articulos("codigo_arancelario")
			
		end if
		
		'RESPONSE.WRITE("<BR>variable descripcion: " & campo_descripcion)
		'RESPONSE.WRITE("<BR>campo descripcion: " & articulos("descripcion"))
		...
		with familias
			...
			'RESPONSE.WRITE("<BR><BR>" & now &  " - CONSULTA FAMILIAS: " & .SOURCE)
			.Open
			vacio_familias=false
			if not .BOF then
				mitabla_familias=.GetRows()
			  else
				vacio_familias=true
			end if
		end with
		...
		'RESPONSE.WRITE("<BR>" & now &  " - DESPUES DE FAMILIAS")
		...
		with stocks_articulo
			...
			.Source=sql
			'RESPONSE.WRITE("<BR><BR>" & now &  " - CONSULTA STOCKS ARTICULO: " & .SOURCE)
			.Open
			'RESPONSE.WRITE("<BR>" & now &  " - DESPUES DE CONSULTAR STOCK ARTICULO")
		end with
...
'RESPONSE.WRITE("<BR><BR>FINAL DEL CODIGO DE CABECERA")		
			...
			<div class="container-fluid">
				...
				<div class="panel panel-default">
				  <div class="panel-body">
				  	<form name="frmdatos_articulo" id="frmdatos_articulo" method="post" action="Guardar_Articulos_GAGAD.asp">
								  ...
								  <section id="content1" class="contenido_pestannas">
								  		<div class="form-group row h-100">
											<div class="col-3 my-auto">
												<div class="d-flex flex-column mx-auto text-center" id="contenedorImagen">
													<span>
													<%if articulo_seleccionado&""<>"" and articulo_seleccionado&""<>"0" then%>
														...
														<%
															Dim timestamp
															timestamp = DateDiff("s", "01/01/1970 00:00:00", Now()) ' Obtiene el timestamp actual
														%>
														<a href="Imagenes_Articulos/<%=carpeta_marca%><%=articulo_seleccionado%>.jpg" target="_blank" id="imagen_enlace">
															<!-- El timestamp sirve para que la imagen no se almacene en cache -->
															<img class="img-responsive" src="Imagenes_Articulos/<%=carpeta_marca%>Miniaturas/i_<%=articulo_seleccionado%>.jpg?timestamp=<%= timestamp %>" border="0" id="imagen_articulo"></a>	
													<%end if%>
													</span>
												</div>	
											</div>
										...
										<div class="form-group row mx-1">
											...
											<div class="col-sm-2 col-md-2 col-lg-2">
												<label for="cmbdevolucion" class="control-label"
													data-toggle="popover"
													data-placement="top"
													data-trigger="hover"
													data-content="¿Permite Devoluci&oacute;n?"
													data-original-title=""
													>Devoluci&oacute;n</label>
												<select class="form-control"  name="cmbdevolucion" id="cmbdevolucion">
												...
												<script language="JavaScript" type="text/javascript">
													if ('<%=campo_devolucion%>'!='')
														{
														document.getElementById("cmbdevolucion").value='<%=campo_devolucion%>'
														}
												</script>
											</div>
											<div class="col-sm-2 col-md-2 col-lg-2">
										...
										<div class="form-group row mx-1">
											...
											<div class="col-sm-2 col-md-2 col-lg-2">
												<label for="cmben_avoris_solo_ver" class="control-label"
													data-toggle="popover"
													data-placement="top"
													data-trigger="hover"
													data-content="En Las Cadenas de AVORIS se pueve ver pero no Pedir"
													data-original-title=""
													>AVORIS Solo Ver</label>
												<select class="form-control"  name="cmben_avoris_solo_ver" id="cmben_avoris_solo_ver">
													<option value="NO" selected>NO</option>
													<option value="SI">SI</option>
												</select>
											  	<%if campo_en_avoris_solo_ver<>"" then%>
													<script language="JavaScript" type="text/javascript">
														document.getElementById("cmben_avoris_solo_ver").value='<%=campo_en_avoris_solo_ver%>'
													</script>
												<%end if%>
											</div>
										</div>
										...
										<div class="form-group row mx-1">
											<div class="col-sm-2 col-md-2 col-lg-2">
												<label for="txtcodigo_arancelario" class="control-label">Cod. Arancelario</label>
												<input type="text" class="form-control" name="txtcodigo_arancelario" id="txtcodigo_arancelario" maxlength="25" value="<%=campo_codigo_arancelario%>"/>
											</div>
										</div>
														...
														<tr>
															...
															<td>
																...
																<select class="form-control cmbempresas_familias"  name="cmbfamilias_<%=mitabla_empresas(CAMPO_ID_EMPRESA,i)%>" id="cmbfamilias_<%=mitabla_empresas(CAMPO_ID_EMPRESA,i)%>" style="display:none ">
																	<option value="" selected>* Seleccione *</option>
																	<%if vacio_familias=false then
																			for j=0 to UBound(mitabla_familias,2)
																				if mitabla_familias(CAMPO_EMPRESA_FAMILIA,j) = mitabla_empresas(CAMPO_ID_EMPRESA,i) then
																					txt_html=""
																					txt_html = "<option value=""" & cint(mitabla_familias(CAMPO_ID_FAMILIA,j)) & """>"
																					txt_html = txt_html & mitabla_familias(CAMPO_DESCRIPCION_FAMILIA,j)
																					txt_html = txt_html & "</option>"
																					response.write(txt_html)
																				end if
																			next
																	end if%>
																</select>
...
//console.log('apartado final de javascript')  

j$(document).ready(function () {
	//console.log('... ENTRAMOS EN EL READY')
...
mostrar_cmbempresa = function(elemento) {
	...
	//console.log('... FUNCION MOSTRAR_CMBEMPRESA')
...
refrescar_stock = function() {
	//console.log('... FUNCION_REFRESCAR_STOCK')
	j$.ajax({
		type: "post",        
...
borrar_entrada = function(id, cantidad, fecha, albaran, tipo, id_articulo) {
	//console.log('... FUNCION BORRAR ENTRADA')
	j$.ajax({
		type: "post",        
...
j$("#cmdguardar_entradas").click(function () {
	//console.log('... CLICK EN CMDGUADAR ENTRADAS')
...
j$("#cmdguardar_salidas").click(function () {
	//console.log('... CLICK EN CMDGUARDAR SALIDAS')
	cadena_url='Guardar_Entradas_Salidas_Articulos.asp'
...
j$("#tab4").click(function(){
	//console.log('... CLIK EN TAB4 - CANTIDADES PRECIOS')
	j$("#capa_cantidades_precios_externa").html('<iframe id="iframe_cantidades_precios" src="Obtener_Cantidades_Precios_Articulo.asp?id_articulo=<%=articulo_seleccionado%>&compromiso_compra=<%=campo_compromiso_compra%>" width="100%" frameborder="0" transparency="transparency" onload="redimensionar_iframe()" scrolling="yes"></iframe>');
...
cargar_entradas = function() {
	//console.log('... FUNCION CARGAR ENTRADAS')  
      var err ="";
...
cargar_salidas = function() {  
	//console.log('... FUNCION CARGAR SALIDAS')
      var err ="";
...
<script languaje="javascript">
	let id_articulo= '<%=articulo_seleccionado%>';
	if (id_articulo!=0) {
...
<script languaje="javascript">
	// let idArticulo="<%=articulo_seleccionado%>";
	// console.log("El id del articulo es: "+idArticulo);
	if (id_articulo!=0) {
		let botonSubidaImagen = document.createElement("button");
		botonSubidaImagen.setAttribute("type", "button");
		botonSubidaImagen.setAttribute("id", "botonSubidaImagen");
		botonSubidaImagen.classList.add("btn", "btn-primary", "mt-1", "mx-auto");
		botonSubidaImagen.style.width = "180px";
		botonSubidaImagen.textContent = "Seleccionar Imagen";
		
		document.querySelector("#contenedorImagen").appendChild(botonSubidaImagen);
		document.querySelector("#botonSubidaImagen").addEventListener("click",function(){
			mostrar_imagen_articulo(id_articulo);
		});
	}
</script>
...
<%
	...
	'RESPONSE.WRITE("<BR>CONSULTA ARTICULOS_EMPRESAS: " & .SOURCE)
	.Open
	end with
