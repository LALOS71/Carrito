...
<BODY onload="validar('<%=mensaje_aviso%>', '<%=accion%>')">
	...
	'conn.execute sql
%>
<form name="frmconfirmar_pedido" id="frmconfirmar_pedido" method="post" action="Consulta_Pedidos_GAGAD.asp">
</form>

<form action="Rellenar_Variables_Sesion_Imprenta_GAGAD.asp" method="post" name="frmmodificar_pedido" id="frmmodificar_pedido">
	<input type="hidden" id="ocultopedido_a_modificar" name="ocultopedido_a_modificar" value="<%=pedido_a_modificar%>" />
	<input type="hidden" id="ocultoempresa_pedido" name="ocultoempresa_pedido" value="<%=empresa_pedido%>" />
</form>
