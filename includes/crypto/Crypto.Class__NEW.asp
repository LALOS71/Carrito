...
'sitio de descarga: https://github.com/as08/ClassicASP.PasswordHashing
    ...
    class crypto
        ...
        public function verifyPassword(ByVal pass, ByVal theHash)
            response.write("<br>dentro de verifypassword")
            ' Invalid by default
            ...
            elseif inStr(1,theHash,"$PBKDF2",1) = 1 then
				...
				response.write("<br>thehash0: " & theHash(0))
				response.write("<br>thehash1: " & theHash(1))
				response.write("<br>thehash2: " & theHash(2))
				response.write("<br>thehash3: " & theHash(3))
				
				response.write("<br>tamaño uBound(theHash): " & uBound(theHash))
				response.write("<br>ultimo else, inicia con $sha256")
				
                if NOT uBound(theHash) = 4 then exit function
                ' Decode the salt
                Dim decoded_salt
				response.write("<br>thehash4: " & theHash(4))
				response.write("<br>thehash2: " & theHash(2))
				response.write("<br>decodesalt: " & decodeSalt(theHash(4),theHash(2)))
                decoded_salt = decodeSalt(theHash(4),theHash(2))
                ' was the pepper used during the original hash? If so, apply during 
                ' the verification
				response.write("<br>thehash1 para buscar virgulillaP: " & theHash(1))
                ...
                ' password hash
				response.write("<br>pass decodificada: " & pass)
                if theHash(3) = _
