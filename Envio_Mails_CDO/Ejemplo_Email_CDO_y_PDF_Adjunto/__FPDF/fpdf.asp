...
this.SetFont=function SetFont(xfamily , xstyle , xsize)
	...
	this.Output=function Output(xfile , xdownload , Overwrite)
		...
		if (this.hasBinary){
			Response.Write("<br>hasbinary false")
			Response.Write(this.buffer)}
		else{

			Response.Write("hasbinary true")
			...
			{
				//dejamos el nombre pasado por parametro
				//xfile=Server.MapPath(lib.fso.GetTempName())
				xfile=Server.MapPath(xfile)
			}
			else
			{
				//reconstruimos la ruta con la ruta relativa pasada
			...
			Response.Write("vemos la ruta: " + xfile)								
