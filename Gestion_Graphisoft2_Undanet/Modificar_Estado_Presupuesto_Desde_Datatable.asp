...
<%
	...
	if id_estado_nuevo<>5 and id_estado_nuevo<>6 and id_estado_nuevo<>3 then
		...
		if id_estado_nuevo=5 or id_estado_nuevo=3 then
			if proxima_revision_nuevo="" or proxima_revision_nuevo=null then
				cadena_ejecucion = cadena_ejecucion & ", PROXIMA_REVISION=NULL" 
			  else
	...
	if id_estado_antiguo<>id_estado_nuevo and id_estado_nuevo=ID_ESTADO_SEGUIMIENTO then
		enviado = mail_seguimiento(presupuesto)
	end If
