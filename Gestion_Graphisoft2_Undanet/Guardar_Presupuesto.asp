...
<%
	...
	proxima_revision = "" & Request.Form("txtfecha_proxima_revision")
	
	'RESPONSE.WRITE("<br><bR>OBTENEMOS EL ID_PRESUPUESTO DESDE EL FORM: " & id_presupuesto)
	'RESPONSE.WRITE("<br><bR>OBTENEMOS EL PRESUPUESTO DESDE EL FORM: " & presupuesto)
	
	
	if proxima_revision<>"" then
		proxima_revision=cdate(proxima_revision)
	end if
	
	
	'response.Write("<br><br>proxima revision: " & proxima_revision)
	...
	If id_presupuesto <> "" Then
		sql = "SELECT B.ID_ESTADO, B.ID_SUBESTADO, B.OBSERVACIONES_GESTION OBSERVACIONES_LOCAL, B.PROXIMA_REVISION"
		...
		If Not presupuesto_ant.EOF Then
			...
			campo_proxima_revision = "" & presupuesto_ant("proxima_revision")
			if campo_proxima_revision<>""then
				campo_proxima_revision=cdate(campo_proxima_revision)
			end if
		End If
		...
		If campo_estado <> estado Then		
			...
			if estado=ID_ESTADO_SEGUIMIENTO then
				'RESPONSE.WRITE("<br>enviamos email de seguimiento para el presupuesto: " & presupuesto & " con ID_PRESUPUESTO: " & id_presupuesto)
		...
		If campo_proxima_revision <> proxima_revision Then		
			''''''''''''''''''''''''''''''''''''''''''''''''''''
			'''' no solo hay que poner el del presupuesto concreto, sino todos los del raiz
			'''''''''''''''''''''''''''''''''''''''''''''''''
			
			'RESPONSE.WRITE("<br><br>cambia la fecha de proxima revisiion")
			CAMPO_ID_PRESUPUESTO = 0
	
			'RESPONSE.WRITE("<br><br>antes del sql")
			'RESPONSE.WRITE("<br><br>id_presupuesto: " & id_presupuesto)
			'RESPONSE.WRITE("<br><br>presupuesto: " & Request.Form("ocultopresupuesto"))
			' GetEstados query
			'presupuesto		= "" & Request.Form("ocultopresupuesto")
			presupuesto_aux = "" & Request.Form("ocultopresupuesto")
			'sql_grupo_presupuestos = "SELECT ID_PRESUPUESTO FROM GESTION_GRAPHISOFT_PRESUPUESTOS WHERE PRESUPUESTO=" & presupuesto
			sql_grupo_presupuestos = "SELECT ID_PRESUPUESTO FROM GESTION_GRAPHISOFT_PRESUPUESTOS WHERE PRESUPUESTO=" & presupuesto_aux
			
			
			vacio_presupuestos = false
		
			'RESPONSE.WRITE("<br><br>cadena seleccion todas las vesiones de presupuesos: " & sql_grupo_presupuestos)
			Set grupo_presupuestos = execute_sql(conn_gag, sql_grupo_presupuestos)
			If Not grupo_presupuestos.BOF Then
				tabla_presupuestos = grupo_presupuestos.GetRows()
			Else
			...
			if not vacio_presupuestos then
				for i=0 to UBound(tabla_presupuestos,2)
					...
					cadena_historico = "INSERT INTO GESTION_GRAPHISOFT_HISTORICO_PRESUPUESTOS (ID_PRESUPUESTO, PRESUPUESTO, FECHA, ACCION, CAMPO, VALOR_ANTIGUO, VALOR_NUEVO,"
					cadena_historico = cadena_historico & "USUARIO, DESCRIPCION, TIPO_INCIDENCIA)"
					cadena_historico = cadena_historico & " SELECT " & tabla_presupuestos(campo_id_presupuesto,i) & ", " & presupuesto_aux & ","
					cadena_historico = cadena_historico & " GETDATE(), 'MODIFICA', 'PROXIMA REVISION', '" & campo_proxima_revision & "',"
					cadena_historico = cadena_historico & " '" & proxima_revision & "', '" & session("usuario") & "', '" & cadena_explicacion & "', NULL"
			...
			'cambio la proxima revision de todas las versiones de un mismo presupuesto
			if proxima_revision="" then
				sql = "UPDATE GESTION_GRAPHISOFT_PRESUPUESTOS SET PROXIMA_REVISION=NULL"
			  else
			  	sql = "UPDATE GESTION_GRAPHISOFT_PRESUPUESTOS SET PROXIMA_REVISION='" & CDATE(proxima_revision)  & "'"
			end if
			'si no es en estudio o rechazado, el subestado va a nulo
			'if id_estado<>5 and id_estado<>6 then
			'	sql = sql & ", ID_SUBESTADO=NULL" 
			'end if
			sql = sql & " WHERE PRESUPUESTO=" & presupuesto_aux
		...
		'if mensaje_aviso="" then
			mensaje_aviso = mensaje_aviso & "<br />PRESUPUESTO MODIFICADO CON ÉXITO"
		'end if
	End If	
%>
<html lang="es">
