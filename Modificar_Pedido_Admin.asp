...
sub comprobar_envio_email_stock(codigo_sap_articulo, descripcion_articulo, stock_articulo, stock_minimo_articulo, marca_articulo)
	...
	mensaje=mensaje & "<BR>Descripci�n Articulo: " & descripcion_articulo
	...
	'response.write("<BR>--------------------------------<br>3B ------------------------ ACABAMOS DEL FOR<br>")
		...
		if not estado_pedido.eof then
			...
			'si algunos de los articulos est� en enviado, el estado del pedido a de ser ENVIO PARCIAL
			...
			if estado_pedido("ESTADO")="ENVIADO" or estado_pedido("ESTADO")="LISTO" THEN
				...
				'si algunos de los articulos est� en LISTO  o LISTO PARCIAL enviado, el estado del pedido a de ser LISTO PARCIAL
		...
		if acciones="ALBARAN" then
			...
			if not pedido_al_proveedor.eof then
				...
				''''''''tambien habra que a�adir las condiciones de que el estado sea ENVIADO y con el albaran a NULL
			...
			if creamos_albaran="SI" then
				   ...
				   cmd.CommandType = 4 'adCmdStoredProc
							...
							if estado_detalle="ENVIADO" OR estado_detalle="ENVIADO AL PROVEEDOR" then
								if albaran_detalle=albaran_nuevo then
									if albaran_nuevo=albaran_parcial_detalle or albaran_parcial_detalle="" then
										...
										'si es una impresora de gls, hay que a�adir los numeros de serie de dichas impresoras para su gestion posterior
										...
										if id_articulo_detalle="4583" then
											...
											For i = LBound(numeros_de_serie) To UBound(numeros_de_serie)
												...
												' para que a�ada un retorno de carrocada 4 numeros de serie porque se sale del ancho
												If (i + 1) Mod 4 = 0 Then
											...
											For i = LBound(numeros_de_serie) To UBound(numeros_de_serie)
												...
												'	'nos salimos de la ejecucion porque la impresora est� activa
												'	connimprenta.RollBackTrans
												'	Response.End
												'  else
												  	'si no est� activa, o se actualiza o se da de alta
					...
					' pero miro a ver si ya hay albaran previo (por envio parcial), ya que se aplicar� el descuento en el primero
					if  descuento_pedido<>"" AND descuento_pedido<>"0" then
						...
						with albaran_previo
							...
							'hay que cambiarlo para lo de globaliagifts que el numero de pedido ser� alfanumerico
					...
					if datos_devoluciones<>"" then
						hay_albaran_previo="NO"
							...
							with albaran_previo
								...
								'.Source= .Source & " WHERE NPedido = '" & pedido_albaran & "'"
								.Source= .Source & " WHERE NPedido = " & pedido_albaran
								.Source= .Source & " and b.concepto like 'Devoluci�n %'"
								'response.write("<br>74 - VEMOS SI HAY ALBARAN PREVIO PARA A�ADIR DETALLE DE DEVOLUCIONES: " & .source)
								.OPEN
							end with
								...
								for each x in tabla_devoluciones
									if x <>"" then
										...
										importe_devolucion=devolucion(1)
											...
											cadena_detalles_albaran="1, " & albaran_nuevo & ", 1, NULL, 'Devoluci�n " & id_devolucion & "'"
					...
					' pero miro a ver si ya hay albaran previo (por envio parcial), ya que se cargar� en el primero los gastos de envio
					...
					while not observaciones_albaran.eof
						if ((observaciones_albaran("estado")<>"ENVIADO") AND (observaciones_albaran("estado")<>"ENVIADO AL PROVEEDOR") AND (observaciones_albaran("estado")<>"ANULADO")) THEN
							if cadena_articulos_pendientes="" then
								cadena_articulos_pendientes="Quedar�an Pendientes de Enviar los Siguientes Articulos:" & chr(13)
							end if
...
function validar(creamos_albaran,numero_albaran, accion, enviar_email_al_proveedor, sn_impresora_activa)
{
	...
	if (enviar_email_al_proveedor=='SI')
		{
		cadena= cadena + '\n\nAl ser un pedido de ROTULACIÓN se enviar&aacute; tambien un correo con este pedido al Proveedor para que lo Gestione Directamente.'
		}
