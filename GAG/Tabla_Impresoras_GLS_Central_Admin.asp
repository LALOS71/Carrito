...
<%
	...
	if sesion_caducada= "NO" then
		...
		renting_seleccionado = "" & Request.Form("renting")
		...
		sql = sql & ", A.ESTADO, A.RENTING_IMPRESORA_GLS AS RENTING"
		...
		if renting_seleccionado<>"" then
			sql = sql & " AND A.RENTING_IMPRESORA_GLS = " & renting_seleccionado
		end if	
		...
		'sql = sql & ", B.ID_PEDIDO , CONVERT(varchar, A.FECHA, 103) as FECHA_ALTA"
		sql = sql & ", B.ID_PEDIDO , CONVERT(varchar, A.FECHA, 103) as ULTIMA_FECHA"
		sql = sql & ", 'PENDIENTE' AS ESTADO, A.RENTING_IMPRESORA_GLS AS RENTING"
		sql = sql & " FROM PEDIDOS A"
		sql = sql & " INNER JOIN PEDIDOS_DETALLES B ON A.ID=B.ID_PEDIDO"
		sql = sql & " LEFT JOIN V_CLIENTES C ON C.ID=A.CODCLI"
		...
		'sql = sql & ", A.SN_IMPRESORA, A.ID_PEDIDO, CONVERT(varchar, A.FECHA_ALTA, 103) AS FECHA_ALTA, A.ESTADO, A.RENTING AS RENTING"
		sql = sql & ", A.SN_IMPRESORA, A.ID_PEDIDO, CONVERT(varchar, HIS.FECHA, 103) AS ULTIMA_FECHA, A.ESTADO, A.RENTING AS RENTING"
		...
		if renting_seleccionado<>"" then
			sql = sql & " AND A.RENTING = " & renting_seleccionado
		end if	
		...
		response.write("<th>Fact.</th>")
		response.write("<th>Rent.</th>")
		...
		while not rs_impresoras.eof
			...
			response.write("<td>" & rs_impresoras("RENTING") & "</td>")
			...
			if rs_impresoras("ESTADO")="SOLICITUD BAJA" then
				...
				response.write("<button class='btn btn-primary btn-sm' onclick=""realizar_accion('" & impresora & "', 'APROBAR-RECHAZAR_BAJA')""")
			...
			if rs_impresoras("ESTADO")="SOLICITUD DEFECTUOSA" then
				response.write("<div class='container'>")
				response.write("<div class='form-inline'>")
				response.write("<div class='form-group'>")
				response.write("<select class='form-control mr-2 acciones' name='cmbacciones_" & rs_impresoras("SN_IMPRESORA_PEDIDOS") & "' id='cmbacciones_" & rs_impresoras("SN_IMPRESORA_PEDIDOS") & "'>")
				response.write("<option value='' selected>Seleccionar Accion</option>")
				response.write("<option value='DEFECTUOSA'>APROBAR DEFECTUOSA</option>")
				response.write("<option value='DEFECTUOSA RECHAZADA'>RECHAZAR DEFECTUOSA</option>")
				response.write("</select>")
				response.write("</div>") 
				
				impresora=rs_impresoras("SN_IMPRESORA_PEDIDOS")
				response.write("<button class='btn btn-primary btn-sm' onclick=""realizar_accion('" & impresora & "', 'APROBAR-RECHAZAR_DEFECTUOSA')""")
				response.write(" data-toggle='popover'")
				response.write(" data-placement='top'")
				response.write(" data-trigger='hover'")
				response.write(" data-content='Guardar Accion'")
			...
			if rs_impresoras("ESTADO")="SOLICITUD AVERIADA" then
				response.write("<div class='container'>")
				response.write("<div class='form-inline'>")
				response.write("<div class='form-group'>")
				response.write("<select class='form-control mr-2 acciones' name='cmbacciones_" & rs_impresoras("SN_IMPRESORA_PEDIDOS") & "' id='cmbacciones_" & rs_impresoras("SN_IMPRESORA_PEDIDOS") & "'>")
				response.write("<option value='' selected>Seleccionar Accion</option>")
				response.write("<option value='AVERIADA'>APROBAR AVERIADA</option>")
				response.write("<option value='AVERIADA RECHAZADA'>RECHAZAR AVERIADA</option>")
				response.write("</select>")
				response.write("</div>") 
				
				impresora=rs_impresoras("SN_IMPRESORA_PEDIDOS")
				response.write("<button class='btn btn-primary btn-sm' onclick=""realizar_accion('" & impresora & "', 'APROBAR-RECHAZAR_AVERIADA')""")
				response.write(" data-toggle='popover'")
				response.write(" data-placement='top'")
				response.write(" data-trigger='hover'")
				response.write(" data-content='Guardar Accion'")
