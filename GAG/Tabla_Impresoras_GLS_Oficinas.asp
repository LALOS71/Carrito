...
<%
		...
		sql = "SELECT 'PENDIENTE DE FIRMA ' + CAST(B.CANTIDAD AS VARCHAR(10)) + ' IMP.' AS SN_IMPRESORA"
		sql = sql & ", B.ID_PEDIDO AS ID_PEDIDO_FIN , CONVERT(varchar, A.FECHA, 103) as FECHA_ALTA"
		sql = sql & ", A.ESTADO"
		sql = sql & " FROM PEDIDOS A"
		sql = sql & " INNER JOIN PEDIDOS_DETALLES B ON A.ID=B.ID_PEDIDO"
		sql = sql & " WHERE CODCLI = " & cliente_seleccionado
		sql = sql & " AND PEDIDO_AUTOMATICO='IMPRESORA_GLS_ADMIN'"
		sql = sql & " AND B.ARTICULO=4583"
		sql = sql & " AND B.ESTADO = 'PENDIENTE_FIRMA'"

		sql = sql & " UNION "
		
		sql = sql & "SELECT 'PENDIENTE ENVIAR ' + CAST(B.CANTIDAD AS VARCHAR(10)) + ' IMP.' AS SN_IMPRESORA, B.ID_PEDIDO AS ID_PEDIDO_FIN"
		sql = sql & " , CONVERT(varchar, A.FECHA, 103) as FECHA_ALTA, 'PENDIENTE' AS ESTADO"
		sql = sql & " FROM PEDIDOS A"
		sql = sql & " INNER JOIN PEDIDOS_DETALLES B ON A.ID=B.ID_PEDIDO"
		sql = sql & " WHERE CODCLI = " & cliente_seleccionado
		sql = sql & " AND (PEDIDO_AUTOMATICO='IMPRESORA_GLS' OR PEDIDO_AUTOMATICO='IMPRESORA_GLS_ADMIN' OR PEDIDO_AUTOMATICO='IMPRESORA_GLS_GAG')"
		sql = sql & " AND B.ARTICULO=4583"
		sql = sql & " AND B.ESTADO NOT IN ('ENVIADO','RECHAZADO', 'ENVIO PARCIAL', 'ANULADO', 'PENDIENTE_FIRMA')"
		
		sql = sql & " UNION"
		
		sql = sql & " SELECT SN_IMPRESORA, ID_PEDIDO AS ID_PEDIDO_FIN, CONVERT(varchar, FECHA_ALTA, 103) as FECHA_ALTA, ESTADO FROM GLS_IMPRESORAS"
		sql = sql & " WHERE ID_CLIENTE = " & cliente_seleccionado
		sql = sql & " ORDER BY ID_PEDIDO_FIN DESC"
...
response.write("<tbody>")
	...
	while not rs_impresoras.eof
		...
		response.write("<td>" & rs_impresoras("ID_PEDIDO_FIN") & "</td>")
		...
		if rs_impresoras("ESTADO") = "ACTIVA" then
			...
			if DateDiff("d", rs_impresoras("FECHA_ALTA"), now()) <= 20 then
				response.write("<option value='SOLICITUD DEFECTUOSA'>Notificar Imp. DEFECTUOSA</option>")
			end if
			response.write("<option value='SOLICITUD AVERIADA'>Notificar Imp. AVERIADA</option>")
