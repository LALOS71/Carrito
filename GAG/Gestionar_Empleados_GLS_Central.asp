...
<%
	...
	with centros_coste
		...
		.Source="SELECT ID, NOMBRE"
		.Source= .Source & " FROM V_CLIENTES"
		.Source= .Source & " WHERE TIPO='GLS PROPIA'"
		.Source= .Source & " AND BORRADO='NO'"
		.Source= .Source & " AND ID NOT IN (7730, 7637, 2784)"
		.Source= .Source & " ORDER BY NOMBRE"
		.Open
					...
					<form name="frmbuscar_articulos" id="frmbuscar_articulos" method="post" action="Consulta_Articulos_Admin.asp">	
						...
						<div class="form-group row mx-2">
							...
							<div class="col-sm-3 col-md-3 col-lg-3">
								...
								<input type="text" class="form-control" name="txtnombre" id="txtnombre" value="" maxlength="50"/>
							</div>
							<div class="col-sm-5 col-md-5 col-lg-5">
								<label for="txtapellidos" class="control-label">Apellidos</label>
								<input type="text" class="form-control" name="txtapellidos" id="txtapellidos" value="" maxlength="100"/>
							</div>
							<div class="col-sm-2 col-md-2 col-lg-2">
								...
								<select class="form-control" name="cmbgrupo_ropa" id="cmbgrupo_ropa">
									...
									<option value="11">11 (Grupo 1 Madrid)</option>
								</select>
							</div>
						</div>	
						...
						<div class="form-group row mx-2">
							...
							<div class="col-sm-2 col-md-2 col-lg-2">
								<label for="cmbborrado" class="control-label">Borrado</label>
								<select class="form-control" name="cmbborrado" id="cmbborrado">
						...
						<div class="form-group row mx-2">
							<div class="col-sm-3 col-md-3 col-lg-3">
								<button type="button" class="btn btn-info" id="cmdvolver" name="cmdvolver">
...
<%
	...
	with empleados_typeahead
		...
		.Source=.Source & " ID, NIF, NOMBRE, APELLIDOS, EMAIL, BORRADO, GRUPO_ROPA, CENTRO_COSTE, NUEVO, FECHA_ALTA"
	...
	do until empleados_typeahead.eof
		...
		cadena_empleados=cadena_empleados & ", 'BORRADO':  '" & empleados_typeahead("BORRADO") & "'"
	...
	j$.typeahead({
		...
		callback: {
			onClick: function (node, a, item, event) {
				...
				j$("#cmbborrado").val(item.BORRADO)
				j$("#cmbgrupo_ropa").val(item.GRUPO_ROPA)
				j$("#cmbcentro_coste").val(item.CENTRO_COSTE)
				console.log('tipo empleado: ' + item.NUEVO)
				if (item.NUEVO=='Verdadero' || item.NUEVO=='True')
					{
				...
				j$("#cmbborrado").prop('disabled', true);
			...
			onCancel: function (node, event) {
				...
				j$("#cmbborrado").prop('disabled', false);
...
j$("#cmdguardar").click(function () {
	...
	if (j$("#cmbborrado").val()=='')
		{
		cadena_error=cadena_error + '- No se ha seleccionar si el Empleado est&oacute; Borrado o No.<br>'
		}
	if (j$("#txtemail").val()=='')
		{
	...
	if (cadena_error!='')
		...
		bootbox.alert({
	  ...
	  else
	  	{
			...
			cadena_mensaje+= 'BORRADO: <B>' + j$("#cmbborrado option:selected").text() + '</B><br>'
