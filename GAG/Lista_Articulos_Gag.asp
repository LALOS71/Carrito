...
<%
		...
		if familia_buscada="" and familias_buscadas_otra="" and articulo_buscado="" and codigo_sap_buscado="" and agrupacion_familia_buscada="" and campo_autorizacion="" then
			...
			'si no se filtra por nada, que muestre los articulos que no requieren autorizacion, salvo para Air Europa que vacia este filtro
			campo_autorizacion="NO"
			if session("usuario_codigo_empresa")=40 then
				campo_autorizacion = ""
			end if
			
			'para la cadena Hospes, que se muestren por defecto los articulos de IMPRENTA, familia 370
			if session("usuario_codigo_empresa")=280 then
				familia_buscada="370"
				familias_buscadas_otra="370"
				realizar_consulta="SI"
			end if
		end if
		...
		if realizar_consulta="NO" and session("usuario_codigo_empresa")<> 280 then
			...
			sql=sql & ", ARTICULOS_PDF_DATOS_TECNICOS.DESCRIPCION_BOTON AS DESCRIPCION_BOTON_PDF"
			...
			if session("usuario_codigo_empresa")=4 then
				if session("usuario_tipo")= "GLS PROPIA" _
					AND session("usuario")<>5089 AND session("usuario")<>7395 _
					AND session("usuario")<>11340 AND session("usuario")<>11341 _
					AND session("usuario")<>11537 then
					if session("usuario_tipoprecio")<>"PROPIA_PORTUGAL" then
			...
			if session("usuario_codigo_empresa")=4 then		
				...
				if session("usuario_pais")<>"PORTUGAL" then
				  ...
				  else 'portugal ya ve la familia del vestuario nuevo logo preventa, y sigue sin ver los productos de navidad
				  	'if session("usuario_tipo")= "GLS PROPIA" then
			...
			sql=sql & " LEFT JOIN TALLAJES ON TALLAJES.ID_ARTICULO=CONSULTON.ID"
			sql=sql & " LEFT JOIN ARTICULOS_MARCAS ON ARTICULOS_MARCAS.ID_ARTICULO=CONSULTON.ID"
			sql=sql & " LEFT JOIN ARTICULOS_PDF_DATOS_TECNICOS ON ARTICULOS_PDF_DATOS_TECNICOS.ID_ARTICULO=CONSULTON.ID"
			'se pone al final, que sino, falla el union
			'sql=sql & " ORDER BY DESCRIPCION_IDIOMA, DESCRIPCION_GRUPO, TALLAJES.ORDEN"



			'2023-03-02 - AHORA TODAS LAS FRANQUICIAS DE GLS PUEDEN PEDIR ESTOS ARTICULOS DE LA FAMILIA GLS SEGURIDAD, Y DE LAS PROPIAS, SOLO COMPRAS 280-5 (5089) y 280-51 GLS PERDIDAS (7395)
			' y P280-5 COMPRAS PORTUGAL (11340) y P280-51 PERDIDAS PORTUGAL (11341)
			' la nueva propia de IRLANDA - I280-5 COMPRAS IRLANDA (11537)
			...
			if session("usuario_codigo_empresa")=4 _
				and session("usuario_tipo")= "GLS PROPIA" _
				and session("usuario")<>5089 and session("usuario")<> 7395 _
				and session("usuario")<> 11340 and session("usuario")<> 11341 _
				and session("usuario")<> 11537 then
			...
			if session("usuario")=9825 then
				...
				sql_aux=sql_aux & " todo group by todo.articulo) AS CANTIDAD_PENDIENTE, NULL"
...
<style type="text/css" media="print">
    /* Oculta todo excepto el contenido de cada artículo */
    /*body * { display: none; }*/
	/*body > *:not(.article) { display: none; }*/
	
	#columna_izquierda_fija { display: none; }
	.zona_filtros { display: none; }
	.btn_opciones { display: none; }
...
window.onbeforeprint = quitarOffset;
window.onafterprint = restaurarOffset;

function quitarOffset() {
	var div = document.getElementById("div_derecha");
	div.classList.remove("col-xs-9");
	div.classList.remove("col-xs-offset-3");
	div.classList.add("col-xs-12");
	div.classList.add("col-sm-12");
	div.classList.add("col-md-12");
	div.classList.add("col-lg-12");
	div.classList.add("col-xl-12");
	document.getElementById("article").classList.remove("col-md-6");
	document.getElementById("article").classList.add("col-xs-6");
	document.getElementById("article").classList.add("col-sm-6");
	document.getElementById("article").classList.add("col-md-6");
	document.getElementById("article").classList.add("col-lg-6");
	document.getElementById("article").classList.add("col-xl-6");
	
}

function restaurarOffset() {
	var div = document.getElementById("div_derecha");
	div.classList.remove("col-xs-12");
	div.classList.remove("col-sm-12");
	div.classList.remove("col-md-12");
	div.classList.remove("col-lg-12");
	div.classList.remove("col-xl-12");
	
	div.classList.add("col-xs-9");
	div.classList.add("col-xs-offset-3");
	document.getElementById("article").classList.remove("col-xs-6");
	document.getElementById("article").classList.remove("col-sm-6");
	document.getElementById("article").classList.remove("col-md-6");
	document.getElementById("article").classList.remove("col-lg-6");
	document.getElementById("article").classList.remove("col-xl-6");
	document.getElementById("article").classList.remove("col-xs-6");
...
function seleccionar_fila_tallaje(agrupacion_tallaje, fila_pulsada, articulo, primera_imagen)
{
	//deseleccionamos todas las filas
	if (!document.getElementsByClassName)
		{
	...
	//cambiamos la imagen
	var imagen_art = document.getElementById('img_' + primera_imagen)
	var enlace_art = document.getElementById('enlace_img_' + primera_imagen)
	imagen_original = imagen_art.src
	if (imagen_original.indexOf('i_no_imagen.jpg') !== -1)
		{
		imagen_a_enlazar = '../Imagenes_Articulos/no_imagen.jpg'
		}
	  else
	  	{
		imagen_a_enlazar = '../Imagenes_Articulos/' + articulo + '.jpg'
		}
	
	
	imagen_art.src = '../Imagenes_Articulos/Miniaturas/i_' + articulo + '.jpg'
	enlace_art.href = imagen_a_enlazar
...
<div class="container-fluid">
  ...
  <div class="row____" id="probando">
    ...
    <div class="col-xs-9 col-xs-offset-3" id="div_derecha">
		<%if session("usuario_codigo_empresa")=4 then%>
			<!-- BOTONES PARA CONSULTAR PEDIDOS, DEVOLUCIONES Y SALDOS-->
			<div class="panel panel-default btn_opciones">
		        <div class="panel-body">
					<div class="row">
      ...
      <div class="panel panel-default">
		...
		<%end if%>
		<div class="panel-body">
			<div class="well well-sm zona_filtros">
				<form class="form-horizontal" role="form" name="frmbusqueda" id="frmbusqueda" method="post" action="Lista_Articulos_Gag.asp?acciones=<%=accion%>">
					...
					<div class="form-group">    
						...
						<div class="col-md-4">
							...
							<%' PARA HALCON, ECUADOR, GROUNDFORCE, AIR EUROPA, CALDERON, HALCON VIAJGENS, TRAVEPLAN, TIBILLETE, GLOBALIA, GEOMOON, GLOBALIA CORPORATE TRAVEL
								' MARSOL, AVORIS, FRANQUICIAS HALCON, FRANQUICIAS ECUADOR, GENERAL CARRITO, HOSPES
							...
							if session("usuario_codigo_empresa")=10 or session("usuario_codigo_empresa")=20 or session("usuario_codigo_empresa")=30 _
								...
								or session("usuario_codigo_empresa")=260 or session("usuario_codigo_empresa")=280 then %>
										...
										<%for i=0 to UBound(tabla_familias,2)
											...
											if session("usuario_trato_especial")=1 and session("usuario_derecho_primer_pedido")="SI" then
											  ...
											  <%else 'de session("usuario_trato_especial")=1 and session("usuario_derecho_primer_pedido")="SI"
													...
													'if session("usuario_codigo_empresa")=4 and session("usuario_tipo")= "GLS PROPIA" and session("usuario")<>5089 and session("usuario")<>7395 and session("usuario")<>11340 and session("usuario")<>11341 and UCASE(tabla_familias(campo_descripcion_familia,i))="GLS VESTUARIO NUEVO LOGO"  then
													if session("usuario_codigo_empresa")=4 and session("usuario_tipo")= "GLS PROPIA" _
														and session("usuario")<>5089 and session("usuario")<>7395 _
														and session("usuario")<>11340 and session("usuario")<>11341 _
														and session("usuario")<>11537 _
														and session("usuario_tipoprecio")<>"PROPIA_PORTUGAL" _
														and UCASE(tabla_familias(campo_descripcion_familia,i))="GLS VESTUARIO NUEVO LOGO" then
					...
					<div class="form-group">
						<%
							...
							' 10 HALCON, 20 ECUADOR, 80 HALCON VIAGENS, 90 TRAVELPLAN, 130 GEOMOON, 170 GLOBALIA CORPORATE TRAVEL, 180 EUROSTARS, 210 MARSOL, 230 AVORIS
							' 240 FRANQUICIAS HALCON, 250 FRANQUICIAS ECUADOR , 260 GENERAL CARRITO y 280 HOSPES tampoco
					...
					<%' PARA HALCON, ECUADOR, GROUNDFORCE, AIR EUROPA, CALDERON, HALCON VIAJGENS, TRAVEPLAN, TIBILLETE, GLOBALIA, GEOMOON, GLOBALIA CORPORATE TRAVEL
						' MARSOL, AVORIS, FRNQUICIAS HALCON, FRNAQUICIAS ECUADOR , GENERAL CARRITO Y HOSPES
													...
													if session("usuario_tipo")<>"ARRASTRES" THEN
														...
														if session("usuario_trato_especial")=1 and session("usuario_derecho_primer_pedido")="SI" then
														  ...
														  <%else 'de session("usuario_trato_especial")=1 and session("usuario_derecho_primer_pedido")="SI"
																...
																'y P280-5 COMPRAS PORTUGAL (11340) y P280-5 PERDIDAS PORTUGAL (11341)
																'la nueva propia de IRLANDA - I280-5 COMPRAS IRLANDA (11537)
																if session("usuario_codigo_empresa")=4 and session("usuario_tipo")= "GLS PROPIA" _
																	and session("usuario")<>5089 and session("usuario")<>7395 _
																	and session("usuario")<>11340 and session("usuario")<>11341 _
																	and session("usuario")<>11537 _
																	and session("usuario_tipoprecio")<>"PROPIA_PORTUGAL" _
																	and UCASE(nombrecito_familia)="GLS_VESTUARIO_NUEVO_LOGO"  then
																	else
					...
					<div class="col-md-6 article">
							<div class="panel panel-primary item col_articulo_1 item_<%=articulos("ID")%>">
								<div class="panel-heading" style="padding-bottom:2px;padding-top:2px">
									<div class="panel-title titulo_art"><H5><%=REPLACE(REPLACE(articulos("DESCRIPCION_IDIOMA"),".",""), "·","")%></H5></div>
								</div>
								<div class="panel-body" style="padding-left:1px; padding-left:1px; padding-top:0px;">
										...
										<div class="col-md-7">
											...
											<div class="panel panel-default__ inf_general_art"  onclick="muestra_datos_articulo(<%=articulos("ID")%>, <%=session("usuario_codigo_empresa")%>)"
												...
												<div class="panel-body" style="cursor:pointer;cursor:hand">
													...
													' el de IMPRENTA TAMPOCO - 220
													' 10 HALCON, 20 ECUADOR, 80 HALCON VIAGENS, 90 TRAVELPLAN, 130 GEOMOON, 170 GLOBALIA CORPORATE TRAVEL, 180 EUROSTARS, 210 MARSOL, 230 AVORIS
													' 240 FRANQUICIAS HALCON, 250 FRANQUICIAS ECUADOR y 260 GENERAL CARRITO Y 280 HOSPES tampoco
													if session("usuario_codigo_empresa")<>4 AND session("usuario_codigo_empresa")<>150 _
														and session("usuario_codigo_empresa")<>10 and session("usuario_codigo_empresa")<>20 and session("usuario_codigo_empresa")<>80 _
														and session("usuario_codigo_empresa")<>90 and session("usuario_codigo_empresa")<>130 and session("usuario_codigo_empresa")<>170 _
														and session("usuario_codigo_empresa")<>180 and session("usuario_codigo_empresa")<>210 and session("usuario_codigo_empresa")<>230 _
									...
									<div class="col-md-12">
										...
										<div class="col-md-6 panel_sinmargen_lados" align="center">
											<div class="thumb-holder" >
												...
												primera_imagen=articulos("id")
												%>
												<a href="<%=imagen_a_enlazar%>" target="_blank" id="enlace_img_<%=articulos("id")%>">
													<img class="img-responsive imagen_art" src="<%=icono_a_mostrar%>" border="0" id="img_<%=articulos("id")%>"/>
												</a>
											</div>
										</div>
										...
										<div class="col-md-6 panel_sinmargen_lados">
											...
											'PARA LAS AGENCIAS PROPIAS DE GLS MENOS LA 280-51, 280-5, 280-01, 280-02, 280-03, 280-04, 81-01, P280-5, P280-51, I280-5 y LAS DE PORTUGAL, NO PUEDEN PEDIR DIRECTAMENTE
											'  - VESTUARIO LABORAL - 186
											'  - VESTUARIO NEGOCIOS - 187
											'  - ROPA NUEVA LINEA - 244
											...
											if (articulos("familia")=186 or articulos("familia")=187 or articulos("familia")=227 or articulos("familia")=228 or articulos("familia")=244) _
												and (session("usuario")<>7395 and session("usuario")<>5089 and session("usuario")<>5085 and session("usuario")<>8351 and session("usuario")<>8352 _
													and session("usuario")<>8353 and session("usuario")<>7633 and session("usuario")<>11340 and session("usuario")<>11341 _
													and session("usuario")<>11537) _
												and session("usuario_tipo")="GLS PROPIA" and session("usuario_codigo_empresa")=4 then
												...
												' y P280-5 COMPRAS PORTUGAL (11340) y P280-51 PERDIDAS PORTUGAL (11341)
												'la nueva propia de IRLANDA - I280-5 COMPRAS IRLANDA (11537)
											  	if session("usuario")<>5089 and session("usuario")<>7395 _
													and session("usuario")<>11340 and session("usuario")<>11341 _
													and session("usuario")<>11537 _
													and articulos("id") >= 3952 and articulos("id") <= 3987 then
													if session("usuario_tipo")= "GLS PROPIA" then
														mostrar_boton="NO_VESTUARIO"
													end if
												end if
									...
									<%'si hay un pdf con manual de instrucciones del articulo, se pone el boton de acceso
									'pero si no hay tallaje, si hay tallaje se pone un icono con enlace al pdf para cada articulo
									if ("" & articulos("DESCRIPCION_BOTON_PDF")) <> "" then
										if ("" & articulos("descripcion_grupo")) = "" then%>
											<div class="text-right">
											  <a href="../Documentacion/PDF_DATOS_TECNICOS/<%=articulos("id")%>.pdf" target="_blank" class="center-block">
												<button type="button" name="cmdpdf_instalacion_<%=articulos("id")%>" id="cmdpdf_instalacion_<%=articulos("id")%>" class="btn btn-primary btn-sm">
												  <i class="fas fa-file-pdf"></i>
												  <span><%=articulos("DESCRIPCION_BOTON_PDF")%></span>
												</button>
											  </a>
											</div>
										<%end if
									...
									' y si la oficina es propia y no es la 280-5, 280-01, 280-02, 280-03, 280-04, 81-01, P280-5, P280-51
															...
															IF articulos("stock")<=0 and articulos("stock_minimo")>=1 then
																	if session("usuario_trato_especial")=1 and session("usuario_derecho_primer_pedido")="SI" then%>
																		<tr id="fila_tallaje_<%=articulos("ID_grupo")%>_<%=filas_tallaje%>" style="cursor:hand;cursor:pointer" class="filas_tallajes"
																			onclick="seleccionar_fila_tallaje(<%=articulos("id_grupo")%>,<%=filas_tallaje%>,<%=articulos("id")%>, <%=primera_imagen%>)">
																				<td align="left" class="align-left">
																					<%=articulos("descripcion_talla")%>
																					<%'si hay un pdf con manual de instrucciones del articulo, se pone el boton de acceso
																						if ("" & articulos("DESCRIPCION_BOTON_PDF")) <> "" then%>
																							<a href="../Documentacion/PDF_DATOS_TECNICOS/<%=articulos("id")%>.pdf" target="_blank" class="center-block"
																								data-toggle="popover" 
																								data-placement="left" 
																								data-trigger="hover" 
																								data-content="Manual de Instalación" 
																								data-original-title=""><i class="far fa-file-pdf" style="color: #0000ff;"></i>
																							</a>
																					<%end if%>
																				</td>
																	...
																	<%else%>
																		<tr id="fila_tallaje_<%=articulos("ID_grupo")%>_<%=filas_tallaje%>" style="cursor:hand;cursor:pointer" class="filas_tallajes"
																			...
																			data-original-title="">
																				<td align="left" style="color:#CCCCCC" class="align-left"><%=articulos("descripcion_talla")%>
																					<%'si hay un pdf con manual de instrucciones del articulo, se pone el boton de acceso
																						if ("" & articulos("DESCRIPCION_BOTON_PDF")) <> "" then%>
																							<a href="../Documentacion/PDF_DATOS_TECNICOS/<%=articulos("id")%>.pdf" target="_blank" class="center-block"
																								data-toggle="popover" 
								...
								<div class="panel-body" style="padding-left:1px; padding-left:1px; padding-top:0px;">
										...
										<div class="col-md-7">
											...
											<div class="panel panel-default__ inf_general_art"  onclick="muestra_datos_articulo(<%=articulos("ID")%>, <%=session("usuario_codigo_empresa")%>)" 
												...
												<div class="panel-body" style="cursor:pointer;cursor:hand">
													...
													' el de IMPRENTA TAMPOCO - 220
													' 10 HALCON, 20 ECUADOR, 80 HALCON VIAGENS, 90 TRAVELPLAN, 130 GEOMOON, 170 GLOBALIA CORPORATE TRAVEL, 180 EUROSTARS 210 MARSOL, 230 AVORIS
													' 240 FRANQUICIAS HALCON, 250 FRANQUICIAS ECUADOR, 260 GENERAL, 280 HOSPES tampoco
													if session("usuario_codigo_empresa")<>4 AND session("usuario_codigo_empresa")<>150 _
														and session("usuario_codigo_empresa")<>10 and session("usuario_codigo_empresa")<>20 and session("usuario_codigo_empresa")<>80 _
														and session("usuario_codigo_empresa")<>90 and session("usuario_codigo_empresa")<>130 and session("usuario_codigo_empresa")<>170 _
														and session("usuario_codigo_empresa")<>180 and session("usuario_codigo_empresa")<>210 and session("usuario_codigo_empresa")<>230 _
									...
									<div class="col-md-12">
										...
										<div class="col-md-6 panel_sinmargen_lados">
											...
											'PARA LAS AGENCIAS PROPIAS DE GLS MENOS LA 280-51, 280-5, 280-01, 280-02, 280-03, 280-04, 81-01, P280-5, P280-51, I280-5 NO PUEDEN PEDIR DIRECTAMENTE
											'  - VESTUARIO LABORAL - 186
											'  - VESTUARIO NEGOCIOS - 187
											'  - ROPA NUEVA LINEA - 244
											'response.write("<br>familia: " & articulos("familia") &"<br>cliente: " & session("usuario") & "<br>tipo usuario: " & session("usuario_tipo") & "<br>empresa: " & session("usuario_codigo_empresa"))
											if (articulos("familia")=186 or articulos("familia")=187 or articulos("familia")=227 or articulos("familia")=228 or articulos("familia")=244) _
												and (session("usuario")<>7395 and session("usuario")<>5089 and session("usuario")<>5085 and session("usuario")<>8351 _
													and session("usuario")<>8352 and session("usuario")<>8353 and session("usuario")<>7633 and session("usuario")<>11340 _
													and session("usuario")<>11341 and session("usuario")<>11537) _
												and session("usuario_tipo")="GLS PROPIA"  and session("usuario_codigo_empresa")=4 then
												mostrar_boton="NO_VESTUARIO"
											  else
												'las nuevas camisas de vestuario de negocio.. ids de 3952 a 3987 solo las pueden pedir 280-5 Compras (5089) y 280-51 Perdidas (7395)
									...
									<%'si hay un pdf con manual de instrucciones del articulo, se pone el boton de acceso
									'pero si es tallaje no, se pondrá ese acceso al pdf para cada articulo
									if ("" & articulos("DESCRIPCION_BOTON_PDF")) <> "" then
										if ("" & articulos("descripcion_grupo")) = "" then%>
											<div class="text-right">
											  <a href="../Documentacion/PDF_DATOS_TECNICOS/<%=articulos("id")%>.pdf" target="_blank" class="center-block">
															  ...
															  IF articulos("stock")<=0 and articulos("stock_minimo")>=1 then
																	...
																	<%else%>
																		<tr id="fila_tallaje_<%=articulos("ID_grupo")%>_<%=filas_tallaje%>" style="cursor:hand;cursor:pointer" class="filas_tallajes"
																			...
																			data-content="<%=articulos("descripcion_talla")%> No Disponible Temporalmente" 
																			data-original-title="">
																			<td align="left" style="color:#CCCCCC" class="align-left">
																				<%=articulos("descripcion_talla")%>
																				<%'si hay un pdf con manual de instrucciones del articulo, se pone el boton de acceso
																					if ("" & articulos("DESCRIPCION_BOTON_PDF")) <> "" then%>
																						<a href="../Documentacion/PDF_DATOS_TECNICOS/<%=articulos("id")%>.pdf" target="_blank" class="center-block"
...
$(document).ready(function() {
	...
	<%' PARA HALCON, ECUADOR, GROUNDFORCE, AIR EUROPA, CALDERON, HALCON VIAJGENS, TRAVEPLAN, TIBILLETE, GLOBALIA, GEOMOON, GLOBALIA CORPORATE TRAVEL, MARSOL Y AVORIS, GENERAL y HOSPES
		'mostramos un combo diferente con familias y agrupaciones de familias
	if session("usuario_codigo_empresa")=10 or session("usuario_codigo_empresa")=20 or session("usuario_codigo_empresa")=30 or session("usuario_codigo_empresa")=40 or session("usuario_codigo_empresa")=50 or session("usuario_codigo_empresa")=80 or session("usuario_codigo_empresa")=90 or session("usuario_codigo_empresa")=100 or session("usuario_codigo_empresa")=110 or session("usuario_codigo_empresa")=130 or session("usuario_codigo_empresa")=170 or session("usuario_codigo_empresa")=210 or session("usuario_codigo_empresa")=230 or session("usuario_codigo_empresa")=240 or session("usuario_codigo_empresa")=250 or session("usuario_codigo_empresa")=260 or session("usuario_codigo_empresa")=280 then %>
