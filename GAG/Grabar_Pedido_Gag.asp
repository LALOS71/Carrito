...
sub mail_autorizacion_pedido(pedido)
	...
	if not datos_mail.eof then
		...
		while not datos_pedido.eof
			...
			'response.write("<br>1 - ruta articulo JSON: " & ruta_json)
		...
		para = correos_recibe 		  	
		asunto = cadena_asunto & "Pedido Num. " & datos_mail("id") & ", Pendiente de Autorizar"
...
<%
	...
	if session("usuario")="" then
		...
		'para ver si es un cliente a facturar existente, o a otro
		facturacion = up.Form("txtfacturacion_d")
		...
		renting_impresoras_gls = "" & up.Form("optrenting_imp_gls")
		...
		'{"name":"oculto_pais_oficina","value":"ESPA�A"},{"name":"txtnumero_empleado_d","value":"19316"},{"name":"txthorario_entrega_d","value":"asdf"},
	...
	'pruebo a no a�adir comprobacion para las familias de GLS ROTULACION porque 
	...
	if session("usuario_codigo_empresa")=10 or session("usuario_codigo_empresa")=20 or session("usuario_codigo_empresa")=80 _
		or session("usuario_codigo_empresa")=90 or session("usuario_codigo_empresa")=130 or session("usuario_codigo_empresa")=170 _
		or session("usuario_codigo_empresa")=180 or session("usuario_codigo_empresa")=210 or session("usuario_codigo_empresa")=230 _
		or session("usuario_codigo_empresa")=240 or session("usuario_codigo_empresa")=250 then		
	...
	if (session("usuario_codigo_empresa")=20 or session("usuario_codigo_empresa")=250) and session("usuario_tipo")="FRANQUICIA" and diferencia_dias>0 then
			...
			cadena_sql = cadena_sql & " WHERE ARTICULOS_EMPRESAS.CODIGO_EMPRESA=" & session("usuario_codigo_empresa")
			cadena_sql = cadena_sql & " AND ARTICULOS.ID IN (" & conjunto_articulos & ")"
			cadena_sql = cadena_sql & " AND UPPER(FAMILIAS.DESCRIPCION) IN ('PAPELER�A PROPIA')"
			...
			cadena_sql = cadena_sql & " WHERE ARTICULOS_EMPRESAS.CODIGO_EMPRESA=" & session("usuario_codigo_empresa")
			cadena_sql = cadena_sql & " AND ARTICULOS.ID IN (" & conjunto_articulos & ")"
			cadena_sql = cadena_sql & " AND UPPER(FAMILIAS.DESCRIPCION) NOT IN ('PAPELER�A PROPIA')"
	...
	if error_mezcla<>"SI" and error_mezcla_merchan_personalizable<>"SI" and error_mezcla_merchan_no_personalizable<>"SI" and error_mezcla_maletas<>"SI" and error_mezcla_higienicos<>"SI" and error_mezcla_papeleria_propia<>"SI" and error_mezcla_gls_navidad<>"SI" then
		...
		if accion="MODIFICAR" then 'aqui modificamos pedidos
				...
				''response.write("<br>MODIFICAMOS EL PEDIDO")
				...
				with detalles_pedido
					...
					'para GENERAL CARRITO, controlo que el pedido est� en PENDIENTE PAGO, solo asi se puede modificar
					if session("usuario_codigo_empresa")=260 then
					...
					if session("usuario_codigo_empresa")=4 	then
						...
						.Source="SELECT * FROM PEDIDOS WHERE ID=" & pedido_modificar & " AND ESTADO<>'" & estado_consulta & "' AND ESTADO <> 'PENDIENTE FIRMA'"
						...
						' a�adamos articulos de gls y lo tenga que pasar a reservado
						if hay_de_gls="SI" then
							.Source="SELECT * FROM PEDIDOS WHERE ID=" & pedido_modificar & " AND ESTADO<>'PENDIENTE PAGO'"
						END IF
					end if
					''response.write("<br>CORPROBAMOS SI PUEDE GUARDARSE EL PEDIDO: " & .Source)
					.Open
				end with
				...
				''response.write("<BR>PODEMOS MODIFICARLOS: " & podemos_modificarlo)
				
				if podemos_modificarlo="SI" then
					'modifico los articulos del pedido
					' para ello, borro los articulos y a�ado lo que tenga en el carrito
					...
					'response.write("<br>2 - BORRAMOS LOS FICHEROS JSON Y SE VUELVEN A GENERAR: " & FICHERO_BORRAR) 
					...
					for i=1 to Session("numero_articulos")
						...
						If up.Ficheros.Exists("txtfichero_" & id) Then
							...
							if session(i & "_fichero_asociado")<>"" then
								'response.write("<BR>3 - VEMOS SI EXISTE EL FICHERO")
								'response.write("<BR>" & Server.MapPath("./pedidos/" & year(fecha_pedido) & "/" & session("usuario") & "__" & pedido_modificar & "/" & session(i & "_fichero_asociado")))
								if fso.FileExists(Server.MapPath("./pedidos/" & year(fecha_pedido) & "/" & session("usuario") & "__" & pedido_modificar & "/" & session(i & "_fichero_asociado"))) then
									'response.write("<BR>4 - existe y lo borro")
						...
						IF up.Form("ocultoautorizacion_" & id)="SI" THEN
							...
							if session("usuario_codigo_empresa")<>10 and session("usuario_codigo_empresa")<>20 and session("usuario_codigo_empresa")<>80_
								...
								and session("usuario_codigo_empresa")<>180 and session("usuario_codigo_empresa")<>210 and session("usuario_codigo_empresa")<>230_
						...
						'para ASM
						if session("usuario_codigo_empresa")=4 then
							cadena_valores=pedido_modificar & ", " & id & ", " & cantidad & ", " & REPLACE(precio,",",".") & ", " & REPLACE(total,",",".") 
							'si el articulo es una impresora de GLS, como no tiene precio va a sin tratar
							if id=4583 then
								cadena_valores=cadena_valores & ", 'SIN TRATAR'"
							  else
							  	cadena_valores=cadena_valores & ", '" & estado_consulta & "'"
							end if
							cadena_valores=cadena_valores & ", '" & fichero_asociado & "'"
						end if
					...
					if datos_adicionales_maletas<>"" then
						...
						if ((valor_oculto_id="") or (valor_oculto_id<>"" and valor_oculto_empresa<>"200")) THEN
							...
							''response.write("<br>se ha a�adido un nuevo cliente: " & numero_cliente)
						...
						'controlamos si para el rentin de impresoras gls es de 8 o 10 �
						if renting_impresoras_gls <> "" then
							cadena_modificacion_pedido=cadena_modificacion_pedido & ", RENTING_IMPRESORA_GLS=" & renting_impresoras_gls
						  else
							cadena_modificacion_pedido=cadena_modificacion_pedido & ", RENTING_IMPRESORA_GLS=NULL"
						end if
					...
					if session("usuario_codigo_empresa")<>4 then
						...
						if session("usuario_codigo_empresa")<>150 then
					  	...
					  	if session("usuario_pais")="ESPA�A" and session("usuario_tipo")="GLS PROPIA" then
							if estado_consulta_general="PENDIENTE AUTORIZACION" then
					...
					mensaje_aviso="NO SE HA PODIDO MODIFICAR El Pedido Porque Ya Est� Siendo Tramitado por Globalia Artes Gr�ficas..."
				end if ' de podemos_modificarlo = SI
				...
				cadena_campos="CODCLI, FECHA, ESTADO, USUARIO_DIRECTORIO_ACTIVO, PEDIDO_AUTOMATICO, RENTING_IMPRESORA_GLS, DESCUENTO_TOTAL"
				...
				cadena_campos=cadena_campos & ", GASTOS_ENVIO, DONDE_ENVIO, CODCLI2"
				...
				cadena_valores=cliente_a_guardar & ", GETDATE(), '" & estado_consulta & "', "
									...
									if hay_de_merchan_no_personalizable="SI" then 'si es un pedido de merchandising personalizable de halcon/ecuador
										...
										if hay_de_higienicos="SI" then 'si es un pedido de higiene y seguridad
											...
											if hay_de_gls_navidad="SI" then 'si son articulos de navidad, el pedidos queda marcado como de NAVIDAD
												cadena_valores=cadena_valores & ", 'NAVIDAD 2023'"
											  else
				...
				'si es un pedido de impresoras, controlamos si es con renting de 8 euros o de 10
				if renting_impresoras_gls <> "" then
					cadena_valores=cadena_valores & ", " & renting_impresoras_gls
				  else
				  	cadena_valores=cadena_valores & ", NULL"
				end if
				...
				for i=1 to Session("numero_articulos")
					...
					cadena_campos="ID_PEDIDO, ARTICULO, CANTIDAD, PRECIO_UNIDAD, TOTAL, ESTADO, FICHERO_PERSONALIZACION, PRECIO_COSTE"
					cadena_valores=numero_pedido & ", " & id & ", " & cantidad & ", " & REPLACE(precio,",",".") & ", " & REPLACE(total,",",".")
					
					'si el articulo es una impresora de GLS, como no tiene precio va a sin tratar
					if session("usuario_codigo_empresa")=4 and id=4583 then
						cadena_valores=cadena_valores & ", 'SIN TRATAR', '" & fichero_asociado & "'"
					  else
					  	cadena_valores=cadena_valores & ", '" & estado_consulta_detalle & "', '" & fichero_asociado & "'"
					end if
			...
			'if session("usuario_derecho_primer_pedido")="SI" then
			'	mensaje_aviso= mensaje_aviso & "<br><br><h3 align=center><font color=#880000>Como es un Pedido con Descuento, primero ser� validado por su central</font></h3>"
			'end if
...
$('#pantalla_avisos').on('hide.bs.modal', function(e){
  ...
  if ('<%=forma_de_pago%>'=='PICARD')
	 ...
	 bootbox.alert({
				size: 'large',
				message: 'Podr� Realizar el pago con tarjeta de este Pedido accediendo de nuevo a �l desde la secci�n "Consultar Pedidos".',
	...
	if ('<%=forma_de_pago%>'=='REDSYS')
  	{
		...
		if ($("#ocultoaviso").val()=="NO")
			{
		     alert('Podr� Realizar el pago con tarjeta de este Pedido accediendo de nuevo a �l desde la secci�n "Consultar Pedidos".')
			}
