...
entorno=C_ENTORNO_EJECUCION
iis_des=C_SERVIDOR_WEB_DESARROLLO
iis_prod=C_SERVIDOR_WEB_PRODUCCION
...
function verpopup(esAdmin, empresa){

	$("#pantalla_popup_villalar").modal("show");
}


function verpopup_2(esAdmin, empresa, usuario, servidor){
	...
	if ((empresa==10)||(empresa==20)||(empresa==240)||(empresa==250))	
		...
		if (empresa==40)
			...
			$("#pantalla_popup_air_europa_unidades_pedido").modal("show");
		  	...
		  	{
			abrirArticulos(esAdmin, servidor)
			}
		}
}
...
function abrirArticulos(esAdmin, servidor) {
    //alert('Administradora:' + esAdmin);
	
    var dir = 'https://' + servidor + '/GAG/';
...
<BODY onload="verpopup_2('<%=administrador_central%>', <%=session("usuario_codigo_empresa")%>, '<%=hotel_seleccionado%>', '<%=iis_server%>')">
...
$('#pantalla_popup_aviso_navidad').on('hidden.bs.modal', function (e) {
  verpopup_2('<%=administrador_central%>', <%=session("usuario_codigo_empresa")%>, '<%=hotel_seleccionado%>', '<%=iis_server%>')
})
...
$('#pantalla_popup_globalbag').on('hidden.bs.modal', function (e) {
	abrirArticulos('<%=administrador_central%>', '<%=iis_server%>')	//para que se dirija a la pagina despues de cerrar el popup
})

$('#pantalla_popup_hal_vec_merchan').on('hidden.bs.modal', function (e) {
  abrirArticulos('<%=administrador_central%>', '<%=iis_server%>')
})
