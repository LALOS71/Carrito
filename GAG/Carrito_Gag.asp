...
<%
		...
		'response.write("<br>facturacion: " & faturacion)
...
function validar(pedido_minimo, impresora_gls)
{
	...
	if (<%=Session("numero_articulos")%>>0)
		...
		if ($("input:radio[name=optfacturacion_nueva]:checked").val()=='NUEVA') {
			if ($('#txtfacturacion_d').val()=='') {
				hay_error='SI'
				cadena_error+='<br>- Nuevo Destinatario de Facturaci&oacute;n no puede estar vac&iacute;o.'
			}
		}
  ...
  <div class="row">
    ...
    <div class="col-xl-9 col-lg-9 col-md-9 col-sm-9 col-xs-9">
	 ...
	 if (session("usuario_codigo_empresa")=4 and session("usuario_tipo")="GLS PROPIA") _
		...
		<!-- ================================================================================================== -->
		<div class="panel panel-default">
			<div class="panel-heading">Facturacci&oacute;n del Pedido</div>	
		   <div class="panel-body">
			   <div class="col-md-12">
				   ...
				   <div class="col-md-12 col-lg-12">
					   <h4>
						   ...
						   <input type="radio" name="optfacturacion_nueva" id="optfacturacion_nueva" value="NUEVA" style="vertical-align:middle ">&nbsp;Nuevo Destinatario de Facturaci&oacute;n </h4>
				   </div>
			   </div>
				...
				<!--  <div class="form-group row">
				   ...
				   if session("usuario")=5089 or session("usuario")=7395 _
					   ...
					   <div class="col-sm-2 col-md-2 col-lg-2"> 
						   <label for="cmbagencia_propia_facturacion_d" class="control-label">Agencia/Propia</label>
						   <select class="form-control" name="cmbagencia_propia_facturacion_d" id="cmbagencia_propia_facturacion_d" readonly>
				   ...
				   <%end if%> -->
					   <label for="txtfacturacion_d" class="control-label">Facturar a:</label>
					   <div class="typeahead__container">
						   <div class="typeahead__field">
							   <div class="typeahead__query">
								   <input type="text" placeholder="" autocomplete="off" class="js-typeahead-destinatario form-control" style="width: 100%;"  id="txtfacturacion_d" name="txtfacturacion_d" value="<%=destinatario%>" readonly/>
							   </div>
						   </div>
					   </div>
...
//facturacion nuevatxtfacturacion_d
$("input:radio[name=optfacturacion_nueva]").on("click", function () {
	//alert($("input:radio[name=optforma_pago]:checked").val())
	//console.log('contenido del combo: ' + $('#txtfacturacion_d').source())
	if ($("input:radio[name=optfacturacion_nueva]:checked").val()=='ACTUAL')
		{
		if( $('#cmbagencia_propia_txtfacturacion_d').length )
			{
			//console.log('hay combo propia-franquicia')
			$('#cmbagencia_propia_txtfacturacion_d').attr('readonly', '')
			}
		$('#txtfacturacion_d').prop('readonly', true)
		$('#txtfacturacion_d').val('<%=destinatario%>')
		}	
	else
		{
		  if( $('#cmbagencia_propia_txtfacturacion_d').length ) {			
		      $('#cmbagencia_propia_txtfacturacion_d').removeAttr('readonly')
			}

		  $('#txtfacturacion_d').prop('readonly', false)					
			$('#txtfacturacion_d').val('')
			$('#txtfacturacion_d').focus()
		}
});
...
$("input:radio[name=optdireccion_envio]").on("click", function () {
	...
	if ($("input:radio[name=optdireccion_envio]:checked").val()=='ACTUAL')
		...
		$('#txtfacturacion_d').prop('readonly', false) //controlar el campo txtfacturacion
...
$('#txfacturacion').keydown(function(event){
    //si no controlo el enter hace el submit del formulario
	if(event.keyCode == 13) {
      event.preventDefault();
      return false;
