	...
	'end if
		...
		if accion="MODIFICAR" then 'aqui modificamos pedidos
				...
				with detalles_pedido
					...
					.Source="SELECT * FROM PEDIDOS_DETALLES WHERE ID_PEDIDO=" & pedido_modificar
					'response.write("<br>source: " & .source)
					.Open
				end with
				
				if not detalles_pedido.eof then
					podemos_modificarlo="SI"
				end if
				detalles_pedido.close
				set detalles_pedido=Nothing
				
				'response.write("<br>podemos modificarlo: " & podemos_modificarlo)
				if podemos_modificarlo="SI" then
					...
					for i=1 to Session("numero_articulos")
						...
						if up.Form("ocultorestado_stock_" & id)<>"" then
							restado_stock=up.Form("ocultorestado_stock_" & id)
						  else
							restado_stock=""
						end if
						...
						If up.Ficheros.Exists("txtfichero_" & id) Then
							...
							if session(i & "_fichero_asociado")<>"" then
								...
								if nombre_empresa="ASM" then
									directorio="./ASM/pedidos/"
								end if
								
								if fso.FileExists(Server.MapPath(directorio & year(fecha_pedido) & "/" & hotel_admin & "__" & pedido_modificar & "/" & session(i & "_fichero_asociado"))) then
									fso.DeleteFile(Server.MapPath(directorio & year(fecha_pedido) & "/" & hotel_admin & "__" & pedido_modificar & "/" & session(i & "_fichero_asociado")))
								end if
							end if
						  else
						...
						cadena_campos="ID_PEDIDO, ARTICULO, CANTIDAD, PRECIO_UNIDAD, TOTAL, ESTADO, FICHERO_PERSONALIZACION, RESTADO_STOCK"
						cadena_valores=pedido_modificar & ", " & id & ", " & cantidad & ", " & REPLACE(precio,",",".") & ", " & REPLACE(total,",",".") & ", 'SIN TRATAR', '" & fichero_asociado & "', '" & restado_stock & "'"
					...
					next
					'cuando modifica la imprenta un pedido lo deja en el estado sin tratar
					connimprenta.Execute "UPDATE PEDIDOS SET ESTADO='SIN TRATAR' WHERE ID=" & pedido_modificar,,adCmdText + adExecuteNoRecords
					...
					if up.Ficheros.Count>0 then
						...
						'response.write("<br>Ruta pagina asp: " & ruta)
						lugar_encontrado=0
						...
						if nombre_empresa="ATESA" then
							carpeta=carpeta & "ATESA\"
						end if
						if nombre_empresa="ASM" then
							carpeta=carpeta & "ASM\"
						end if
						'response.write("<br>carpeta: " & carpeta)
						carpeta=carpeta & "pedidos"
						
						'response.write("<br>carpeta de los pedidos: " & carpeta)
					...
					'response.write("<br>carpeta donde se guardara: " & carpeta)
					For each fich in up.Ficheros.Items	
						'subo el fichero al servidor
						'response.write("<br>nombre del fichero: " & fich.Nombre)
						'response.write("<br>carpeta: " & carpeta)
						'fich.GuardarComo fich.Nombre, ruta
						'fich.GuardarComo fich.Nombre, Server.MapPath(directorio & year(fecha_pedido) & "/" & hotel_admin & "__" & pedido_modificar)
						fich.GuardarComo fich.Nombre, carpeta
				...
				For each fich in up.Ficheros.Items	
					...
					'fich.GuardarComo fich.Nombre, ruta
					'response.write("<br>guardamos el fichero (" & fich.name & ") en: " & Server.MapPath(directorio & year(date()) & "/" & hotel_Admin & "__" & numero_pedido))
					fich.GuardarComo fich.Nombre, Server.MapPath(directorio & year(date()) & "/" & hotel_Admin & "__" & numero_pedido)
					'guardo el nombre del documento en la base de datos
				Next
...
<BODY onload="validar('<%=mensaje_aviso%>')">
	...
	'conn.execute sql
%>
<form name="frmgrabar_pedido" id="frmgrabar_pedido" method="post" action="Consulta_Pedidos_GAGAD.asp">
</form>
</BODY>
   <%	
   		'regis.close			
