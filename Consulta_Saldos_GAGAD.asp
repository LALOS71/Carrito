						...
						<div class="form-group row mt-3 align-items-end">
								...
								<div class="typeahead__container">
									<div class="typeahead__field">
										<div class="typeahead__query">
											<input class="js-typeahead-cliente form-control" name="txtbuscar_cliente" id="txtbuscar_cliente" type="search" placeholder="Buscar Cliente (Por C�digo o Descripci�n)" autocomplete="off">
										</div>
									</div>
								</div>
							</div>
							...
							<div class="col-sm-4 col-md-2 col-lg-2">
								...
								<button type="button" class="btn btn-primary btn-block" id="cmdannadir_saldo" name="cmdannadir_saldo"
									...
									data-bs-placement="top"
									data-bs-trigger="hover"
									data-bs-content="A�adir un Nuevo Saldo"
									title=""
									>
...
j$("#cmdannadir_saldo").click(function () {
	...
	cadena_mensaje+='<input class="js-typeahead-cliente form-control" name="txtnuevo_saldo_buscar_cliente" id="txtnuevo_saldo_buscar_cliente" type="search" placeholder="Buscar Cliente (Por C�digo o Descripci�n)" autocomplete="off">'
	cadena_mensaje+='</div>'
	cadena_mensaje+='</div>'
	cadena_mensaje+='</div>'
	cadena_mensaje+='</div>'
			...
			if (result)
				...
				if (cadena_error!='')
					...
					bootbox.confirm({
						...
						closeButton: false,
						centerVertical: true,
						message: "<h5>�Est� seguro que desea guardar el saldo con la siguiente informaci�n?</h5>" + cadena_texto + "<br><br>",
						buttons: {
							cancel: {
...
comprobar_datos_saldo = function(){
	...
	if (j$("#txtnuevo_saldo_importe").val()=='')
		...
		if (!j$.isNumeric(j$("#txtnuevo_saldo_importe").val().replace(',', '.'))) 
			{
    		cadena_error=cadena_error + '- El Importe del Saldo Ha De Ser Num�rico.<br>'
			}	
		  else
		  	{
			if (parseFloat(j$("#txtnuevo_saldo_importe").val().replace(',', '.')) <= 0) 
				{
				cadena_error=cadena_error + '- El Importe del Saldo Ha De Ser Num�rico Positivo.<br>'
				}
			}	
		}
...
consultar_saldos = function() {  
      var err ="";
        ...
        if (typeof lst_saldos== "undefined") {
            lst_saldos = j$("#lista_saldos").DataTable({dom:'<"toolbar">Blfrtip',
                                                          ajax:{url:"tojson/Consulta_Saldos_GAGAD_Obtener_Saldos.asp?"+prm.toString(),
                                                           type:"POST",
                                                           dataSrc:"ROWSET"},
                                                     columnDefs: [
    ...
    //  Fin Scroller
			  ...
			  lst_saldos.ajax.url("tojson/Consulta_Saldos_GAGAD_Obtener_Saldos.asp?"+prm.toString());
