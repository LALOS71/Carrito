...
<body>
    <div class="card mx-auto" style="width: 70%;">
        ...
        <div class="card-body">
            <h5 class="card-title">Solicitar alta</h5>
...
<script language="javascript">
    enviarCorreo=function(datos){
        fetch('./PHP/correo/correo.php', {
                ...
                .catch(error => {
                    ...
                    bootbox.alert('Se produjo un error al procesar la solicitud.');
                });
    }
    insertar=function(datos){
        fetch('./PHP/Altas/Control_Solicitar_Alta.php', {
                    method: 'POST',
                    body: datos
                })
                .then(response => {
    ...
    botonEnviar.addEventListener('click', () => {
        ...
        let campoCaptcha;
        if (errores.length > 0) {
            bootbox.alert('Error: Debe rellenar todos los campos del formulario');
        } else {
            const formData = new FormData(formulario);
            fetch('./PHP/captcha/captcha.php', {
                    method: 'GET',
                }).then(response => {
                    if (!response.ok) {
                        throw new Error('La respuesta de la red no fue exitosa');
