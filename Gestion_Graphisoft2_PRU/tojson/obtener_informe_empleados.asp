...
<%
	...
	If fecha_inicio_seleccionada <> "" Then
		...
		sql = sql & " AND CAST(D.FECHA AS SMALLDATETIME) >= CAST('" & cdate(fecha_inicio_seleccionada) & "' AS SMALLDATETIME)"
		
	End If
	
	If fecha_fin_seleccionada <> "" Then
		'sql = sql & " AND CONVERT(VARCHAR, D.FECHA, 103)<=CONVERT(VARCHAR, '" & cdate(fecha_fin_seleccionada) & "', 103)"
		sql = sql & " AND CAST(D.FECHA AS SMALLDATETIME) <= CAST(DATEADD(dd, 1, '" & cdate(fecha_fin_seleccionada) & "') AS SMALLDATETIME)"
	End If
