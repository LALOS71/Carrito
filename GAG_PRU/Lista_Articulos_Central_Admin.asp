...
<%
		...
		with tipos_precios
			...
			.Source="SELECT TIPO_PRECIO FROM V_EMPRESAS_TIPOS_PRECIOS WHERE ID_EMPRESA=" & session("usuario_codigo_empresa") & "  ORDER BY TIPO_PRECIO"
			.Open
		...
		sql="Select articulos.*  from articulos INNER JOIN ARTICULOS_EMPRESAS ON ARTICULOS.ID = ARTICULOS_EMPRESAS.ID_ARTICULO "
		...
		'Como puede haber un artículo asociado a dos empresas, con la siguiente condición evitamos que aparezca si no tiene
		'  definido ningún precio para la empresa en cuestión.
		sql=sql & " and (articulos.id in (select codigo_articulo from cantidades_precios where "
		sql=sql & " cantidades_precios.codigo_empresa=" & session("usuario_codigo_empresa") & ")) "
		...
		<div class="sidebarcell">
			...
			<div class="contentcell">
				<div class="sidefreetext" ><div align="left">
					· <a href="Lista_Articulos_Central_Admin.asp">Consultar Artículos <%=session("usuario_empresa")%></a>
					<br />
					· <a href="Consulta_Pedidos_Central_Admin.asp">Consultar Pedidos <%=session("usuario_empresa")%></a>
			...
			<table width="90%" cellspacing="6" cellpadding="0" class="logintable" align="center">
				...
				<tr>
					<td width="50%" class="dottedBorder vt al">
						...
						<form name="frmbusqueda" id="frmbusqueda" method="post" action="Lista_Articulos_Central_Admin.asp?acciones=<%=accion%>">
																											...
																											<td valign="middle" align="center">
																												...
																												%>
																													...
																													<%if session("usuario_codigo_empresa")=1 then 'BARCELÓ 
																														carpeta_marca=marcas_articulo("marca")&"/"
																													  else
																														carpeta_marca=""
																													  end if
																													%>
																													<a href="Imagenes_Articulos/<%=carpeta_marca%><%=articulos("id")%>.jpg" target="_blank">
																														<img class="product_thumbnail" src="Imagenes_Articulos/<%=carpeta_marca%>Miniaturas/i_<%=articulos("id")%>.jpg" border="0"></a>
																													<br />
																												<%	
