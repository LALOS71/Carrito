...
<%
		...
		dim fso_envio
		set fso_envio=Server.CreateObject("Scripting.FileSystemObject")
		...
		if not pedido_al_proveedor.eof then
			...
			while not detalles_pedido_proveedor.eof
				...
				if fso_envio.FileExists(server.mappath(imagen_pdf)) then
			...
			while not detalles_pedido_proveedor.eof
				...
				if plantilla_personalizacion<>"" then
					...
					Set fso_envio_json = CreateObject("Scripting.FileSystemObject")
					cadena_json = fso_envio_json.OpenTextFile(ruta_fichero_json).ReadAll
					set fso_envio_json = Nothing
		...
		''''''''''''''''''''''''''''
			...
			asunto = "Solicitud de Rotulacion GAG - Pedido " & pedido_seleccionado
		   
				
			adjunto= ""
			servidor = "AMAZON"
			
			'no se envia nada en el cuerpo del mensaje, solo un adjunto
			mensaje=""
		   ...
		   'RESPONSE.WRITE("<br>14 - veo si se manda el correo. si se hace el send de objCDO")
		   
		  	if envio_correo="SI" then
				'RESPONSE.WRITE("<br>15 - antes del send.")
				respuesta_envio = envio_email(de, para, asunto, mensaje, adjunto, servidor)
				'objCDO.Send
				'RESPONSE.WRITE("<br>16 - despues del send.")
