...
<body>
	<div class="card" id="cardPrecioBloque">
		<div class='card-header d-flex justify-content-between'>
			<h4 >ESTABLECER PRECIO EN BLOQUE</h4>
			<button class="btn btn-primary" data-bs-toggle="collapse" data-bs-target="#formularioPrecioBloque" aria-expanded="false" aria-controls="formularioPrecioBloque">Mostrar/Ocultar</button>
		</div>
		<form action='#' method='post' class='card-body collapse' id='formularioPrecioBloque'>
			<div class='input-group mb-3'>
				<label class='input-group-text' for='precioBloque'>Elige el precio:</label>
				<input class='form-control' type='number' name='precioBloque' id='precioBloque' step='any'>
				<input type='submit' class='btn btn-primary' value='Establecer precio'>
			  </div>			  
			<table id='tablaPrecioBloque' class='table table-striped table-hover mx-auto' style="width: 50%;">
				<thead>
					<tr class='table-dark'>
						<td>#</td>
						<td>Empresa</td>
...
<script language="javascript">
	function mensajeInformativo(mensaje,reload=false) {
	...
	async function obtenerDatos(url,formData) {
	...
	let compromisoCompra="<%=campo_compromiso_compra%>";
	console.log(compromisoCompra);

	if (compromisoCompra=="SI") {
		// let empresa = '<%=empresa_anterior%>';
		const formData = new FormData();
		let tabla = document.querySelector("#tablaPrecioBloque");
		let tbody = document.querySelector("#cuerpoTabla");
		
		valor_codigo_articulo='<%=articulo_seleccionado%>';
		// Añado al formData el valor del idArticulo para obtener los datos que se mostraran en la tabla
		formData.append("idArticulo",valor_codigo_articulo);
		let consultaEmpresas=(async () => {
			let consulta;
			  try {
				// Espero a que la consulta se complete
				consulta = await obtenerDatos('./PHP/establecerPrecioEnBloque/Consulta.php',formData);
				// Construyo la tabla
				consulta.forEach((tipoEmpresa,indice) => {
					let checkBox=`<input type='checkbox' name=checkbox value=${indice}>`;
					let cuerpoTabla=`<tr>
						<td>${checkBox}</td>
						<td>${tipoEmpresa.EMPRESA}</td>
						<td>${tipoEmpresa.TIPO_SUCURSAL}</td>
						</tr>`;
					let fila = document.createElement("tr");
					fila.innerHTML = cuerpoTabla;
		...
		let formularioPrecioBloque = document.querySelector("#formularioPrecioBloque");
				formularioPrecioBloque.addEventListener("submit", function(event){
					event.preventDefault();
					let formulario = event.target;
					let jsonAEnviar=[];
					// Creo un formData con los valores de los diferentes inputs del formulario
					let formDataInputs = new FormData(formulario);
					// Y este formData es el que se le enviara al archivo PHP para modificar los precios
					let formDataInsertar = new FormData();
					let valoresFormulario=[...formDataInputs.values()];
					consultaEmpresas.then(data=>{
						// Hago comprobaciones y si no se cumplen muesto una advertencia en un modal
						if (formDataInputs.get('precioBloque')=='') {
							mensajeInformativo("Debes de introducir un precio");
						}else if(formDataInputs.get('precioBloque')<0){
							mensajeInformativo("Debes de introducir un precio mayor o igual a 0");
						}else if(formDataInputs.getAll("checkbox").length==0){
							mensajeInformativo("Debes seleccionar al menos una empresa");
						}
						else{
							// Elimino de valoresFormulario el primer input que es el precio para quedarme solo con los valores de los checkbox
							if (valoresFormulario.length > 1) {
								valoresFormulario.shift();
							}
							// Añado al json que enviaremos a PHP las empresas que han de ser modificadas 
							valoresFormulario.forEach(posicion => {
								jsonAEnviar.push(data[posicion]);
							});
							// Relleno el formData que envio a PHP
							formDataInsertar.append("precio",formDataInputs.get('precioBloque'));
							formDataInsertar.append("json",JSON.stringify(jsonAEnviar));
	
							// console.log(formDataInsertar.get('json'));
							(async () => {
								// Hago la llamada y si todo es correcto muestro el mensaje
								try {
									const segundosDatos = await enviarDatos('./PHP/establecerPrecioEnBloque/ModificarPrecios.php', formDataInsertar);
									mensajeInformativo(segundosDatos.message,true);
								} catch (error) {
									mensajeInformativo("Error al actualizar el precio en las empresas",true);								
								}
							})();
						}
					});
		...
		let h6Informativo = document.createElement("h6");
		h6Informativo.textContent = "Para poder establecer el precio en bloque el compromiso de compra debe de ser igual a 'SI'";
		h6Informativo.classList.add("text-danger","text-center");
		document.querySelector("#cardPrecioBloque").appendChild(h6Informativo);
	}
</script>
</html>