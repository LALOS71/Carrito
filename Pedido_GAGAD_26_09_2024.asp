									...
									<tr style="background-color:#FCFCFC" valign="top">
												...
												IF not envios_parciales.eof then%>
															<table class="table table-sm no_imprimir"  border="0" cellspacing="0" cellpadding="0" id="tabla_envios_parciales_<%=pedidos("articulo")%>" style="display:none"  bgcolor="#FFFFFF">
																...
																while not envios_parciales.eof																 
																	ReDim Preserve envioParcial(i)																	
																	' Llenar el array con el valor del remito (campo albaran)
																	envioParcial(i) = envios_parciales("albaran")																	
																	i = i + 1
																	%>
										...
										<%if pedidos("albaran")<>"" then%>
											...
											If pedidos("codigo_ruc") <> "" Then											
												...
												If IsNull(serialValue) Or serialValue = "" Or serialValue = 0 Then
													if fila = 1 then												
														response.write("<a href='#' class='link link-info' id='openModal' title='Cargar Información Air WayBill'>")
														response.write("<img src='../images/upload.png' alt='Cargar Información Air WayBill' class='img-responsive' />")
														response.write("</a>")
													end if											
												End If
												'Else
										...
										<%ElseIf pedidos("estado_articulo")="ENVIO PARCIAL" Then %>
												...
												<%
													...
													' Verificar si el array contiene datos
													If IsArray(envio_Parcial) Then
														' Recorrer e imprimir cada remito almacenado en el array
														For i = 0 To UBound(envio_Parcial)

															If IsNull(serialValues) Or serialValues = "" Or serialValues = 0 Then
																response.write("Esto es un Pedido parcial " & serialValues)

																'response.write("Remito: " & envio_Parcial(i) & "<br/>")%>
																<div id="celda_albaran_parcial<%=envio_Parcial(i)%>" onclick="ver_albaran('<%=envio_Parcial(i)%>', '<%=entorno%>')" style="text-decoration:none;color:#000000;cursor:pointer;cursor:hand">
																	<%=envio_Parcial(i)%>
																</div>
																												
																		<a href='#' class='link link-info' id="openModal_<%=envio_Parcial(i)%>" title='Cargar Información Air WayBill'>
																		<img src='../images/upload.png' alt='Cargar Información Air WayBill' class='img-responsive' /></a>
															<%		
															End If										
														
													'response.write(pedidos)
															...
															' 	end if											
															' End If
													''response.write("Esto es un Pedido parcial " & pedidos("albaran"))%>



													<%	Next
													Else
														response.write("No hay albaran disponibles.<br/>")
													End If%>
