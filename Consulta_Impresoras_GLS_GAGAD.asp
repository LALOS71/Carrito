...
mostrar_impresoras = function (){
	...
	$.ajax({
        url: "Tabla_Impresoras_GLS_GAGAD.asp",
...
realizar_accion = function(sn_o_pedido, accion) {
	...
	if (estado=='BAJA' || estado=='EN REVISION' || estado=='RETIRADA')
		{
		accion=estado
		}
...
lanzar_accion = function(sn_o_pedido, estado, perfil, accion) {
	...
	$.ajax({
		url: "Modificar_Impresoras_GLS_GAGAD.asp",
			...
			$("#lista_impresoras").on("dblclick", "tbody td:nth-child(3)", function(e) {
				...
				bootbox.alert({
						...
						onShown: function() {
							...
							$.ajax({
									url: "Historico_Impresoras_GLS_GAGAD.asp",
									contentType: "application/x-www-form-urlencoded; charset=UTF-8",
