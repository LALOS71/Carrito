		...
		<div class="container-fluid">
			<div class="row">
				<div class="card col-12">
					<div class="card-body">
						<form name="frmconsulta_pedidos" action="Consulta_Pedidos_GAGAD.asp" method="post">
...
consultar_pedidos = function(perfil) {  
		...
		*/
        ...
        if (typeof lst_pedidos == 'undefined') {
			...
			if(esCarlos){
				...
				let iva = document.createElement("th");
				iva.textContent = "IVA";

				let total = document.createElement("th");
				total.textContent = "TOTAL";
				let costeProd = document.createElement("th");
				costeProd.textContent = "Coste Producción";

				// Utiliza el método insertBefore() para insertar los nuevos elementos th justo después del elemento específico
				importe.parentNode.insertBefore(costeProd, importe.nextSibling);
				importe.parentNode.insertBefore(total, importe.nextSibling);
				importe.parentNode.insertBefore(iva, importe.nextSibling);
    ...
    lst_pedidos = j$('#lista_pedidos').DataTable({dom:'<"toolbar">Blfrtip',
                                         ...
                                         columns:[
                                                    ...
                                                    {data:'TotIva',render: function (data, type, row, meta) 
                                                                {
                                                                if ( type === "display" ) //si se visualiza se formatea
                                                                    {
                                                                    valor=j$.fn.dataTable.render.number( '.', ',', 2).display(data.replace(',', '.'))
                                                    ...
                                                    {data: 'TotalSinIVA',render: function (data, type, row, meta) 
                                                                {
                                                                if ( type === "display" ) //si se visualiza se formatea
                                                                    {
                                                                    valor=j$.fn.dataTable.render.number( '.', ',', 2).display(data.replace(',', '.'))
                                                    ...
                                                    {data: 'TotalCoste',render: function (data, type, row, meta) 
                                                                {
                                                                if ( type === "display" ) //si se visualiza se formatea
                                                                    {
                                                                    valor=j$.fn.dataTable.render.number( '.', ',', 2).display(data.replace(',', '.'))
                                                    ...
                                                    {data:function(row, type, val, meta)
                                                            ...
                                                            if ((row.ESTADO!='ENVIADO') && (row.EMPRESA_ID!=4) && (row.EMPRESA_ID!=280) && (row.EMPRESA_ID!=260) && (row.EMPRESA_ID!=30) && (row.EMPRESA_ID!=180) && (row.Nreg!=0))
                                                                {
