...
'****************************************************************************
	...
	if not hoteles.eof then
		...
		if contrasenna_hotel=contrasenna_seleccionada then
			...
			'si es una oficina de ASM-GLS y es FRANQUICIA...
			'si la fecha de alta es posterior a la fecha limite (13/11/2017) y todavia no ha hecho pedidos
			'TIENE DERECHO AL DESCUENTO DEL PRIMER PEDIDO
			if session("usuario_codigo_empresa")=4 and session("usuario_tipo")="AGENCIA" then
				if hoteles("fecha_alta")<>"" then
					if datediff("d", "13/07/2017", hoteles("fecha_alta"))>=0 then
...
<div class="modal fade" id="pantalla_popup_asm" data-keyboard="false" data-backdrop="static">	
...
<!--pantalla con el popup a mostrar en GAG-->
<div class="modal fade" id="pantalla_popup_gag" data-keyboard="false" data-backdrop="static">	
    <div class="modal-dialog modal-md">	  
      <div class="modal-content">	    
        <div class="container-fluid">
			<div class="col-md-12" align="center">
				<img class="img-responsive" src="popup/GAG_r.jpg" border="0">
			</div>
		</div>	
        <div class="modal-footer">                  
          <p><button type="button" class="btn btn-default" data-dismiss="modal"><%=validar_ventana_mensajes_boton_cerrar%></button></p>                
