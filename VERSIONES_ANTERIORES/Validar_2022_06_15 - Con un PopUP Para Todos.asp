...
'****************************************************************************
	...
	if empresa_query="AVORIS" then
		hotel_seleccionado=7032 'GLOBALIA AUTOCAES - MADRID
		contrasenna_seleccionada="AUT-MADRID"
		empresa_entrada=230
	end if
	...
	if empresa_query="GROUNDFORCE" then
		hotel_seleccionado=1465 'GROUNDFORCE SEVILLA UTE
...
function moverse(sino, administrador_central, carpeta, empresa, mostrar_aviso_popup, usuario)	{
	...
	if (sino=='SI')
		...
		if (empresa==4)  
			...
			//$("#pantalla_popup_devoluciones").modal("show");
			$("#pantalla_popup_san_juan_de_sahagun").modal("show");
			/*
			if (('<%=session("usuario_tipo")%>'!='GLS PROPIA') && ('<%=session("usuario_pais")%>'!='PORTUGAL'))
				{
				$("#pantalla_popup_gls_preventa_vestuario_nuevo_logo").modal("show");
				//$("#pantalla_popup_saldos").modal("show");
			...
			*/	
			...
			/*
			if ((('<%=session("usuario_tipo")%>'=='AGENCIA') && ('<%=session("usuario_pais")%>'=='ESPAÑA')) || (('<%=session("usuario_tipo")%>'=='ARRASTRES') && ('<%=session("usuario_pais")%>'=='ESPAÑA')))
				{
				$("#pantalla_popup_semana_santa_2021").modal("show");  	
				}
			  else
...
<div class="modal fade" id="pantalla_popup_san_juan_de_sahagun" data-keyboard="false" data-backdrop="static">	
    <div class="modal-dialog modal-md">	  
      <div class="modal-content">	    
        <div class="container-fluid">
			<div class="col-md-12" align="center">
				<img class="img-responsive" src="popup/San_Juan_de_Sahagun_r.jpg" border="0">
			</div>
		</div>	
...
<div class="modal fade" id="pantalla_popup_gls_preventa_vestuario_nuevo_logo" data-keyboard="false" data-backdrop="static">	
    <div class="modal-dialog modal-md">	  
      <div class="modal-content">	    
        <div class="container-fluid">
			<div class="col-md-12" align="center">
				<img class="img-responsive" src="popup/GLS_Preventa_Vestuario_Nuevo_Logo_r.jpg" border="0">
			</div>
		</div>	
        <div class="modal-footer">                  
          <p><button type="button" class="btn btn-default" data-dismiss="modal"><%=validar_ventana_mensajes_boton_cerrar%></button></p>                
...
<div class="modal fade" id="pantalla_popup_saldos" data-keyboard="false" data-backdrop="static">	
    <div class="modal-dialog modal-md" style="width:850px;">	  
      <div class="modal-content">	    
        <div class="container-fluid">
			<div class="col-md-12" align="center" style="margin-top:5px ">
				<img class="img-responsive" src="popup/GLS_Saldos_r.jpg" border="0">
				
			</div>
		</div>	
        <div class="modal-footer">  
				<div class="text-right">
					<button type="button" class="btn btn-primary corporativo_gls" id="cmdmanual_saldos">Ver Manual</button>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<div class="modal fade" id="pantalla_popup_impresora_termica" data-keyboard="false" data-backdrop="static">	
    <div class="modal-dialog modal-md">	  
      <div class="modal-content">	    
        <div class="container-fluid">
			<div class="col-md-12" align="center">
				<img class="img-responsive" src="popup/GLS_Impresora_Termica_r.jpg" border="0">
			</div>
		</div>	
        <div class="modal-footer">                  
          <p><button type="button" class="btn btn-default" data-dismiss="modal"><%=validar_ventana_mensajes_boton_cerrar%></button></p>                
...
$("#cmdmanual_saldos" ).on( "click", function() {
  window.open('GAG/manuales/Manual_Compensacion_Saldos_GLS.pdf')
});


$('#pantalla_popup_gls_preventa_vestuario_nuevo_logo').on('hidden.bs.modal', function (e) {
	$("#pantalla_popup_saldos").modal("show")
})


$('#pantalla_popup_san_juan_de_sahagun').on('hidden.bs.modal', function (e) {
	if (<%=empresa_entrada%>==4)	
		{
		if (('<%=session("usuario_tipo")%>'!='GLS PROPIA') && ('<%=session("usuario_pais")%>'!='PORTUGAL'))
				{
				$("#pantalla_popup_gls_preventa_vestuario_nuevo_logo").modal("show");
		...
		{
		if (<%=empresa_entrada%>==2)
			{
			$("#pantalla_popup_be_live_oferta").modal("show");
			}
	
		if (<%=empresa_entrada%>==7)
			{
			$("#pantalla_popup_abba_documentacion").modal("show"); 
			}
	
		if (<%=empresa_entrada%>==130)
			{
			$("#pantalla_popup_geomoon_oferta").modal("show");  	
			}
			
		if (<%=empresa_entrada%>==5)
			{
			location.href=document.getElementById('ocultoruta').value
			}
			
		if ((<%=empresa_entrada%>==10)||(<%=empresa_entrada%>==20)||(<%=empresa_entrada%>==240)||(<%=empresa_entrada%>==250))
			{
			if ((<%=empresa_entrada%>==10)||(<%=empresa_entrada%>==240))
				{
				cadena_imagen='popup/Tarjeta_de_Visita_Halcon.jpg'
				}
			  else
			...
			{
			if (<%=empresa_entrada%>!=2 && <%=empresa_entrada%>!=5 && <%=empresa_entrada%>!=7 && <%=empresa_entrada%>!=130)
				{
				location.href=document.getElementById('ocultoruta').value
				}
			}
...
$('#pantalla_popup_saldos').on('hidden.bs.modal', function (e) {
	/*
	if (('<%=session("usuario_tipo")%>'=='ARRASTRES'))
		{
		location.href=document.getElementById('ocultoruta').value
		}
	  else
		{
		$("#pantalla_popup_impresora_termica").modal("show");
		}
	*/
	location.href=document.getElementById('ocultoruta').value
...
$('#pantalla_popup_impresora_termica').on('hidden.bs.modal', function (e) {
  location.href=document.getElementById('ocultoruta').value	//para que se dirija a la pagina despues de cerrar el popup
})
