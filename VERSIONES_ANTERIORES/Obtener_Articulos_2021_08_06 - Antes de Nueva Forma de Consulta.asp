	...
	<select  name="cmbarticulos" id="cmbarticulos">
		<option value="" selected>* TODOS *</option>
				<%for i=0 to UBound(mitabla_articulos,2)%>
							<%if valor_seleccionado=mitabla_articulos(CAMPO_CODIGO_SAP_ARTICULO,i) then%>
								<option value="<%=mitabla_articulos(CAMPO_CODIGO_SAP_ARTICULO,i)%>" selected><%=mitabla_articulos(CAMPO_CODIGO_SAP_ARTICULO,i)%> - <%=mitabla_articulos(CAMPO_DESCRIPCION_ARTICULO,i)%></option>
							  <%else%>
								<option value="<%=mitabla_articulos(CAMPO_CODIGO_SAP_ARTICULO,i)%>"><%=mitabla_articulos(CAMPO_CODIGO_SAP_ARTICULO,i)%> - <%=mitabla_articulos(CAMPO_DESCRIPCION_ARTICULO,i)%></option>
							<%end if%>
