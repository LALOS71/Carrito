...
mostrar_sn_impresoras = function (cantidad, estado)
{
		...
		botones = {
			  ...
			  ok: {
				...
				callback: function() {
				  ...
				  if (!size14) {
					bootbox.alert({message: "<h5>La Longitud del Numero de Serie no es de 14 caracteres.</h5>", centerVertical: true});
					return false;
				  }
				  
				  if (!valid) {
					bootbox.alert({message: "<h5>Introduzca valores en todas las cajas de texto.</h5>", centerVertical: true});
					return false;
				  }
				  var serials = [];
				  j$(".sn_impresoras").each(function() {
	...
	if (typeof array_impresoras_previas !== "undefined") {
      ...
      if (isNaN(cantidad) || cantidad <= 0) {
		bootbox.alert({message: "<h5>Introduzca una cantidad válida.</h5>", centerVertical: true});
        return;
      }
