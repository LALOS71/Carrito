...
<%
	...
	with pedidos
		...
		.Source= .Source & " PEDIDOS.PEDIDO_AUTOMATICO,"
		.Source= .Source & " Total * 0.21 TotIva, Total+(Total * 0.21) TotalEnvio, isnull(Nreg,0) Nreg"
...
function refrescar_pagina(orden){
	//alert(document.getElementById("cmbempresas").value)
	Actualizar_Combos('Obtener_Clientes.asp',document.getElementById("cmbempresas").value, document.getElementById("ocultocliente_seleccionado").value,'capa_hoteles', orden)
...
<body onload="refrescar_pagina(document.getElementById('ocultoorden_clientes').value)">
							...
							<td width="50%" class="dottedBorder vt al">								
									<table cellpadding="2" cellspacing="1" border="0" width="100%">
										<tr>
											...
											<td width="28%">
												<select  name="cmbempresas" id="cmbempresas"  onchange="refrescar_pagina(document.getElementById('ocultoorden_clientes').value)">
						...
						<td>	                                                       						
							<table border="0" cellpadding="1" cellspacing="1" width="102%" class="info_table" align="center">
								<tr style="background-color:#FCFCFC" valign="top">
									<th class="menuhdr" width="252">Cliente</th>
									<th class="menuhdr" width="81">Num. Pedido</th>
									<th class="menuhdr" width="72">Fecha</th>
									<th class="menuhdr" width="90">Importe</th>
									<th class="menuhdr" width="115">Estado</th>
                                    <th class="menuhdr" width="115">Acción</th>
								</tr>
								<%if pedidos.eof then%>
									<tr> 
								...
								<%vueltas=1
									...
									<tr  style="cursor:hand;cursor:pointer;" valign="top" onmouseover="javascript:this.style.background='#ffc9a5';" onmouseout="javascript:this.style.background='#FCFCFC'">
										<td  onclick="mostrar_pedido(<%=pedidos("id")%> ,<%=pedidos("Nreg")%>);return false" class="item_row" width="252" align="left" style="background-color:<%=color_fila%>;">
										<%=pedidos("empresa")%> -
										...
										<td  onclick="mostrar_pedido(<%=pedidos("id")%>,<%=pedidos("Nreg")%>);return false" class="ac item_row" width="81" align="right" style="background-color:<%=color_fila%>;"><%=pedidos("id")%></td>
										<td  onclick="mostrar_pedido(<%=pedidos("id")%>,<%=pedidos("Nreg")%>);return false" class="item_row" style="background-color:<%=color_fila%>;text-align:left" width="72" ><%=pedidos("fecha")%></td>                                            
                                        <td  onclick="mostrar_pedido(<%=pedidos("id")%>,<%=pedidos("Nreg")%>);return false" 
                                            class="item_row" style="background-color:<%=color_fila%>;text-align:left" width="72" >                                        
                                            <%																											
										...
										<td  onclick="mostrar_pedido(<%=pedidos("id")%>,<%=pedidos("Nreg")%>);return false" width="146" class="ac item_row" style="background-color:<%=color_fila%>;"><%=pedidos("estado")%></td>
										<td  width="115" class="ac item_row" style="background-color:<%=color_fila%>;">
