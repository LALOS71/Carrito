		...
		with consumos
			...
			if agrupacion_seleccionada="empresa" then
				...
				if diferenciar_articulos_seleccionada="SI" then
					.Source= .Source & ", ARTICULOS.CODIGO_SAP, ARTICULOS.DESCRIPCION, ARTICULOS.UNIDADES_DE_PEDIDO, ARTICULOS.RAPPEL, ARTICULOS.VALOR_RAPPEL"
					.Source= .Source & ", ARTICULOS.PRECIO_COSTE, (SELECT DESCRIPCION FROM PROVEEDORES WHERE ID=ARTICULOS.PROVEEDOR) AS PROVEEDOR"
					.Source= .Source & ", ARTICULOS.REFERENCIA_DEL_PROVEEDOR"
				end if
				...
				if diferenciar_articulos_seleccionada="SI" then
					.Source= .Source & ", ARTICULOS.CODIGO_SAP, ARTICULOS.DESCRIPCION, ARTICULOS.UNIDADES_DE_PEDIDO, ARTICULOS.RAPPEL, ARTICULOS.VALOR_RAPPEL"
					.Source= .Source & ", ARTICULOS.PRECIO_COSTE, ARTICULOS.PROVEEDOR"
					.Source= .Source & ", ARTICULOS.REFERENCIA_DEL_PROVEEDOR"
				end if
				if diferenciar_marca_seleccionada="SI" then
					.Source= .Source & ", V_CLIENTES.MARCA"
...
'FROM
'(SELECT 
'--MAX(A.ID) AS ID_ARTICULO
'A.ID AS ID_ARTICULO
', A.CODIGO_SAP as CODIGO_SAP, A.DESCRIPCION as ARTICULO
', A.UNIDADES_DE_PEDIDO, A.RAPPEL, A.VALOR_RAPPEL, A.PRECIO_COSTE
', (SELECT DESCRIPCION FROM PROVEEDORES WHERE ID=A.PROVEEDOR) AS PROVEEDOR
', A.REFERENCIA_DEL_PROVEEDOR
', E.TIPO
'--, SUM(B.CANTIDAD) as CANTIDAD_TOTAL
', B.CANTIDAD as CANTIDAD_TOTAL
'--, SUM(ROUND(((D.TOTAL / D.CANTIDAD) * B.CANTIDAD), 2)) AS TOTAL_IMPORTE 
', ROUND(((D.TOTAL / D.CANTIDAD) * B.CANTIDAD), 2) AS TOTAL_IMPORTE 
', B.FECHA AS FECHA_ENVIO
'FROM ARTICULOS A 
'INNER JOIN ENTRADAS_SALIDAS_ARTICULOS B 
'ON A.ID=B.ID_ARTICULO AND B.E_S='SALIDA' AND B.TIPO='PEDIDO' 
'INNER JOIN PEDIDOS C 
'ON C.ID = B.PEDIDO 
'INNER JOIN PEDIDOS_DETALLES D 
'ON C.ID=D.ID_PEDIDO AND A.ID=D.ARTICULO 
'INNER JOIN V_CLIENTES E 
'ON C.CODCLI = E.Id 
'INNER JOIN V_EMPRESAS F 
'ON E.EMPRESA = F.Id 

'WHERE 1=1 

'AND (CONVERT(VARCHAR(8), B.FECHA, 112) >= CONVERT(VARCHAR(8), CONVERT(DATETIME,'10-06-2020', 103) , 112)) 
'AND (CONVERT(VARCHAR(8), B.FECHA, 112) <= CONVERT(VARCHAR(8), CONVERT(DATETIME,'14-04-2021', 103) , 112)) 
'AND (A.CODIGO_SAP = 'RBDEC0026') 

'--GROUP BY A.CODIGO_SAP, A.DESCRIPCION, A.UNIDADES_DE_PEDIDO, A.RAPPEL
'--, A.VALOR_RAPPEL, A.PRECIO_COSTE, A.PROVEEDOR, A.REFERENCIA_DEL_PROVEEDOR
'--, E.TIPO 

'--order by A.DESCRIPCION, E.TIPO
') ARTICULOS

'LEFT JOIN
'(SELECT Z.ID_ARTICULO
'--, SUM(Z.UNIDADES_ACEPTADAS) AS UNIDADES_DEVUELTAS
', Z.UNIDADES_ACEPTADAS AS UNIDADES_DEVUELTAS
', Z.FECHA_ACEPTACION
', Y.FECHA AS FECHA_SALIDA
' FROM DEVOLUCIONES_DETALLES Z
' ---
' INNER JOIN ENTRADAS_SALIDAS_ARTICULOS Y
'ON Z.ID_ARTICULO=Y.ID_ARTICULO AND Z.ID_PEDIDO=Y.PEDIDO AND Z.UNIDADES_ACEPTADAS>=1 AND Y.E_S='SALIDA' AND Y.TIPO='PEDIDO'

'WHERE Z.ID_ARTICULO=2257
'AND Z.UNIDADES_ACEPTADAS>=1
'--AND (CONVERT(VARCHAR(8), Z.FECHA_ACEPTACION, 112) >= CONVERT(VARCHAR(8), CONVERT(DATETIME,'10-06-2020', 103) , 112)) 
'--AND (CONVERT(VARCHAR(8), Z.FECHA_ACEPTACION, 112) <= CONVERT(VARCHAR(8), CONVERT(DATETIME,'14-04-2021', 103) , 112)) 
'--GROUP BY ID_ARTICULO
') DEVOLUCIONES
'ON ARTICULOS.ID_ARTICULO=DEVOLUCIONES.ID_ARTICULO
			...
			'FALTA POR AGRUPAR..... 
				...
				'.Source= .Source & " GROUP BY A.CODIGO_SAP, A.DESCRIPCION, A.UNIDADES_DE_PEDIDO, A.RAPPEL, A.VALOR_RAPPEL"
				...
				.Source= .Source & " INNER JOIN ENTRADAS_SALIDAS_ARTICULOS Y"
				...
				'.Source= .Source & " GROUP BY ID_ARTICULO"
				.Source= .Source & ") DEVOLUCIONES"
			...
			if articulos_sin_consumo_seleccionada="SI" then
				...
				cadena_envios=cadena_envios & " WHERE PEDIDOS_DETALLES.ESTADO='ENVIADO'"
				if fecha_i<>"" then
		...
		<div id="main">
														...
														<tr  valign="top" id="fila_articulo_<%=i%>">
															...
															<%if diferenciar_articulos_seleccionada="SI" then%>
																...
																<%if diferenciar_rappel_seleccionado="SI" then%>
																	...
																	<td  class="ac item_row" width="50">
																		<%=consumos("VALOR_RAPPEL")%>
																	</td>
																<%end if%>	
																<%if diferenciar_costes_seleccionado="SI" then%>
																	<td  class="ac item_row" width="101">
																		<%=consumos("PRECIO_COSTE")%>
																	</td>
																	<td  class="ac item_row" width="101">
																		<%=consumos("PROVEEDOR")%>
																	</td>
																	<td  class="ac item_row" width="101">
																		<%=consumos("REFERENCIA_DEL_PROVEEDOR")%>
																	</td>
																<%end if%>												
															<%end if%>
															<%if diferenciar_marca_seleccionada="SI" then%>
															...
															<%if diferenciar_rappel_seleccionado="SI" then%>
																<td  class="ar item_row" width="101">
																<%
																if consumos("TOTAL_IMPORTE")<>"" and consumos("VALOR_RAPPEL")<>"" AND (consumos("TIPO")="AGENCIA" OR consumos("TIPO")="ARRASTRES") then
		  ...
		  <div class="submit_btn_container___" align="center">	
					...
					<table width="13%" border="0" cellpadding="0" cellspacing="0" align="center" class="info_column">
						<tr>
							<td>
							<form id="frmpasar_excel" name="frmpasar_excel" method="post" action="Informe_Excel.asp" onsubmit="rellenar_nombre_articulo()">
