...
<%
	...
	Next
		...
		''''''''''''''''
		'si es un pedido que enviamos a un proveedor
		...
		cadena_pedido_proveedor=cadena_pedido_proveedor & " AND A.ESTADO='EN PROCESO'"
		...
		'''''''''''''''''
		'FINAL ENVIO DEL PEDIDO AL PROVEEDOR
...
<BODY onload="validar('<%=creamos_albaran%>','<%=albaran_nuevo%>', '<%=acciones%>', '<%=enviar_email_al_proveedor%>')">
