...
<%
	...
	connimprenta.BeginTrans 'Comenzamos la Transaccion
		...
		if entrada_salida_seleccionada="ENTRADA" and tipo_seleccionado="APROVISIONAMIENTO" then
			...
			cadena_historico="INSERT INTO [192.168.156.175\SERVERSQL].GAG.dbo.GESTION_GRAPHISOFT_HISTORICO_HOJAS (ID_HOJA_RUTA, HOJA_RUTA, FECHA, ACCION, CAMPO, VALOR_ANTIGUO, VALOR_NUEVO,"
			cadena_historico=cadena_historico & "USUARIO, DESCRIPCION, TIPO_INCIDENCIA)"
			cadena_historico=cadena_historico & " SELECT"
			
			cadena_historico=cadena_historico & " (SELECT ID from [192.168.156.175\SERVERSQL].GAG.dbo.GESTION_GRAPHISOFT_HOJAS_IMPORTADAS WHERE HOJA_DE_RUTA=" & albaran_seleccionado & ")"
			cadena_historico=cadena_historico & ", '" & albaran_seleccionado & "'"
			 
			cadena_historico=cadena_historico & ", GETDATE(), 'CAMBIO', 'ESTADO'"
			cadena_historico=cadena_historico & ", (SELECT DESCRIPCION FROM [192.168.156.175\SERVERSQL].GAG.dbo.GESTION_GRAPHISOFT_ESTADOS"
			cadena_historico=cadena_historico & " WHERE ID=(SELECT ID_ESTADO from [192.168.156.175\SERVERSQL].GAG.dbo.GESTION_GRAPHISOFT_HOJAS_IMPORTADAS WHERE HOJA_DE_RUTA=" & albaran_seleccionado & "))"
