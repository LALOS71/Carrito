...
'****************************************************************************
	...
	if empresa_query="ECUADOR" and tipo_query="FRANQUICIA" then
		hotel_seleccionado=5508 'C21 ECUADOR franquicia
		contrasenna_seleccionada="ECUADOR_C21"
		empresa_entrada=20
	end if
...
'CADE	170	GLOBALIA CORPORATE TRAVEL	GLOBALIA_CORPORATE_TRAVEL	1	#8232#
	...
	response.write("<br>session usuario: " & session("usuario"))
	response.write("<br>session usuario_codigo_externo: " & session("usuario_codigo_externo"))
	response.write("<br>session usuario_nombre: " & session("usuario_nombre"))
	response.write("<br>session usuario_direccion: " & session("usuario_direccion"))
	response.write("<br>session usuario_poblacion: " & session("usuario_poblacion"))
	response.write("<br>session usuario_cp: " & session("usuario_cp"))
	response.write("<br>session usuario_provincia: " & session("usuario_provincia"))
	response.write("<br>session usuario_telefono: " & session("usuario_telefono"))
	response.write("<br>session usuario_fax: " & session("usuario_fax"))
	response.write("<br>session usuario_pedido_minimo_sin_compromiso: " & session("usuario_pedido_minimo_sin_compromiso"))
	response.write("<br>session usuario_pedido_minimo_con_compromiso: " & session("usuario_pedido_minimo_con_compromiso"))
	response.write("<br>session usuario_empresa: " & session("usuario_empresa"))
	response.write("<br>session usuario_codigo_empresa: " & session("usuario_codigo_empresa"))
	response.write("<br>session usuario_marca: " & session("usuario_marca"))
	response.write("<br>session usuario_tipo: " & session("usuario_tipo"))
	response.write("<br>session usuario_requiere_autorizacion: " & session("usuario_requiere_autorizacion"))
	response.write("<br>session usuario_directorio_activo: " & session("usuario_directorio_activo"))
	response.write("<br>session usuario_fecha_alta: " & session("usuario_fecha_alta"))
	response.write("<br>session usuario_pais: " & session("usuario_pais"))
	response.write("<br>session usuario_trato_especial: " & session("usuario_trato_especial"))
	response.write("<br>session usuario_idsap: " & session("usuario_idsap"))
	
	response.write("<br>session usuario_primer_pedido: " & session("usuario_primer_pedido"))
	response.write("<br>session usuario_derecho_primer_pedido: " & session("usuario_derecho_primer_pedido"))
	response.write("<br>session color_asociado_empresa: " & session("color_asociado_empresa"))
	
	response.write("<br>session seleccion_asm_gls: " & session("seleccion_asm_gls"))
		
	response.write("<br>session numero_articulos: " & session("numero_articulos"))
	
	
	response.write("<br>objetos almacenados en la sesion....<br><br>")
	dim i
...
function moverse(sino, administrador_central, carpeta, empresa, mostrar_aviso_popup, usuario)	{
	...
	//  globalia (los gsc)(110), geomoon(130), glovalia corporate travel(170), marsol(210)
	if ((empresa==2)||(empresa==10)||(empresa==20)||(empresa==30)||(empresa==40)||(empresa==50)||(empresa==80)||(empresa==90)||(empresa==100)||(empresa==110)||(empresa==130)||(empresa==170)||(empresa==210))	{
	    cadena_carpeta='GAG/'
	    cadena_empresa='_Gag'			
	}		
	...
	if (sino=='SI')
		...
		if (empresa==4)  
			...
			document.getElementById('ocultoruta').value=ruta_redireccion	
			console.log('ruta_redireccion: ' + ruta_redireccion)
			//location.href=document.getElementById('ocultoruta').value
			
			/*
			if (('<%=session("usuario_tipo")%>'=='AGENCIA') && ('<%=session("usuario_pais")%>'=='ESPAÑA'))
				{
...
<div class="modal fade" id="pantalla_popup_gls_productos_navidad" data-keyboard="false" data-backdrop="static">	
    <div class="modal-dialog modal-md">	  
      <div class="modal-content">	    
        <div class="container-fluid">
			<div class="col-md-12" align="center">
				<img class="img-responsive" src="popup/GLS_Reserva_Productos_Navidad_r.jpg" border="0">
			</div>
		</div>	
        <div class="modal-footer">                  
          <p><button type="button" class="btn btn-default" data-dismiss="modal"><%=validar_ventana_mensajes_boton_cerrar%></button></p>                
