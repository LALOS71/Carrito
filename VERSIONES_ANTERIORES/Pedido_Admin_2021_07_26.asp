					  ...
					  <table width="98%" cellspacing="6" cellpadding="0" class="logintable" align="center">
							<tr>
								<td width="50%" class="dottedBorder vt al">
									...
									<h4>
										<font color="#000000">
										Destinarario: <%=ucase(pedidos("DESTINATARIO"))%>
										<%if nombre_empleado_gls <> "" then%>
											<br />Att: <%=ucase(nombre_empleado_gls)%>
										<%end if%>
										<br />Dirección: <%=ucase(pedidos("DESTINATARIO_DIRECCION"))%>
										<br />Localidad: <%=ucase(pedidos("DESTINATARIO_POBLACION"))%>
										<br />C.P.: <%=ucase(pedidos("DESTINATARIO_CP"))%>
										<br />Provincia: <%=ucase(pedidos("DESTINATARIO_PROVINCIA"))%>
										<br />País: <%=ucase(pedidos("DESTINATARIO_PAIS"))%>
										<br />Teléfono: <%=ucase(pedidos("DESTINATARIO_TELEFONO"))%>
								...
								<table border="0" cellpadding="1" cellspacing="1" width="100%" class="info_table">
											...
											<%'29-06-2016...  comprobamos si ha de ser un articulo personalizable
												...
												%>
													<img src="images/paper_verde_16x16.png" 
														...
														onclick="mostrar_capas_new('capa_informacion', '<%=plantilla_personalizacion%>','<%=pedidos("codcli")%>', '<%=carpeta_anno%>', '<%=pedido_modificar%>', '<%=id%>', '<%=cantidad%>')" style="cursor:pointer"
														/>
												<%end if%>
										...
										<td id="fila_pedido_<%=fila%>_estado" width="142">
											<div id="tabla_estado_<%=fila%>" style="width:100%">
														...
														<script language="javascript">
															...
															$("#cmbestados_<%=pedidos("articulo")%>").prop('oldvalue', '<%=pedidos("estado_articulo")%>');
														</script>
											</div>
											<%
									...
									<tr style="background-color:#FCFCFC" valign="top">
										...
										<%if  gastos_envio<>"" AND gastos_envio<>"0" then%>
											<tr style="background-color:#FCFCFC" valign="top">
												<th class="menuhdr" style="text-align:right" colspan="4"><font color="#880000">Gastos de Env&iacute;o</font></th>
												<th class="menuhdr" style="text-align:right"><font color="#880000"><%=FORMATNUMBER(gastos_envio,2,-1,0,-1)%> €</font></th>
												<td colspan="5"></td>
											</tr>
										  <%else
											gastos_envio=0
									...
									<tr style="background-color:#FCFCFC" valign="top">
										<th class="menuhdr" style="text-align:right" colspan="4"><font  color="#000000">IVA del 21% (<%=round(((total_pedido - resultado_descuento + gastos_envio) * 0.21),2)%>)</font></th>
										<th class="menuhdr" style="text-align:right"><font  color="#000000">
													
											<%
											resultado_iva=((total_pedido - resultado_descuento + gastos_envio) * 0.21)
									...
									<tr style="background-color:#FCFCFC" valign="top">
										...
										<th class="menuhdr" style="text-align:right"><font  color="#000000">
											<%
												total_pago_iva=(total_pedido - resultado_descuento + gastos_envio) + iva_21
    ...
    for (i=0;i<tabla_articulos_cantidades.length;i++)    {
	    ...
	    */
		...
		if ((document.getElementById('cmbestados_' + articulo_cantidad[0]).value=='ENVIO PARCIAL') || (document.getElementById('cmbestados_' + articulo_cantidad[0]).value=='ENVIADO'))
			...
			valor_combo_antiguo=$('#cmbestados_' + articulo_cantidad[0]).prop('oldvalue')
			...
			codigo_referencia=''
			$('#cmbestados_' + articulo_cantidad[0]).closest("tr").find("td:first-child a:first-child font:first-child").each(function(){
                codigo_referencia+=$(this).html();
            });
