...
<%
	...
	with pedidos
		...
		.Source= .Source & " ARTICULOS.DESCRIPCION, PEDIDOS_DETALLES.CANTIDAD, PEDIDOS_DETALLES.PRECIO_UNIDAD,"
						...
						<tr>
							<td width="785">
								...
								<table border="0" cellpadding="1" cellspacing="1" width="100%" class="info_table">
									<tr style="background-color:#FCFCFC" valign="top">
										<th class="menuhdr" colspan="9">
											...
											<%if pedidos("PEDIDO_AUTOMATICO")<>"" then%>
												&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
												Pedido Automatico: <font size="2" color="#880000"><b><%=pedidos("PEDIDO_AUTOMATICO")%></b></font>
											<%end if%>
									...
									<%if pedidos.eof then%>
										<tr> 
											<td bgcolor="#999966" align="center" colspan="9"><b><FONT class="fontbold">El 
									...
									<tr style="background-color:#FCFCFC" valign="top" onmouseover="informacion_adicional('<%=pedidos("DESCRIPCION")%>','<%=cadena_stocks%>', '<%=cadena_stocks_minimos%>', '<%=cadena_marcas%>')">
										...
										<td id="fila_pedido_<%=fila%>_estado" class="ac item_row" width="142">
											<select  name="cmbestados_<%=pedidos("articulo")%>" id="cmbestados_<%=pedidos("articulo")%>" style="font-size:9px;width:138px">
											<%if vacio_estados=false then %>
			                            		<%for i=0 to UBound(mitabla_estados,2)%>
									...
									<tr style="background-color:#FCFCFC" valign="top">
										<th class="menuhdr" style="text-align:right" colspan="4"><font  color="#000000">IVA del 21% (<%=round((total_pedido* 0.21),2)%>)</font></th>
										<th class="menuhdr" style="text-align:right"><font  color="#000000">
													
											<%
											resultado_iva=(total_pedido* 0.21)
									...
									<tr style="background-color:#FCFCFC" valign="top">
										...
										<th class="menuhdr" style="text-align:right"><font  color="#000000">
											<%
												total_pago_iva=total_pedido + iva_21
