	...
	%>
		...
		<%end if%>
						...
						<p class="card-text">
							...
							<%end if 'de compromiso de compra si%>
							...
							<%end if 'de compromiso de compra no%>
						</p>
...
j$(document).on('click', '.btn_add_escalados', function(event) 
	{
		...
		cadena = cadena + '<input type="hidden" name="oculto_id_' + valores[1] + '_' + valores[2] + '_x" id="oculto_id_' + valores[1] + '_' + valores[2] + '_x" value="" />'
		cadena = cadena + '</td>'
		cadena = cadena + '<td align="right">'
