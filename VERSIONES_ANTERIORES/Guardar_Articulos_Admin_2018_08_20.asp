...
<%
		...
		if accion="MODIFICAR" then 'aqui modificamos articulos
				...
				if accion="ALTA" then
					...
					if Request.Form("txtreferencia_por_proveedor")<>"" then
						cadena_valores=cadena_valores & Request.Form("txtreferencia_por_proveedor")
					  else
					  	cadena_valores=cadena_valores & "null"
					end if
					...
					if empresa_barcelo="S" then
						...
						if stock_marca<>"" then
							'metemos la linea de control de stock en el historico
							cadena_historico="INSERT INTO HISTORICO_STOCKS (FECHA, PEDIDO, ARTICULO, CANTIDAD, STOCK_ACTUAL, STOCK_NUEVO, PROCEDENCIA)"
							cadena_historico=cadena_historico & " VALUES (GETDATE(), NULL, " & numero_articulo_nuevo
							cadena_historico=cadena_historico & ", NULL, NULL" 
							cadena_historico=cadena_historico & ", " & stock_marca & ", 'Guardar_Articulos_Admin - CREAR ARTICULO')"
							'response.write("<br>" & cadena_historico)
							connimprenta.Execute "set dateformat dmy",,adCmdText + adExecuteNoRecords
							connimprenta.Execute cadena_historico,,adCmdText + adExecuteNoRecords
						END IF
