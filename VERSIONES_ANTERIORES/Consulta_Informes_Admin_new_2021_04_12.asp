	...
	set empresas=Server.CreateObject("ADODB.Recordset")
		...
		with consumos
			...
			if agrupacion_seleccionada="empresa" then
				...
				.Source="SELECT A.CODIGO_SAP as CODIGO_SAP"
				...
				if fecha_i<>"" then
					...
					.Source= .Source & " AND (CONVERT(VARCHAR(8), B.FECHA, 112) >= CONVERT(VARCHAR(8), CONVERT(DATETIME,'" & fecha_i & "') , 112))" 'pone las 2 fechas en formato yyyymmdd 
				end if
				if fecha_f<>"" then
					'.Source= .Source & " AND (B.FECHA <= '" & fecha_f & "')"
					.Source= .Source & " AND (CONVERT(VARCHAR(8), B.FECHA, 112) <= CONVERT(VARCHAR(8), CONVERT(DATETIME,'" & fecha_f & "') , 112))" 'pone las 2 fechas en formato yyyymmdd 
				end if
