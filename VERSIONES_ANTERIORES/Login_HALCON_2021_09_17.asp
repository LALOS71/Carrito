...
<%
	...
	dim datossucursal
		...
		direccion_ip_oculto=Request.Form("ocultoip")
		if direccion_ip_oculto="" then
			direccion_ip_oculto=direccion_ip
		end if

		'direccion_ip=Request.QueryString("ip")
		...
		if codigo_sucursal&""="" then 
			...
			'response.write("ip:" & direccion_ip)
			set  sucursales=Server.CreateObject("ADODB.Recordset")
			with sucursales
				.ActiveConnection=connimprenta
				.Source="Select  *  from [192.168.156.175\SERVERSQL].SUCPC.dbo.detalles_ips where ip='" & direccion_ip_oculto & "'"
...
function ir(empresa, suc, cont)
{
	...
	console.log('hotel: ' + document.getElementById('cmbhoteles').value)
