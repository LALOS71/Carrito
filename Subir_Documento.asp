...
'vemos si tiene las extensiones correctas
function extensiones_permitidas(extension)
	Dim i
	...
	For i = LBound(extensionesPermitidas) To UBound(extensionesPermitidas)
	...
	extensiones_permitidas= extensionPermitida
end function
...
' TamaÒo m·ximo del archivo en bytes (por ejemplo, 2MB)
tamanoMaximo = 2 * 1024 * 1024


' Cabeceras de archivos permitidas
Dim cabecerasPermitidas
Set cabecerasPermitidas = Server.CreateObject("Scripting.Dictionary")
cabecerasPermitidas.Add "jpg", Array(&HFF, &HD8, &HFF)
cabecerasPermitidas.Add "png", Array(&H89, &H50, &H4E, &H47)
cabecerasPermitidas.Add "gif", Array(&H47, &H49, &H46)
cabecerasPermitidas.Add "pdf", Array(&H25, &H50, &H44, &H46)
		...
		for i=0 to up.Form("ocultonum_elementos") - 1
					...
					if up.Form("ocultoid_carrusel_" & i)<>"" then
						...
						if up.Form("ocultoaccion_" & i)="BORRAR" then
							'response.write("<br>fichero - BORRAR: " + ruta & "\" & up.Form("ocultofichero_" & i))
							if fso.FileExists(ruta & "\" & up.Form("ocultofichero_" & i)) then
							...
							If up.Ficheros.Exists("txtfichero_" & i) Then
								...
								wend
								if not extensiones_permitidas(ext_fichero) then
								 	hay_error="SI"
									cadena_error=cadena_error & "&nbsp;&nbsp;&nbsp;&nbsp;- Los Ficheros han de ser jpg, jpeg, png<br>"
								end if
								if not tamanoMaximo > up.Ficheros("txtfichero_" & i).Tamano then
								 	hay_error="SI"
									cadena_error=cadena_error & "&nbsp;&nbsp;&nbsp;&nbsp;- Los Ficheros han de ser jpg, jpeg, png<br>"
								end if
						...
						If up.Ficheros.Exists("txtfichero_" & i) Then
							...
							while fso.FileExists(ruta & "\" & fichero_asociado)
								...
								'response.write("<br>numevo nombre de fichero: " & fichero_asociado)
							wend
							if extensiones_permitidas(ext_fichero) then
								up.Ficheros("txtfichero_" & i).GuardarComo fichero_asociado, ruta
							  else
							 	hay_error="SI"
								cadena_error=cadena_error & "&nbsp;&nbsp;&nbsp;&nbsp;- Los Ficheros han de ser jpg, jpeg, png<br>"
...
<script language="javascript">
	...
	function volver(hay_error, cadena_error)
	{
		if (hay_error=='NO')
			{
		...
		if (hay_error=='SI')
			...
			cadena="<div class='col-md-10 col-md-offset-1' style='margin-top:7px'>"
			cadena=cadena + "<h4>Se han producido los siguientes errores:.<h4><br><br>" + cadena_error
			cadena=cadena + "</div>"
			}
...
<body onload="volver('<%=hay_error%>', '<%=cadena_error%>')">
