...
////////////////////////////////////////////////////////////
			...
			var Plantilla_23 = function(nombre_agencia, calle, numero_calle, poblacion, cp, provincia, telefono, movil, email, web, fichero, email_pruebas){  
				 this.nombre_agencia;
				 this.calle = calle;  
...
$(function(){
	...
	$(document).on("click",".plantilla_23 .eliminar",function(){
		...
		$('.plantilla_23 .nombre_agencia').val('')
		$('.plantilla_23 .calle').val('')
		$('.plantilla_23 .numero_calle').val('')
		$('.plantilla_23 .poblacion').val('')
		$('.plantilla_23 .cp').val('')
		$('.plantilla_23 .provincia').val('')
		$('.plantilla_23 .telefono').val('')
		$('.plantilla_23 .movil').val('')
		$('.plantilla_23 .email').val('')
		$('.plantilla_23 .web').val('')
		$('.plantilla_23 .fichero').val('')
		$('.plantilla_23 .email_prueba').val('')
	...
	$("#guardar_plantillas").on("click", function(){
		...
		if ('<%=plantilla%>'=='plantilla_23')
			...
			$(".plantilla_23 .email_tarjeta").each(function(indice, elemento) {
				if (indice!=0)
					{
					if ($(elemento).val()!='')
						{
			...
			$(".plantilla_23 .provincia_tarjeta").each(function(indice, elemento) {
				if (indice!=0)
					{
					if ($(elemento).val()=='')
						{
...
function getFormJson(){ 
	...
	if ('<%=plantilla%>'=='plantilla_23')
		{
		var plan_nombre_agencias = $(".plantilla_23 .nombre_agencia");  
		var plan_calles = $(".plantilla_23 .calle");  
		var plan_numeros_calles = $(".plantilla_23 .numero_calle");  
		var plan_poblaciones = $(".plantilla_23 .poblacion");  
		var plan_cps = $(".plantilla_23 .cp");  
		var plan_provincias = $(".plantilla_23 .provincia");  
		var plan_telefonos = $(".plantilla_23 .telefono");  
		var plan_moviles = $(".plantilla_23 .movil");  
		var plan_emails = $(".plantilla_23 .email_tarjeta");  
		var plan_webs = $(".plantilla_23 .web");  
		var plan_ficheros = $(".plantilla_23 .ficheros");  
		var plan_emails_pruebas = $(".plantilla_23 .email_prueba");  
		
		jQuery.each(plan_telefonos, function(pos, item){  
			if (pos>0)
			{
			pedidoObj.addPlantilla(new Plantilla_23(
				...
				$.getJSON(ruta_fichero_json, function(plantillas) {
						...
						for (x in plantillas.plantillas)
							...
							if ('<%=plantilla%>'=='plantilla_23')
								...
								$('.plantilla_23 .nombre_agencia:eq(' + indice_plantillas + ')').val(plantillas.plantillas[x].nombre_agencia)
								$('.plantilla_23 .calle:eq(' + indice_plantillas + ')').val(plantillas.plantillas[x].calle)
								$('.plantilla_23 .numero_calle:eq(' + indice_plantillas + ')').val(plantillas.plantillas[x].numero_calle)
								$('.plantilla_23 .poblacion:eq(' + indice_plantillas + ')').val(plantillas.plantillas[x].poblacion)
								$('.plantilla_23 .cp:eq(' + indice_plantillas + ')').val(plantillas.plantillas[x].cp)
								$('.plantilla_23 .provincia:eq(' + indice_plantillas + ')').val(plantillas.plantillas[x].provincia)
								$('.plantilla_23 .telefono:eq(' + indice_plantillas + ')').val(plantillas.plantillas[x].telefono)
								$('.plantilla_23 .movil:eq(' + indice_plantillas + ')').val(plantillas.plantillas[x].fax)
								$('.plantilla_23 .email:eq(' + indice_plantillas + ')').val(plantillas.plantillas[x].email)
								$('.plantilla_23 .web:eq(' + indice_plantillas + ')').val(plantillas.plantillas[x].web)
								$('.plantilla_23 .fichero:eq(' + indice_plantillas + ')').val(plantillas.plantillas[x].fichero)
								$('.plantilla_23 .email_prueba:eq(' + indice_plantillas + ')').val(plantillas.plantillas[x].email_pruebas)
								}
	...
	<div class="plantilla_23" style="display:none">
		<div class="panel panel-default">
  			<div class="panel-body">
				<div class="row">
					<div class="col-xs-10 col-sm-10 col-md-10 col-lg-10 has-error">
						<span class="text-danger">Nombre Agencia:</span>
						<input type="text" class="form-control nombre_agencia requerir">
					</div>
				</div>
				<div class="row">&nbsp;</div>
				<div class="row">
					<div class="col-xs-2 col-sm-2 col-md-2 col-lg-2 has-error">
						<span class="text-danger">Tfno. Fijo:</span>
						<input type="text" class="form-control telefono requerir">
					</div>
				...
				<div class="row">
					<div class="col-xs-4 col-sm-4 col-md-4 col-lg-4 has-error">
						<span class="text-danger">Calle:</span>
						<input type="text" class="form-control calle requerir">
					</div>
					<div class="col-xs-2 col-sm-2 col-md-2 col-lg-2 has-error">
						<span class="text-danger">N&uacute;m.:</span>
						<input type="text" class="form-control numero_calle requerir">
					</div>
					<div class="col-xs-4 col-sm-4 col-md-4 col-lg-4 has-error">
						<span class="text-danger">Poblaci&oacute;n:</span>
						<input type="text" class="form-control poblacion requerir">
					</div>
					<div class="col-xs-2 col-sm-2 col-md-2 col-lg-2 has-error">
						<span class="text-danger">C.P.:</span>
						<input type="text" class="form-control cp requerir">
					</div>
				</div>
				<div class="row">&nbsp;</div>
				<div class="row">
					<div class="col-xs-5 col-sm-5 col-md-5 col-lg-5 has-error">
						<span class="text-danger">Provincia:</span>
						<input type="text" class="form-control provincia requerir">
					</div>
					<div class="col-xs-5 col-sm-5 col-md-5 col-lg-5 has-error">
						<span class="text-danger">Email Envio Prueba:</span>
						<input type="text" class="form-control email_prueba requerir">
					</div>
					<div class="col-xs-2 col-sm-2 col-md-2 col-lg-2">
						<br />
				...
				<div class="row">
					<div class="col-xs-10 col-sm-10 col-md-10 col-lg-10 has-error">
						<span class="text-danger">Fichero Para el Logo:</span>
						<input type="text" class="form-control fichero requerir">
					</div>
				</div>
