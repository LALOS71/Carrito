...
////////////////////////////////////////////////////////////
			...
			var Plantilla_Rotulacion_1 = function(campo, descripcion, email_pruebas){  
				 this.campo = campo;  
...
$(function(){
	...
	$(document).on("click",".plantilla_rotulacion_1 .eliminar",function(){
		
		$('.plantilla_rotulacion_1 .descripcion').val('')
		$('.plantilla_rotulacion_1 .email_pruebas').val('')
		
		$('.plantilla_rotulacion_1 .requerir').closest("div").addClass("has-error")
		$('.plantilla_rotulacion_1 .requerir').siblings().addClass("text-danger")
	});
	...
	$("#guardar_plantillas").on("click", function(){
		...
		//PLANTILLA_1		
		if ('<%=plantilla%>'=='plantilla_rotulacion_1')
			{
			$(".plantilla_rotulacion_1 .descripcion").each(function(indice, elemento) {
				if (indice!=0)
					{
					if ($(elemento).val()=='')
						{
						cadena_error=cadena_error + '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;- Este Dato es Obligatorio.<br>'
						hay_error='SI'
						} 
					}
			});
			
			$(".plantilla_rotulacion_1 .email_pruebas").each(function(indice, elemento) {
				if (indice!=0)
					{
					if ($(elemento).val()=='')
						{
...
function getFormJson(){ 
	...
	if ('<%=plantilla%>'=='plantilla_rotulacion_1')
		{
		var plan_campo = $(".plantilla_rotulacion_1 .campo");  
		var plan_descripcion = $(".plantilla_rotulacion_1 .descripcion");  
		var plan_email_pruebas = $(".plantilla_rotulacion_1 .email_pruebas");  
		
		jQuery.each(plan_descripcion, function(pos, item){  
			if (pos>0)
			{
			pedidoObj.addPlantilla(new Plantilla_Rotulacion_1(plan_campo[pos].value, plan_descripcion[pos].value, plan_email_pruebas[pos].value));  
			}
		});  
		}
						...
						for (x in plantillas.plantillas)
							...
							if ('<%=plantilla%>'=='plantilla_rotulacion_1')
								...
								if (indice_plantillas!=1)
									{
									$("#contenedor_plantillas .plantilla_rotulacion_1:first").clone().appendTo("#contenedor_plantillas").hide().fadeIn('slow');
									}
								
								$('.plantilla_rotulacion_1 .descripcion:eq(' + indice_plantillas + ')').val(plantillas.plantillas[x].descripcion)
								$('.plantilla_rotulacion_1 .email_pruebas:eq(' + indice_plantillas + ')').val(plantillas.plantillas[x].email_pruebas)
								}
							indice_plantillas++
							}				
...
<div class="container-fluid" id="contenedor_plantillas" name="contenedor_plantillas">
	...
	<div class="plantilla_rotulacion_1" style="display:none">
		<div class="panel panel-default">
  			<div class="panel-body">
				<div class="row">
					<div class="col-xs-7 col-sm-7 col-md-7 col-lg-7 has-error">
						<span class="text-danger"><%=campos%>:</span>
						<input type="hidden" class="campo" value="<%=campos%>" />
						<input type="text" class="form-control descripcion requerir">
					</div>
				</div>
				<div class="row">
					<div class="col-xs-7 col-sm-7 col-md-7 col-lg-7 has-error">
	...
	</div><!--fin plantilla_rotulacion_1-->	
