...
<%
	...
	if session("usuario")="" then
		...
		with detalles_pedido
			...
			.Source="SELECT * FROM PEDIDOS_DETALLES WHERE ID_PEDIDO=" & pedido_a_borrar & " AND ESTADO<>'SIN TRATAR'"
			.Open
		end with
...
<BODY onload="validar('<%=mensaje_aviso%>')">
	...
	'conn.execute sql
%>
<form name="frmborrar_pedido" id="frmborrar_pedido" method="post" action="Consulta_Pedidos.asp">
</form>
</BODY>
   <%	
   		'regis.close			
