...
<%
	...
	response.write("<br>entrando en pago_realizado.asp")
	...
	OpenFileobj.WriteLine("DENTRO DE PAGO_REALIZADO.ASP")
	...
	If (Request.Form.Count>0 or Request.QueryString.Count>0) Then'//URL DE RESP. ONLINE
		...
		If (firma = signatureRecibida) Then
			...
			if cint(miObj.getParameter("Ds_Response"))<= 99 then
				...
				OpenFileobj.WriteBlankLines(4)
				OpenFileobj.WriteLine("cadena historico: " & cadena_ejecucion)
				...
				with datos_pedido_pagado
					...
					OpenFileobj.WriteBlankLines(4)
					OpenFileobj.WriteLine("CADENA pedido consultado: " & .source)
				...
				OpenFileobj.WriteBlankLines(4)
				OpenFileobj.WriteLine("cadena pagos tarjeta: " & cadena_ejecucion)

				connimprenta.Execute "set dateformat dmy",,adCmdText + adExecuteNoRecords 	
				connimprenta.Execute cadena_ejecucion,,adCmdText + adExecuteNoRecords 
					
				'actualizamos el estado del pedido
				cadena_ejecucion="UPDATE PEDIDOS SET ESTADO='SIN TRATAR' WHERE ID=" & pedido_consultado_pedido
				
				'response.write("<br>cadena pagos tarjeta: " & cadena_ejecucion)
				OpenFileobj.WriteBlankLines(4)
				OpenFileobj.WriteLine("cadena actualizacion estado del pedido: " & cadena_ejecucion)

				connimprenta.Execute cadena_ejecucion,,adCmdText + adExecuteNoRecords 
				
				'actualizamos el estado de los detalles del pedido
				cadena_ejecucion="UPDATE PEDIDOS_DETALLES SET ESTADO='SIN TRATAR' WHERE ID_PEDIDO=" & pedido_consultado_pedido
				
				'response.write("<br>cadena pagos tarjeta: " & cadena_ejecucion)
				OpenFileobj.WriteBlankLines(4)
				OpenFileobj.WriteLine("cadena actualizacion estado del pedido EN LOS DETALLES: " & cadena_ejecucion)

				connimprenta.Execute cadena_ejecucion,,adCmdText + adExecuteNoRecords 
				
				OpenFileobj.WriteBlankLines(4)
				OpenFileobj.WriteLine("CON ESTO TERMINARIAMOS....")
				...
				with tipos_errores
					...
					'response.write("<br>cadena descripciones de error: " & .source)
					OpenFileobj.WriteBlankLines(4)
					OpenFileobj.WriteLine("cadena descripciones de error: " & .source)
				...
				OpenFileobj.WriteBlankLines(4)
				OpenFileobj.WriteLine("cadena historico con error: " & cadena_ejecucion)
		...
		OpenFileobj.WriteBlankLines(4)
		OpenFileobj.WriteLine("ANTES DEL COMMITTRANS")
		connimprenta.CommitTrans
		OpenFileobj.WriteBlankLines(4)
		OpenFileobj.WriteLine("DESPUES DEL COMMTITRNAS")
