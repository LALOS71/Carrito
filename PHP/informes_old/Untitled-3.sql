CONSULTA: -- TODOS LOS FILTROS INVOLUCRADOS
SELECT
  ARTICULOS.*
...
FROM (SELECT
   ...
   ,D.NOMBRE
   ,D.CODIGO_EXTERNO
   ,B.ARTICULO AS ID_ARTICULO
   ,F.CODIGO_SAP
   ,F.DESCRIPCION
   ,F.UNIDADES_DE_PEDIDO
   ,F.RAPPEL
   ,F.VALOR_RAPPEL
   ,F.PRECIO_COSTE
   ,(SELECT
        DESCRIPCION
      FROM PROVEEDORES
      WHERE ID = F.PROVEEDOR)
    AS PROVEEDOR
   ,F.REFERENCIA_DEL_PROVEEDOR
   ,D.MARCA
   ,D.TIPO
...
LEFT JOIN (SELECT
  ...
  ON ARTICULOS.NOMBRE_EMPRESA = DEVOLUCIONES.EMPRESA
    ...
    AND ARTICULOS.ID_ARTICULO = DEVOLUCIONES.ID_ARTICULO
    AND ARTICULOS.MARCA = DEVOLUCIONES.MARCA
    AND ARTICULOS.TIPO = DEVOLUCIONES.TIPO
ORDER BY ARTICULOS.NOMBRE_EMPRESA, ARTICULOS.NOMBRE, ARTICULOS.DESCRIPCION