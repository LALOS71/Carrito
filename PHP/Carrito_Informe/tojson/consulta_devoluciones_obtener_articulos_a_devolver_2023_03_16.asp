...
<%
	...
	'response.write("<br>nombre establecimiento: " & nombre_establecimiento)
		...
		cadena_sql= cadena_sql & "SELECT B.CODCLI AS CLIENTE, A.ID_ARTICULO, C.CODIGO_SAP, C.DESCRIPCION"
		...
		cadena_sql= cadena_sql & " AND CONVERT(VARCHAR(8), GETDATE(), 112) <= CONVERT(VARCHAR(8), DATEADD(day, 20, D.FECHA), 112)" 'convierte las fechas a formato yyyymmdd

		cadena_sql= cadena_sql & " UNION"
		
		cadena_sql= cadena_sql & " SELECT C.CODCLI AS CLIENTE, A.ARTICULO AS ID_ARTICULO, D.CODIGO_SAP, D.DESCRIPCION"
		...
		cadena_sql= cadena_sql & " AND CONVERT(VARCHAR(8), GETDATE(), 112) <= CONVERT(VARCHAR(8), DATEADD(day, 20, E.FECHA), 112)" 'convierte las fechas a formato yyyymmdd
