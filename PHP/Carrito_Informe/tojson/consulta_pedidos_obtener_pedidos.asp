...
<%
	...
	'response.write("<br>nombre establecimiento: " & nombre_establecimiento)
		...
		cadena_sql= cadena_sql & ", round( (isnull(total,0 ) + isnull(gastos_envio,0)  - isnull(total_devoluciones,0)) * 0.21, 2)  as TotIva"
		'cadena_sql= cadena_sql & ", round( (isnull(total,0 ) + isnull(gastos_envio,0) - isnull(total_devoluciones,0)), 2)  as newtotalfinal"
		cadena_sql= cadena_sql & ", round( ((isnull(total,0 ) + isnull(gastos_envio,0) - isnull(TOTAL_SALDOS,0) - isnull(total_devoluciones,0)) + ((isnull(total,0 ) + isnull(gastos_envio,0) - isnull(total_devoluciones,0)) * 0.21)), 2)  as TotalEnvio"
        ...
        cadena_sql= cadena_sql & " LEFT JOIN (SELECT ID_Pedido, sum(total) Total, Sum(1) NReg FROM  Pedidos_Detalles"
