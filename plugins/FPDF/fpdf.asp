...
function FPDF()
	...
	//variable nueva para la ruta del fichero
	...
	//le creo una cabecera
	/*
	this.Header=function Header(){
		// Logo
	    this.Image("Logo_GAG.jpg",10,8,33);
    	// Arial bold 15
	...
	*/
	...
	this.SetRelativeTempPath=function SetRelativeTempPath(xRelativeTempPath) {this.RelativeTempPath=xRelativeTempPath;}
	...
	this.Output=function Output(xfile , xdownload , Overwrite)
		...
		if(xfile!=""){ // <<< alteração aqui 
			if(xdownload){
					...
					{
						//no quiero que se muestre el pdf, solo quiero que se grabe, y sin mostrar
			...
			if (!this.hasBinary){
				...
				if (this.RelativeTempPath == '')
					{
					//xfile=Server.MapPath(lib.fso.GetTempName())
					//Response.Write("<br>ruta para el mappath: " + xfile)
					xfile=Server.MapPath(xfile)
					}
				else
					{
					//xfile=Server.MapPath(this.RelativeTempPath + "/" + lib.fso.GetTempName())
					//Response.Write("<br>ruta para el mappath: " + this.RelativeTempPath + "/" + xfile)
					xfile=Server.MapPath(this.RelativeTempPath + "/" + xfile)
					}
				...
				//no muestro por pantalla el contenido del pdf, solo quiero que se grabe el pdf en el servidor
