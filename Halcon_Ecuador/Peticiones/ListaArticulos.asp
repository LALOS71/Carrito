...
<form id="frmArticulos" name="frmArticulos" method="post" >
  <div align="center">
			...
			<table align="center" width="90%"  border="1" cellspacing="0" cellpadding="0">
  				<tr>
					...
					<font color="#FFFFFF" size="4">
						...
						debéis solicitarlo directamente a LVEB: <a href="mailto:soportedeventas@smartbox.es">soportedeventas@smartbox.es</a> (Consultar posibles gastos de envío)
	...
	<%if codigofamilia=46 then%>
			<br>&nbsp;
			<table align="center" width="90%"  border="1" cellspacing="0" cellpadding="0">
  				<tr>
    				<td bgcolor="#880000" align="center">
					<font color="#FFFFFF" size="4">
						En el caso de necesitar un cofre de una temática no disponible en la Distribuidora, debéis solicitarlo directamente a SMB al mail <a href="mailto:soportedeventas@smartbox.es">soportedeventas@smartbox.es </a>(Consultar posibles gastos de envío)
      ...
      <tr> 
        <td width="10%"> 
          <div align="center"><a href="anadir.asp?id=<%=articulos("cod")%>&codfamilia=<%=codigofamilia%>&nomfamilia=<%=nombrefamilia%>&codsucursal=<%=codigosucursal%>&logo=<%=logotipo_empresa%>&codigo_empresa=<%=codigo_empresa%>" target="_parent"><img src="../Imagenes_Articulos/<%=articulos("cod")%>.jpg" width="48" height="67" border="0"> 
			...
			<%
				'response.write("<br>codigo: " & articulos("cod"))
				if articulos("cod")=1798 or articulos("cod")=5448 then
					presente_set_maletas="SI"
				end if
				'response.write("<br>set maletas: " & presente_set_maletas)
			%>
			
        </td>
        <!-- <td><img src="Images/1012.jpg" width="100" height="150"></td>  -->
        <td width="40%">
			<a class="nosub" href="anadir.asp?id=<%=articulos("cod")%>&codfamilia=<%=codigofamilia%>&nomfamilia=<%=nombrefamilia%>&codsucursal=<%=codigosucursal%>&logo=<%=logotipo_empresa%>&codigo_empresa=<%=codigo_empresa%>" target="_parent"><font size="2"><b><%=articulos("descripcion")%></b></font></a>
			<br>
       		<%if articulos("cod")="5586" then%>
					<div align="right"><a href="Documentos/Bono_Regalo_Smartbox_NOVIOS_HV.pdf" target="_blank">Imprimir Bono</a></div>
			<%end if%>
		
			<%if articulos("cod")="5587" then%>
					<div align="right"><a href="Documentos/Bono_Regalo_Smartbox_NOVIOS_VE.pdf" target="_blank">Imprimir Bono</a></div>
			<%end if%>
			<%if articulos("cod")="5598" then%>
					<div align="right"><a href="Documentos/Bono_Maletas_Mayores_55_VE.pdf" target="_blank">Imprimir Bono</a></div>
			<%end if%>
			<%if articulos("cod")="5599" then%>
					<div align="right"><a href="Documentos/Bono_Maletas_Mayores_55_HV.pdf" target="_blank">Imprimir Bono</a></div>
			<%end if%>
			<%if articulos("cod")="5695" then%>
					<div align="right"><a href="Documentos/Bono_Maletas_Senior_HV.pdf" target="_blank">Imprimir Bono</a></div>
			<%end if%>
			<%if articulos("cod")="5696" then%>
					<div align="right"><a href="Documentos/Bono_Maletas_Senior_VE.pdf" target="_blank">Imprimir Bono</a></div>
			<%end if%>
			<%if articulos("cod")="5697" then%>
					<div align="right"><a href="Documentos/Bono_Maletas_Circuitos_Culturales_HV.pdf" target="_blank">Imprimir Bono</a></div>
			<%end if%>
			<%if articulos("cod")="5698" then%>
					<div align="right"><a href="Documentos/Bono_Maletas_Circuitos_Culturales_VE.pdf" target="_blank">Imprimir Bono</a></div>
			<%end if%>
		</td>
        <%mostrarliteral1=1%>
        <%articulos.movenext%>
        ...
        <!-- <td><img src="Images/1012.jpg" width="100" height="150"></td>  -->
        <td width="40%">
			<a class="nosub" href="anadir.asp?id=<%=articulos("cod")%>&codfamilia=<%=codigofamilia%>&nomfamilia=<%=nombrefamilia%>&codsucursal=<%=codigosucursal%>&logo=<%=logotipo_empresa%>&codigo_empresa=<%=codigo_empresa%>" target="_parent"><b><font size="2"><%=articulos("descripcion")%></font></b></a>
			<br>
       		<%if articulos("cod")="5586" then%>
					<div align="right"><a href="Documentos/Bono_Regalo_Smartbox_NOVIOS_HV.pdf" target="_blank">Imprimir Bono</a></div>
			<%end if%>
	...
	<%if presente_set_maletas="SI" then%>
		<br><br><br>
			<table align="center" width="90%"  border="1" cellspacing="0" cellpadding="0">
  				<tr>
    				<td align="center"><font color="red" size="5"><b>NOTA IMPORTANTE: S&Oacute;LO SOLICITAR MALETAS PARA LA CAMPAÑA DE NOVIOS.</b></font></td>
  				</tr>
			</table>
