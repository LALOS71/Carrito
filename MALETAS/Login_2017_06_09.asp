...
<html>
  ...
  <!-- FIN capa mensajes -->
    ...
    <div class="container">
      ...
      <form class="form-signin" method="post" id="frmlogin" name="frmlogin" action="Validar.asp">
		...
		j$.ajax({
			...
			success:
				function (data) {
					...
					switch (valores[0]) {                     
							case '0': {             
									alert('usuario del dominio.... entramos a validar en la aplicacion')     
									validando_acceso()                                                                                          
									//j$("frmlogin").submit();
									break;
									}
...
function validando_acceso()
	{	
		console.log('entrando en validar acceso')
		...
		j$.ajax({
			...
			success:
				function (data) {
					...
					console.log('valor devuelto: ' + data)
					switch (data) {                     
							case '0': { //el usuario no esta dado de alta en la aplicacion            
									cadena='El Usuario no Está Dado de Alta Para Poder Acceder a Esta Aplicación.'                                                                                               
									//j$("frmlogin").submit();
									break;
									}
									
							case '1': //usuario encontrado dado de alta en la aplicacion
									{
									alert('accediendo')
									break;
									}
