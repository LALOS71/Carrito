...
<%
	...
	fecha_fin_entrega = "" & request.QueryString("p_fecha_fin_entrega")
	fecha_inicio_facturacion = "" & request.QueryString("p_fecha_inicio_facturacion")
	fecha_fin_facturacion = "" & request.QueryString("p_fecha_fin_facturacion")
	
	ocultoperfil_usuario = "" & request.QueryString("p_ocultoperfil_usuario")
	ocultousuario = "" & request.QueryString("p_ocultousuario")
	ocultoproveedor_usuario = "" & request.QueryString("p_ocultoproveedor_usuario")
	...
	'response.write("<br>nombre establecimiento: " & nombre_establecimiento)
		...
		cadena_sql=cadena_sql & ", B.DESCRIPCION AS TIPO_BAG_ENTREGADA, PIRS.FECHA_ENVIO" 
		...
		cadena_sql=cadena_sql & ", E.INCIDENCIA AS ULTIMA_INCIDENCIA, ESTADOS.DESCRIPCION AS ESTADO_DESCRIPCION,"
		...
		cadena_sql=cadena_sql & " PIRS.COLOR_BAG_ENTREGADA, PIRS.OBSERVACIONES_PROVEEDOR"
		cadena_sql=cadena_sql & " ,PIRS.NOMBRE, PIRS.APELLIDOS, PIRS.DIRECCION_ENTREGA, PIRS.CP_ENTREGA, PIRS.MOVIL"
		cadena_sql=cadena_sql & " ,PIRS.FIJO, PIRS.REFERENCIA_BAG_ENTREGADA"
		cadena_sql=cadena_sql & ", C.DESCRIPCION AS TIPO_BAG_ORIGINAL"

		cadena_sql=cadena_sql & " ,PIRS.TIPO_DIRECCION_ENTREGA, PIRS.DESDE_HASTA, PIRS.FECHA_DESDE_HASTA, PIRS.MARCAWT, PIRS.EMAIL"
		
		cadena_sql=cadena_sql & " FROM PIRS LEFT JOIN ESTADOS"
		cadena_sql=cadena_sql & " ON PIRS.ESTADO = ESTADOS.ID"
		cadena_sql=cadena_sql & " INNER JOIN TIPOS_MALETA B ON B.ID= PIRS.TIPO_BAG_ENTREGADA"
		cadena_sql=cadena_sql & " INNER JOIN TIPOS_MALETA C ON C.CODIGO=PIRS.TIPO_BAG_ORIGINAL"
		cadena_sql=cadena_sql & " LEFT JOIN"
		cadena_sql=cadena_sql & " (SELECT ID_PIR, INCIDENCIA, NUMERITO FROM"
		cadena_sql=cadena_sql & " (SELECT ID_PIR, CONVERT(nvarchar(10), HISTORICO_PIRS.FECHA, 103) + '#||#' +"
		cadena_sql=cadena_sql & " CONVERT(nvarchar(8), HISTORICO_PIRS.FECHA, 108) + '#||#' + CONVERT(nvarchar(250),DESCRIPCION) AS INCIDENCIA"
		cadena_sql=cadena_sql & " , ROW_NUMBER() OVER (PARTITION BY ID_PIR ORDER BY FECHA DESC) AS NUMERITO"
		cadena_sql=cadena_sql & " FROM HISTORICO_PIRS"
		cadena_sql=cadena_sql & " WHERE ACCION='INCIDENCIA'"
		cadena_sql=cadena_sql & " ) D"
		cadena_sql=cadena_sql & " WHERE D.NUMERITO=1) E ON E.ID_PIR=PIRS.ID"


		cadena_sql=cadena_sql & " WHERE 1=1"

		if ocultoperfil_usuario="PROVEEDOR" then
			cadena_sql=cadena_sql & " AND PIRS.PROVEEDOR=" & ocultoproveedor_usuario
		end if
		
		if pir<>"" then
			'cadena_sql=cadena_sql & " AND UPPER(PIRS.PIR)='" & UCASE(pir) & "'"
			cadena_sql=cadena_sql & " AND UPPER(PIRS.PIR) LIKE '%" & UCASE(pir) & "%'"
		end if
		if estado<>"" then
		...
		if fecha_inicio_facturacion<>"" then
			cadena_sql=cadena_sql & " AND PIRS.FECHA_FACTURACION>='" & cdate(fecha_inicio_facturacion) & "'"
		end if
		if fecha_fin_facturacion<>"" then
			cadena_sql=cadena_sql & " AND PIRS.FECHA_FACTURACION<='" & cdate(fecha_fin_facturacion) & "'"
		end if
