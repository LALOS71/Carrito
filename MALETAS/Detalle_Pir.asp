...
<%
if session("perfil_usuario")="" then
	Response.Write("<script>top.location='Login.asp';parent.location='Login.asp';</script>")
end if

if session("perfil_usuario")="PROVEEDOR" then
	proveedor=session("proveedor_usuario")
  else
...
if id_seleccionado<>"" then
	set detalle_pir=Server.CreateObject("ADODB.Recordset")
		with detalle_pir
			...
			.Source= .Source & ", TIPO_EQUIPAJE_BAG_ORIGINAL, MARCA_BAG_ORIGINAL, MARCAWT,  MATERIAL_BAG_ORIGINAL"
	...
	if not detalle_pir.eof then
		...
		if detalle_pir("marcawt")<>"" then
			campo_marcawt="" & replace(detalle_pir("marcawt"),"""","&quot;")
		end if
					...
					<div class="row">
						<div class="col-sm-12 col-md-12 col-lg-12">
                          <div class="form-group row">
							...
							<div class="col-sm-2 col-md-2 col-lg-2">
								<label for="txtcaseid_d" class="control-label">CASE ID</label>
								<input type="text" id="txtcaseid_d" class="form-control" required="" name="txtcaseid_d" value="<%=campo_caseid%>"/> 
							</div>
                          </div>
						</div>
						...
						<div class="col-sm-12 col-md-12 col-lg-12">  
						  <div class="form-group row">
							...
							<div class="col-sm-2 col-md-2 col-lg-2">
								<label for="txtmarcawt_d" class="control-label">Marca WT</label>
								<input type="text" id="txtmarcawt_d" class="form-control" required="" name="txtmarcawt_d" value="<%=campo_marcawt%>" /> 
							</div>
                          </div>
						</div>						  
				...
				<div class="row">
					...
					<div class="form-group row">
						...
						<div class="col-sm-8 col-md-8 col-lg-8">
							<div class="col-sm-12 col-md-12 col-lg-12">
									<label for="txtbuscar_referencias_d" class="control-label">Buscar Referencia / Tipo Maleta / Tamaño / Color</label>
									<div class="typeahead__container">
										<div class="typeahead__field">
											<div class="typeahead__query">
												<input class="js-typeahead-referencia form-control" name="txtbuscar_referencias_d" id="txtbuscar_referencias_d" type="search" placeholder="Buscar Referencia / Tipo Maleta / Tamaño / Color" autocomplete="off">
											</div>
										</div>
									</div>
							</div>
							...
							<div class="col-sm-12 col-md-12 col-lg-12">
								<div class="col-sm-3 col-md-3 col-lg-3">
									Referencia
									<input type="text" id="txtreferencia_maleta_entregada_d" class="form-control" required="" name="txtreferencia_maleta_entregada_d" value="<%=campo_referencia_bag_entregada%>" /> 
								</div>
								<div class="col-sm-3 col-md-3 col-lg-3">
									<span for="cmbtipo_maleta_entregada_d"
										data-toggle="popover" 
										data-placement="top" 
										data-trigger="hover"
								...
								<div class="col-sm-3 col-md-3 col-lg-3">
									<span for="cmbtamanno_maleta_entregada_d" class="control-label">Tamaño</span>
									<div class="clearfix visible-md-block"></div>
				...
				<div class="row">
					<div class="col-sm-12 col-md-12 col-lg-12">
					  <div class="form-group row">
				  		...
				  		<div class="col-sm-2 col-md-2 col-lg-2">
								<label for="txtnumero_expedicion_d" class="control-label">N&uacute;m Expedici&oacute;n</label>
...
j$(document).ready(function () {
	...
	//j$("#txtreferencia_maleta_entregada_d").val('<%=campo_referencia_bag_entregada%>');
	...
	j$.typeahead({
		...
		callback: {
			onClick: function (node, a, item, event) {
				...
				j$("#txtreferencia_maleta_entregada_d").val(item.REFERENCIA)
				j$("#cmbtipo_maleta_entregada_d").val(item.CODIGO_TIPO_MALETA)
	...
	<%if session("perfil_usuario")="PROVEEDOR" then%>
		...
		j$("#txtcaseid_d").prop('disabled',true);
		...
		*/
		...
		j$("#txtmarcawt_d").prop('disabled',true);
...
j$('#cmbestado_d').on('change', function(){
	...
	if (opcion_seleccionada=='6') //ENTREGADO
		{
		<%'si es globalia artes graficas pone por defecto como coste 37,5€
		if session("perfil_usuario")="PROVEEDOR" and proveedor="1" then%>
			if (j$("#txtcostes_d").val()=="")
				{
				j$("#txtcostes_d").val('37,5')
				}
		<%end if%>
		}
...
j$('#cmdguardar_pir').on('click', function() {
	...
	if (j$("#ocultoid_pir").val()=='')
		...
		if (j$("#txtmarcawt_d").val()=='')
			{
			hay_error=hay_error + '- Al Dar De Alta El PIR, Ha de Introducir la Marca WT de la Maleta.<br>'
			}
	...
	if (j$("#cmbestado_d").val()=='5') //ENVIADO
		...
		if (j$("#txtreferencia_maleta_entregada_d").val()=='')
			{
