										...
										<%if vacio_tipos_maleta=false then %>
											<%for i=0 to UBound(mitabla_tipos_maleta,2)%>
												<tr id="fila_tipos_maleta_<%=i%>" style="cursor:hand;cursor:pointer" row_id="fila_tipos_maleta_<%=i%>">
													...
													<td align="center">
														<div class="row_data row_data_tipos_maleta" edit_type="click" col_name="borrado_tipos_maleta">
															<select class="form-control cmbborrado_tipos_maleta" style="display:none">
																	<option value="SI">SI</option>
																	<option value="NO">NO</option>
															</select>
															<div class="row_data row_data_tipos_maleta_oculto" edit_type="click" col_name="borrado_tipos_maleta_oculto"><%=mitabla_tipos_maleta(CAMPO_BORRADO_TIPOS_MALETA,i)%></div>
														</div>
													</td>
													<td>
...
j$(document).on('click', '.btn_add_tipos_maleta', function(event) 
	{
		...
		cadena = '<tr id="fila_tipos_maleta_x" style="cursor:hand;cursor:pointer" row_id="fila_tipos_maleta_x">'
		...
		cadena = cadena + '<input type="hidden" name="oculto_id_x" id="oculto_id_x" value="" />'
		cadena = cadena + '</td>'
		cadena = cadena + '<td>'
		...
		cadena = cadena + '<select class="form-control cmbborrado_tipos_maleta">'
		cadena = cadena + '<option value="SI">SI</option>'
		cadena = cadena + '<option value="NO">NO</option>'
		cadena = cadena + '</select>'
		cadena = cadena + '<div class="row_data row_data_tipos_maleta_oculto" edit_type="click" col_name="borrado_tipos_maleta_oculto" style="display:none"></div>'
		cadena = cadena + '</div>'
		cadena = cadena + '</td>'
		cadena = cadena + '<td>'
		cadena = cadena + '<div style="text-align:center">'
		cadena = cadena + '<i class="fas fa-pencil-alt fa-lg btn_edit_tipos_maleta_x" row_id="fila_tipos_maleta_x" style="color:blue;cursor:hand;cursor:pointer"'
		cadena = cadena + 'data-toggle="popover" data-placement="top" data-trigger="hover" data-content="Editar el Tipo de Maleta"></i>&nbsp;'
		cadena = cadena + '<i class="fas fa-save fa-lg btn_save_tipos_maleta_x" row_id="fila_tipos_maleta_x" style="color:darkblue;display:none;cursor:hand;cursor:pointer"'
		cadena = cadena + 'data-toggle="popover" data-placement="top" data-trigger="hover" data-content="Guardar El Tipo de Maleta"></i>&nbsp;'
		cadena = cadena + '<i class="fas fa-times fa-lg btn_cancel_tipos_maleta_x" row_id="fila_tipos_maleta_x" style="color:red;display:none;cursor:hand;cursor:pointer"'
		...
		//j$(tabla).find('tbody:first-child').append(cadena);
		
		j$("#fila_tipos_maleta_x").find('.btn_edit_tipos_maleta_x').click()
...
j$(document).on('click', '.btn_edit_tipos_maleta', function(event) 
	{
		...
		if (j$(".row_data.bg-warning").length > 0)
			...
			tbl_row.find('.cmbborrado_tipos_maleta').val(tbl_row.find('.row_data_tipos_maleta_oculto').html())
			tbl_row.find('.cmbborrado_tipos_maleta').show()
			tbl_row.find('.row_data_tipos_maleta_oculto').hide()
	...
	//--->button > edit > start	
j$(document).on('click', '.btn_edit_tipos_maleta_x', function(event) 
	{
		//event.preventDefault();
		...
		if (j$(".row_data.bg-warning").length > 0)
			...
			tbl_row.find('.btn_save_tipos_maleta_x').show();
			tbl_row.find('.btn_cancel_tipos_maleta_x').show();
			tbl_row.find('.btn_delete_tipos_maleta_x').show();
		
			//hide edit button
			tbl_row.find('.btn_edit_tipos_maleta_x').hide(); 
...
j$(document).on('click', '.btn_cancel_tipos_maleta', function(event) 
	{
		...
		tbl_row.find('.cmbborrado_tipos_maleta').hide()
		tbl_row.find('.row_data_tipos_maleta_oculto').show()
...
j$(document).on('click', '.btn_cancel_tipos_maleta_x', function(event) 
	{
		event.preventDefault();

		j$("#fila_tipos_maleta_x").remove()
		j$("#tabla_tipos_maleta").find('.btn_add_tipos_maleta').show();
	});
	//--->button > cancel > end
...
//--->button > delete > start	
j$(document).on('click', '.btn_delete_tipos_maleta', function(event) 
	{
		event.preventDefault();
		...
		window.parent.bootbox.confirm({
			message: "¿Está seguro que desea borrar este Tipo de Maleta?",
			buttons: {
				confirm: {
					label: 'Si',
