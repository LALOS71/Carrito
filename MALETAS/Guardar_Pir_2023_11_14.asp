...
<%
		...
		if id_seleccionado<>"" then 'ES UNA MODIFICACION
			...
			'*****************************************************************************************
			'--------------------------------------------------------
			' SECCION DONDE INFORMAMOS DEL CAMBIO DE ESTADO A INDIANA
				...
				if access_token <> "" then
					...
					If xmlHttpLogin.Status = 200 Then
						...
						if CheckProperty(Info_Indiana, "pyStatusMessage") Then
							txt= "" & Info_Indiana.pyStatusMessage
						  else
						  	txt = "" & xmlHttpLogin.responseText
						end if
			...
			end if ' del control del combo estado donde se ve si se ha de mandar a indiana un aviso
...
<BODY onload="validar__('<%=mensaje_aviso%>','<%=id_seleccionado%>')">
