...
<%
		...
		if id_seleccionado<>"" then 'ES UNA MODIFICACION
			...
			if campo_estado<>request.form("cmbestado_d") then		
				...
				'*************************************************
				'en cada cambio de estado tenemos que informar a indiana
				...
				parametros = "key=C59ABE15811E20AA1EC304E6CDE9945B_" & _
								"&codexp=" & campo_expediente & _
								"&tag=" & campo_tag & _
								"&time=" & fecha_accion & _
								"&status=" & request.form("cmbestado_d") & _
								"&info=PROBANDO"
				...
				For x = 0 to oxml.childNodes.length 
					...
					'Response.write(" ...nodename: " & objnodos(x).nodeName)
					'Response.write(" ...contenido: " & objnodos(x).text)
					'Response.write(" ...tipo: " & objnodos(x).nodeType)
