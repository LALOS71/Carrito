...
<!-- capa TIPOs INCIDENCIA -->
<div class="modal fade" id="capa_tipos_incidencia">	
	<div class="modal-dialog modal-lg">	  
    	<div class="modal-content">	    
        	<div class="modal-header">	      
				<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>     
          		<h4 class="modal-title" id="cabecera_tipos_incidencia"></h4>	    
        	</div>	    
        	<div class="modal-body">
				<div class="form-group row">
					<div class="col-sm-12 col-md-12 col-lg-12">
						...
						<div class="col-sm-2 col-md-2 col-lg-2">
							<button type="button" class="btn btn-primary btn-md btn-block" id="cmdaceptar_tipo_incidencia" name="cmdaceptar_tipo_incidencia">
							  <span class="glyphicon glyphicon-ok" aria-hidden="true"></span> Aceptar
							</button>
						</div>
						<div class="col-sm-2 col-md-2 col-lg-2">
							<button type="button" class="btn btn-warning btn-md btn-block" id="cmdcancelar_tipo_incidencia" name="cmdcancelar_tipo_incidencia">
...
j$(document).ready(function () {
    ...
    */                                               
	...
	lst_historico_pir.column(3).search('INCIDENCIA').draw();
...
j$('#cmbestado_d').on('change', function(){
	...
	if (opcion_seleccionada=='INCIDENCIA')
		...
		j$("#capa_tipos_incidencia").modal("show");
	  	...
	  	{
		j$("#cmbestado_d").attr("val_ant", opcion_seleccionada);
...
obtener_valor_actual = function(){
	j$("#cmbestado_d").attr("val_ant", j$("#cmbestado_d").val());
	console.log('valor actual en cmbestado en onfocus: ' + j$("#cmbestado_d").val())
};

j$('.dropdown-toggle').focus(function(){
	j$("#cmbestado_d").attr("val_ant", j$("#cmbestado_d").val());
	console.log('valor actual en cmbestado en onfocus: ' + j$("#cmbestado_d").val())
...
j$('#cmdaceptar_tipo_incidencia').on('click', function() {
	console.log('entrando en el clic de cmdaceptar tipo incidencia')
	console.log('valor del combo tipos_incidencia: ' + j$("#cmbtipos_incidencia").val())

	if (j$("#cmbtipos_incidencia_d").val()=='')
		{
		j$("#cabecera_pantalla_avisos").html("<h3>Errores</h3>")
		j$("#body_avisos").html('<br><h4>Se Ha De Seleccionar un Tipo de Incidencia</h4><br>');
		j$("#pantalla_avisos").modal("show");
		}
	  else
	  	{
		j$("#capa_tipos_incidencia").modal("hide");	
		}
...
j$('#cmdcancelar_tipo_incidencia').on('click', function() {

	j$("#cmbtipos_incidencia_d").val('').change()
	console.log('valor tipos incidencia: ' + 	j$("#cmbtipos_incidencia_d").val())
	console.log('valor cmbestado antes de cancelar: ' + j$("#cmbestado_d").val())
	console.log('valor antiguo cmbestado antes de cancelar: ' + j$("#cmbestado_d").attr("val_ant"))
	
	
	j$("#cmbestado_d").val(j$("#cmbestado_d").attr("val_ant")).change()
	console.log('valor cmbestado despues de cancelar: ' + j$("#cmbestado_d").val())
	j$("#capa_tipos_incidencia").modal("hide");	
...
/*
j$('#capa_tipos_incidencia').on('hide.bs.modal', function(e) {
...
//j$("#botones_historico > .btn").click(function(){
	...
	if (boton_activo=='Todo')
		...
		lst_historico_pir.column(3).search('').draw();
		}
	...
	if (boton_activo=='Histórico')
		...
		// ..... /^(?:(?!PATTERN).)*$/ ... para todas
		lst_historico_pir.column(3).search('^(?!INCIDENCIA$).*$', true, true, false).draw();
		}
