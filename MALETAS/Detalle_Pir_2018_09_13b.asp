							...
							<select id="cmbestado_d" name="cmbestado_d" data-width="100%" class="cmb_bt">
							  ...
							  <%if not vacio_estados then%>
									<%for i=0 to UBound(tabla_estados,2)
										'los proveedores no ven la opcion de PENDIENTE DE AUTORIZACION ni CERRADO, ni en GESTION CIA
											if session("perfil_usuario")="PROVEEDOR" and (tabla_estados(campo_id_estados,i)=1 or tabla_estados(campo_id_estados,i)=7 or tabla_estados(campo_id_estados,i)=8)then
												if tabla_estados(campo_id_estados,i)=7 or tabla_estados(campo_id_estados,i)=8 then%>
...
j$(document).ready(function () {
	...
	<%if session("perfil_usuario")="PROVEEDOR" then%>
		j$("#txtpir_d").attr('disabled','disabled');
		
		j$("#txtfecha_pir_d").prop("type", "text");
		j$("#txtfecha_pir_d").attr('disabled','disabled');
		
		j$("#txtfecha_orden_d").prop("type", "text");
		j$("#txtfecha_orden_d").attr('disabled','disabled');
		
		j$("#txttag_d").attr('disabled','disabled');
		j$("#txtruta_d").attr('disabled','disabled');
		j$("#txtvuelos_d").attr('disabled','disabled');
	
		j$("#txtnombre_d").attr('disabled','disabled');
		j$("#txtapellidos_d").attr('disabled','disabled');
		j$("#txtmovil_d").attr('disabled','disabled');
		j$("#txtfijo_d").attr('disabled','disabled');
		
		j$("#txtdireccion_entrega_d").attr('disabled','disabled');
		j$("#txtcp_entrega_d").attr('disabled','disabled');
		j$("#txtemail_d").attr('disabled','disabled');
		
		j$("#cmbtipo_direccion_entrega_d").attr('disabled','disabled');
		j$("#cmbdesde_hasta_d").attr('disabled','disabled');
		j$("#txtfecha_desde_hasta_d").prop("type", "text");
		j$("#txtfecha_desde_hasta_d").attr('disabled','disabled');
		j$("#txttipo_equipaje_bag_original_d").attr('disabled','disabled');
		j$("#txtmarca_bag_original_d").attr('disabled','disabled');
		
		j$("#txtmaterial_bag_original_d").attr('disabled','disabled');
		j$("#txtcolor_bag_original_d").attr('disabled','disabled');
		j$("#txtlargo_bag_original_d").attr('disabled','disabled');
		j$("#txtalto_bag_original_d").attr('disabled','disabled');
		j$("#txtancho_bag_original_d").attr('disabled','disabled');
		
		console.log('combo estado: ' + j$("#cmbestado_d").val()) 
		 // PENDIENTE AUTORIZACION, CERRADO o GESTION CIA
		 if (j$("#cmbestado_d").val()=='1' || j$("#cmbestado_d").val()=='7' || j$("#cmbestado_d").val()=='8')
			{
			j$("#cmbestado_d").attr('disabled','disabled');
			j$("#cmdguardar_pir").attr('disabled','disabled');
...
j$('#cmdguardar_pir').on('click', function() {
	...
	if (j$("#cmbestado_d").val()=='6') //ENTREGADO
		...
		if (j$("#cmbtipo_maleta_entregada_d").val()=='')
			{
			hay_error=hay_error + '- Antes de pasar a Entegado hay que Seleccionar el Tipo de Maleta Entregada.<br>'
			}
		}
