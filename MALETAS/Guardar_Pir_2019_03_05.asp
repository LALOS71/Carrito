...
<%
		...
		if id_seleccionado<>"" then 'ES UNA MODIFICACION
			...
			with detalle_pir
				...
				.Source= .Source & ", REFERENCIA_BAG_ENTREGADA, COLOR_BAG_ENTREGADA, NUM_EXPEDICION, COSTES, OBSERVACIONES_PROVEEDOR"
				.Source= .Source & ", ESTADO, PROVEEDOR"
			...
			'*************************************************
			'en cada cambio de estado o incidencia tenemos que informar a indiana
			'Le url para mandar los cambios de estados es la siguiente:
			'POST https://mylostbag.aireuropa.com/weblf/rest/dpr/order/status
			'POST http://pre.mylostbag.aireuropa.com/weblf/rest/dpr/order/status
			'Los parámetros a enviar son: 
			'codexp : Código del expediente .........campo_expediente (LISUX12138D20180612)
			'tag : "tag number" de la maleta afectada ..... campo_tag (UX733028)
			'time: fecha/hora en formato dd/mm/yyyy hh:mi:ss en que se produce el cambio
			if campo_estado<>request.form("cmbestado_d") or (request.form("cmbtipos_incidencia_d")<>"" and  request.form("cmbestado_d")="9") then
				...
				'status: Nuevo estado .... request.form("cmbestado_d")
				'info: información adicional del estado
				'key: key API de acceso al servicio la misma que para las otras llamadas... C59ABE15811E20AA1EC304E6CDE9945B
			
				'Estará disponible en producción el próximo miércoles.
				'Si se acepta en nuevo estado se recibirá <OK/> en caso contrario un <error cod="999">texto del error</error>
				'Un saludo,
				descripcion_estado=""
				...
				'parametros=Server.URLEncode(parametros)
				...
				cadena_historico=cadena_historico & "USUARIO, DESCRIPCION, TIPO_INCIDENCIA)"
				cadena_historico=cadena_historico & " VALUES (" & id_seleccionado & ", '" & request.form("txtpir_d") & "',"
				cadena_historico=cadena_historico & " CAST('" & fecha_accion & "' as datetime), 'INDIANA', NULL, NULL,"
				cadena_historico=cadena_historico & " '" & resultado_indiana & "', '" & session("usuario") & "', 'Respuesta de Indiana al Cambio', NULL)"
