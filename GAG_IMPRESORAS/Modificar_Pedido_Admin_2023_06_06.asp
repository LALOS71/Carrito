...
<%
	...
	RUTA_SERVIDOR_GAG="[192.168.156.175\SERVERSQL].GAG_PRU.dbo."
	
	'cuando usamos connimprenta para hacer la consulta pero usamos la base de datos REAL... GAG
	'RUTA_SERVIDOR_GAG="[192.168.156.175\SERVERSQL].GAG.dbo."
	...
	Next
		...
		if acciones="ALBARAN" then
			...
			if creamos_albaran="SI" then
				   ...
				   response.write("<br><BR>27---------------------<BR>CONNGAG -- BEGINTRANS")
				   'conn_gag.BeginTrans 'Comenzamos la Transaccion
				   'cmd.CommandText = "SP_ACTUALIZA_ALBARAN"
				   cmd.CommandText = RUTA_SERVIDOR_GAG & "sp_Actualiza_Albaran"
				   cmd.CommandType = 4 'adCmdStoredProc
				   
				   'EL PROCEDIMIENTO ALMACENADO HACE ESTE INSERT
				   'INSERT INTO Albaranes (IdEmpresa, Ip_Mod, Fecha_Mod, IdCliente, IdFormaEnvio, DirEntrega, Anulado, IdEstado, NPedido,Fecha, Observaciones,  
				   'NoFacturable, FechaValija, 	PermitirEdicion, Origen) 
					'VALUES ( 1, @IP, GETDATE(), @CLIENTE, @FormaEnvio, @DirEntrega, @Anulado, @Estado, @Pedido, @Fecha, @Observaciones,  @NoFacturable,  null, 1, 1	)	
		
				
					'creo los parametros del procedimiento almacenado
					'cmd.parameters.append cmd.createparameter("ALBARAN",adInteger,adParamInput,4,cint(codsucursal))
					...
					RESPONSE.WRITE("<BR>-11- empiezo a asignar parametros del albaran a crear")
					response.write("<br>comando: " & cmd.commandtext)
					response.write("<br>numero albaran: " & numero_albaran)
					'Paso los parametros para que se ejecute el comando
					'cmd.parameters(1)=numero_albaran
					'cmd.parameters.append cmd.createparameter("Albaran",adInteger,adParamInput,4,numero_albaran)
					
					cmd.Parameters.Append cmd.CreateParameter("@Albaran", adInteger, adParamInput, , numero_albaran)
					'.Parameters.Append .CreateParameter("@IP", adVarChar, adParamInput, 20, valorIP)
					'.Parameters.Append .CreateParameter("@Cliente", adInteger, adParamInput, , valorCliente)
					'.Parameters.Append .CreateParameter("@FormaEnvio", adInteger, adParamInput, , valorFormaEnvio)
					'.Parameters.Append .CreateParameter("@DirEntrega", adVarWChar, adParamInput, 250, valorDirEntrega)
					'.Parameters.Append .CreateParameter("@Anulado", adInteger, adParamInput, , valorAnulado)
					'.Parameters.Append .CreateParameter("@Estado", adInteger, adParamInput, , valorEstado)
					'.Parameters.Append .CreateParameter("@Pedido", adInteger, adParamInput, , valorPedido)
					'.Parameters.Append .CreateParameter("@Fecha", adDate, adParamInput, , valorFecha)
					'.Parameters.Append .CreateParameter("@Observaciones", adLongVarChar, adParamInput, , valorObservaciones)
					'.Parameters.Append .CreateParameter("@NoFacturable", adInteger, adParamInput, , valorNoFacturable)
					...
					response.write("<br><BR>28 CONNGAG COMMAND SP_ACTUALIZA_ALBARAN")   
					cmd.execute()
							...
							if estado_detalle="ENVIADO" OR estado_detalle="ENVIADO AL PROVEEDOR" then
								if albaran_detalle=albaran_nuevo then
									if albaran_nuevo=albaran_parcial_detalle or albaran_parcial_detalle="" then
										...
										cadena_ejecucion_detalles_albaran="INSERT INTO " & RUTA_SERVIDOR_GAG & "ALBARANES_DETALLES (" & campos_albaranes & ")"
					...
					if cadena_articulos_pendientes<>"" then
						cadena_actualizacion_albaran="UPDATE " & RUTA_SERVIDOR_GAG & "ALBARANES"
					...
					'response.write("<br>----------------------<BR>CONNGAG -- COMMITTRANS")
