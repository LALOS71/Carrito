...
'****************************************************************************
	...
	if empresa_query="ATESA" then
		hotel_seleccionado=3717 '225 ATESA TERRASSA
		contrasenna_seleccionada="atesa.225"
		empresa_entrada=5
	end if
	...
	'OTRAS EMPRESAS DEL GRUPO GLOBALIA
	if empresa_query="GROUNDFORCE" then
		hotel_seleccionado=6617 'GROUNDFORCE AGP 2015, U.T.E.
		contrasenna_seleccionada="GROUNDFORCE_AGP"
		empresa_entrada=30
	end if
	
	if empresa_query="AIR_EUROPA" then
		hotel_seleccionado=5246 'AIR EUROPA GFF-GESTION FLOTA Y FUEL
		contrasenna_seleccionada="AIREUROPA_GFF"
		empresa_entrada=40
	end if
	
	if empresa_query="CALDERON" then
		hotel_seleccionado=5296 'GROUNDFORCE CARGO AEROP SONDIKA
		contrasenna_seleccionada="CALDERON_CBIO"
		empresa_entrada=50
	end if
	
	if empresa_query="TRAVELPLAN" then
		hotel_seleccionado=7322 'TRD - ODC RECEPTIVO CATALUÑA
		contrasenna_seleccionada="WELCOME_ODC"
		empresa_entrada=90
	end if
	
	if empresa_query="TUBILLETE" then
		hotel_seleccionado=3872 'TU BILLETE.COM
		contrasenna_seleccionada="TUBILLETE"
		empresa_entrada=100
	end if
	
	if empresa_query="GLOBALIA" then
		hotel_seleccionado=7709 'GLOBALIA - COE DIR. DESARROLLO EXPAN
		contrasenna_seleccionada="COE-DEI"
		empresa_entrada=110
	end if
	
	if empresa_query="GEOMOON" then
		hotel_seleccionado=7574 'I47 KORKULA TOURS
		contrasenna_seleccionada="geomooni47"
		empresa_entrada=130
	end if
	
	if empresa_query="CORPORATE" then
		hotel_seleccionado=8132 'O15-TARRAGONA - COMERCIAL
		contrasenna_seleccionada="gct.O15"
		empresa_entrada=170
	end if
	
	if empresa_query="MARSOL" then
		hotel_seleccionado=9104 'MARSOL OVIEDO
		contrasenna_seleccionada="MARSOL.OVIEDO"
		empresa_entrada=210
	end if
		
	if empresa_query="BE_LIVE" then
		hotel_seleccionado=6780 'BE LIVE SON ANTEM
		contrasenna_seleccionada="BELIVE_SONANTEM"
		empresa_entrada=2
	end if
