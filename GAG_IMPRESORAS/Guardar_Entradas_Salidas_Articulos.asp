...
<%
	...
	connimprenta.BeginTrans 'Comenzamos la Transaccion
		...
		if entrada_salida_seleccionada="ENTRADA" and tipo_seleccionado="APROVISIONAMIENTO" then
			...
			'en los aprovisionamientos que vienen con una hoja de ruta en vez de un albaran
			' tenemos que poner la hoja de ruta en estado de ENTRADA ALMACEN (10) para el programa de graphisoft
			'***********LO QUITO DE AQUI PORQUE DA ERROR DE TRANSACCION DISTRIBUIDA, lo caso fuera de la transaccion
			'cadena_ejecucion="UPDATE [192.168.156.175\SERVERSQL].GAG.dbo.GESTION_GRAPHISOFT_HOJAS_IMPORTADAS"
			'cadena_ejecucion=cadena_ejecucion & " SET ID_ESTADO=10"
			'cadena_ejecucion=cadena_ejecucion & " WHERE HOJA_DE_RUTA=" & albaran_seleccionado
			'connimprenta.Execute cadena_ejecucion,,adCmdText + adExecuteNoRecords
		end if
		...
		'lo saco de la transaccion porque me da error, no permite transacciones distribuidas entre servidores
		if entrada_salida_seleccionada="ENTRADA" and tipo_seleccionado="APROVISIONAMIENTO" then
			'en los aprovisionamientos que vienen con una hoja de ruta en vez de un albaran
			' tenemos que poner la hoja de ruta en estado de ENTRADA ALMACEN (10) para el programa de graphisoft
			...
			if Request.ServerVariables("SERVER_NAME")<>"carrito.globalia-artesgraficas.com" then
				...
				'ENTORNO REAL
				cadena_servidor="[192.168.156.175\SERVERSQL]."
			end if
			
			
			cadena_historico="INSERT INTO " & cadena_servidor & "GAG.dbo.GESTION_GRAPHISOFT_HISTORICO_HOJAS (ID_HOJA_RUTA, HOJA_RUTA, FECHA, ACCION, CAMPO, VALOR_ANTIGUO, VALOR_NUEVO,"
			cadena_historico=cadena_historico & "USUARIO, DESCRIPCION, TIPO_INCIDENCIA)"
			cadena_historico=cadena_historico & " SELECT"
			
			cadena_historico=cadena_historico & " (SELECT ID from " & cadena_servidor & "GAG.dbo.GESTION_GRAPHISOFT_HOJAS_IMPORTADAS WHERE HOJA_DE_RUTA=" & albaran_seleccionado & ")"
			cadena_historico=cadena_historico & ", '" & albaran_seleccionado & "'"
			 
			cadena_historico=cadena_historico & ", GETDATE(), 'CAMBIO', 'ESTADO'"
			cadena_historico=cadena_historico & ", (SELECT DESCRIPCION FROM " & cadena_servidor & "GAG.dbo.GESTION_GRAPHISOFT_ESTADOS"
			cadena_historico=cadena_historico & " WHERE ID=(SELECT ID_ESTADO from " & cadena_servidor & "GAG.dbo.GESTION_GRAPHISOFT_HOJAS_IMPORTADAS WHERE HOJA_DE_RUTA=" & albaran_seleccionado & "))"

			cadena_historico=cadena_historico & ", 'ENTRADA DE ALMACEN', 'CARRITO', 'APROVISIONAMIENTO EN EL CARRITO', NULL"
					
			'response.write("<br>" & cadena_historico)		
			connimprenta.Execute cadena_historico,,adCmdText + adExecuteNoRecords
			
			
			cadena_ejecucion="UPDATE " & cadena_servidor & "GAG.dbo.GESTION_GRAPHISOFT_HOJAS_IMPORTADAS"
			cadena_ejecucion=cadena_ejecucion & " SET ID_ESTADO=10"
			cadena_ejecucion=cadena_ejecucion & " WHERE HOJA_DE_RUTA=" & albaran_seleccionado
			connimprenta.Execute cadena_ejecucion,,adCmdText + adExecuteNoRecords
