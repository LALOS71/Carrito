						...
						<div>
								...
								<table id="lista_pedidos" name="lista_pedidos" class="table table-striped table-bordered" cellspacing="0" width="100%">
									...
									<%while not pedidos.eof%>
										...
										sql=sql & " (SELECT ARTICULOS_MARCAS.ID_ARTICULO, ARTICULOS_MARCAS.MARCA COLLATE DATABASE_DEFAULT AS MARCA, ARTICULOS_MARCAS.STOCK, ARTICULOS_MARCAS.STOCK_MINIMO"
										...
										sql=sql & " ON V_CLIENTES_MARCA.MARCA COLLATE DATABASE_DEFAULT = a.MARCA"
...
mostrar_sn_impresoras = function (cantidad, estado)
{
		...
		botones = {
			  ...
			  ok: {
				...
				callback: function() {
				  ...
				  if (!valid) {
					bootbox.alert("Introduzca valores en todas las cajas de texto.");
					return false;
				  }
				  var serials = [];
				  j$(".sn_impresoras").each(function() {
	...
	if (typeof array_impresoras_previas !== "undefined") {
      ...
      if (isNaN(cantidad) || cantidad <= 0) {
        bootbox.alert("Introduzca una cantidad válida.");
        return;
      }
