...
<%
	...
	response.write("<BR><br>1 --------------------------<br> conn_gag -- BEGINTRANS")
	conn_gag.BeginTrans 'Comenzamos la Transaccion
	response.write("<br><BR>2 conn_gag EXECUTE SET DATEFORMAT DMY")
	...
	if observaciones<>"" then	
		...
		response.write("<br><br>3 conn_gag EXECUTE " & CADENA_HISTORICO)
	...
	For i = 0 to UBound(tabla_articulos_cantidades)
		...
		response.write("<br><br>4 conn_gag EXECUTE " & CADENA_EJECUCION)
		...
		if Request.Form("cmbestados_" & articulo_cantidad(0))="ENVIO PARCIAL" or Request.Form("cmbestados_" & articulo_cantidad(0))="ENVIADO" or Request.Form("cmbestados_" & articulo_cantidad(0))="ENVIADO AL PROVEEDOR" THEN
			...
			response.write("<br><br>5 conn_gag EXECUTE " & CADENA_HISTORICO)
			conn_gag.Execute cadena_historico,,adCmdText + adExecuteNoRecords
		end if
		...
		if ((articulo_cantidad(2)<>"SI" AND Request.Form("cmbestados_" & articulo_cantidad(0))="ENVIADO") _
			...
			if not control_kit.eof then
				...
				IF Request.Form("cmbestados_" & articulo_cantidad(0))<>"LISTO PARCIAL" THEN
					...
					while not control_kit.eof
						...
						response.write("<br><br>6 conn_gag EXECUTE " & CADENA_EJECUCION)
						conn_gag.Execute cadena_ejecucion,,adCmdText + adExecuteNoRecords
						...
						if Request.Form("cmbestados_" & articulo_cantidad(0))="ENVIO PARCIAL" or Request.Form("cmbestados_" & articulo_cantidad(0))="ENVIADO"  or Request.Form("cmbestados_" & articulo_cantidad(0))="ENVIADO AL PROVEEDOR" THEN
							...
							response.write("<br><br>7 conn_gag EXECUTE " & CADENA_HISTORICO)
							conn_gag.Execute cadena_historico,,adCmdText + adExecuteNoRecords
							...
							response.write("<br><br>8 conn_gag EXECUTE " & CADENA_EJECUCION)
							conn_gag.Execute cadena_ejecucion_salida,,adCmdText + adExecuteNoRecords
					...
					'si es un envio parcial, meto los datos que se envian en la tabla correspondiente						
						...
						IF Request.Form("cmbestados_" & articulo_cantidad(0))="ENVIO PARCIAL" OR Request.Form("cmbestados_" & articulo_cantidad(0))="LISTO PARCIAL" THEN
							...
							response.write("<br><BR>9 conn_gag EXECUTE " & CADENA_PARCIAL)
							'RESPONSE.WRITE("<BR> es un envio parcial DE KIT: " & CADENA_PARCIAL)
							conn_gag.Execute cadena_parcial,,adCmdText + adExecuteNoRecords
						  ELSE
							IF (Request.Form("cmbestados_" & articulo_cantidad(0))="ENVIADO" AND Request.form("ocultocantidad_enviada_total_" & articulo_cantidad(0))<>"" _
								AND cantidad_pedido<>oculto_cantidad_enviada) THEN
								...
								response.write("<br><BR>10 conn_gag EXECUTE " & CADENA_PARCIAL)
					...
					IF Request.Form("cmbestados_" & articulo_cantidad(0))<>"LISTO PARCIAL" THEN
						...
						response.write("<br><BR>11 conn_gag EXECUTE " & CADENA_EJECUCION)
						conn_gag.Execute cadena_ejecucion,,adCmdText + adExecuteNoRecords
						...
						if Request.Form("cmbestados_" & articulo_cantidad(0))="ENVIO PARCIAL" or Request.Form("cmbestados_" & articulo_cantidad(0))="ENVIADO" or Request.Form("cmbestados_" & articulo_cantidad(0))="ENVIADO AL PROVEEDOR" THEN
							'response.write("<br>" & cadena_historico)
							response.write("<br><BR>12 conn_gag EXECUTE " & CADENA_HISTORICO)
							conn_gag.Execute cadena_historico,,adCmdText + adExecuteNoRecords
							...
							'mentemos la correspondiente salida de mercancia de la ficha del producto
								...
								response.write("<br><BR>13 conn_gag EXECUTE " & CADENA_EJECUCION)
								conn_gag.Execute cadena_ejecucion_salida,,adCmdText + adExecuteNoRecords
						end if
						...
						if pedido_con_rollo_impresora="SI"then
							...
							response.write("<br><BR>14 conn_gag EXECUTE " & CADENA_EJECUCION)
							conn_gag.Execute cadena_ejecucion,,adCmdText + adExecuteNoRecords
							...
							if Request.Form("cmbestados_" & articulo_cantidad(0))="ENVIO PARCIAL" or Request.Form("cmbestados_" & articulo_cantidad(0))="ENVIADO" or Request.Form("cmbestados_" & articulo_cantidad(0))="ENVIADO AL PROVEEDOR" THEN
								'response.write("<br>" & cadena_historico)
								response.write("<br><BR>15 conn_gag EXECUTE " & CADENA_HISTORICO)
								conn_gag.Execute cadena_historico,,adCmdText + adExecuteNoRecords
								...
								'mentemos la correspondiente salida de mercancia de la ficha del producto
									...
									'response.write("<br><br>cadena entradas salidas articulos: " & cadena_ejecucion_salida)
									response.write("<br><BR>16 conn_gag EXECUTE " & CADENA_EJECUCION)
									conn_gag.Execute cadena_ejecucion_salida,,adCmdText + adExecuteNoRecords
							end if
					...
					IF Request.Form("cmbestados_" & articulo_cantidad(0))="ENVIO PARCIAL" or Request.Form("cmbestados_" & articulo_cantidad(0))="LISTO PARCIAL" THEN
						...
						response.write("<br><BR>17 conn_gag EXECUTE " & CADENA_PARCIAL)
						conn_gag.Execute cadena_parcial,,adCmdText + adExecuteNoRecords
					  ELSE
							...
							IF ((Request.Form("cmbestados_" & articulo_cantidad(0))="ENVIADO" OR Request.Form("cmbestados_" & articulo_cantidad(0))="LISTO") AND Request.form("ocultocantidad_enviada_total_" & articulo_cantidad(0))<>"" _
								...
								response.write("<br><BR>18 conn_gag EXECUTE " & CADENA_PARCIAL)
								conn_gag.Execute cadena_parcial,,adCmdText + adExecuteNoRecords
							END IF
					END IF
			...
			IF articulo_cantidad(1)<>"" AND sumado<>"" THEN
				...
				if CINT(articulo_cantidad(1))=CINT(sumado) then
					...
					'response.write("<br>" & cadena_historico)
					response.write("<br><BR>19 conn_gag EXECUTE " & CADENA_HISTORICO)
					conn_gag.Execute cadena_historico,,adCmdText + adExecuteNoRecords
				end if
			END IF
			...
			response.write("<br><BR>20 conn_gag EXECUTE " & CADENA_EJECUCION)
			conn_gag.Execute cadena_ejecucion,,adCmdText + adExecuteNoRecords
	...
	Next
		...
		if not estado_pedido.eof then
			...
			response.write("<br><BR>21 conn_gag EXECUTE " & CADENA_EJECUCION)	
			conn_gag.Execute cadena_ejecucion,,adCmdText + adExecuteNoRecords
			...
			if (estado_a_grabar="LISTO" or estado_a_grabar="LISTO PARCIAL" or estado_a_grabar="ENVIADO" or estado_a_grabar="ENVIO PARCIAL" ) AND (palets<>"" and bultos<>"" and peso<>"") then
				...
				'conn_gag.Execute poner_pesos,,adCmdText + adExecuteNoRecords
				...
				'response.write("<br>poner pesos: " & poner_pesos)
				response.write("<br><BR>22 conn_gag EXECUTE " & PONER_PESOS)
				conn_gag.Execute poner_pesos,,adCmdText + adExecuteNoRecords
			...
			if estado_a_grabar="RECHAZADO" then
				...
				response.write("<br><BR>23 conn_gag EXECUTE " & CADENA_EJECUCION3)
				conn_gag.Execute cadena_ejecucion3,,adCmdText + adExecuteNoRecords
				response.write("<br><BR>24 conn_gag EXECUTE " & CADENA_EJECUCION4)
				conn_gag.Execute cadena_ejecucion4,,adCmdText + adExecuteNoRecords
				response.write("<br><BR>25 conn_gag EXECUTE " & CADENA_EJECUCION5)
				conn_gag.Execute cadena_ejecucion5,,adCmdText + adExecuteNoRecords
				response.write("<br><BR>26 conn_gag EXECUTE " & CADENA_EJECUCION6)
				conn_gag.Execute cadena_ejecucion6,,adCmdText + adExecuteNoRecords
			end if
		...
		if acciones="ALBARAN" then
			...
			if creamos_albaran="SI" then
				   ...
				   response.write("<br><BR>27---------------------<BR>INSERTAMOS LA CABECERA DEL ALBARAN EN " & RUTA_SERVIDOR_GAG & RUTA_BD_GAG)
					...
					Set rs_albaran_nuevo = conn_gag.Execute(cad_albaran_total)
					...
					response.write("<br><BR>29 conn_gag EXECUTE " & CADENA_EJECUCION_ALBARAN)
					conn_gag.Execute cadena_ejecucion_albaran,,adCmdText + adExecuteNoRecords
					...
					response.write("<br><BR>30 conn_gag EXECUTE " & CADENA_EJECUCION_ALBARAN)
					conn_gag.Execute cadena_ejecucion_albaran,,adCmdText + adExecuteNoRecords
					...
					response.write("<br><BR>31 conn_gag EXECUTE " & CADENA_ACTUALIZACION_BULTOS)
					conn_gag.Execute cadena_actualizacion_bultos,,adCmdText + adExecuteNoRecords
									...
									if albaran_nuevo=albaran_parcial_detalle or albaran_parcial_detalle="" then
										...
										if id_articulo_detalle="4583" then
											...
											For i = LBound(numeros_de_serie) To UBound(numeros_de_serie)
												...
												response.write("<br><BR>33 conn_gag EXECUTE " & CADENA_INSERCION_IMP)
												conn_gag.Execute cadena_insercion_imp,,adCmdText + adExecuteNoRecords
												...
												response.write("<br><BR>34 conn_gag EXECUTE " & HISTORICO_IMP)
												conn_gag.Execute historico_imp,,adCmdText + adExecuteNoRecords
											Next
										end if
									end if
								end if
								...
								if albaran_detalle=albaran_nuevo then
										...
										if id_articulo_detalle="4583" then
											...
											For i = LBound(numeros_de_serie) To UBound(numeros_de_serie)
												...
												response.write("<br><BR>36 conn_gag EXECUTE " & CADENA_INSERCION_IMP)
												conn_gag.Execute cadena_insercion_imp,,adCmdText + adExecuteNoRecords
												...
												historico_imp = historico_imp & " VALUES ('" & numeros_de_serie(i) & "', GETDATE(), 'ACTIVA', " & cliente_albaran
												historico_imp = historico_imp & ", '" & direccion_ip & "', 'GAG ADMIN')"
												response.write("<br><BR>37 conn_gag EXECUTE " & HISTORICO_IMP)
												conn_gag.Execute historico_imp,,adCmdText + adExecuteNoRecords
	...
	response.write("<br>------------------<BR>conn_gag -- COMMITTRANS")
