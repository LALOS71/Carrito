...
<div class="container-fluid">
	<div class="row mt-1">
		...
		<div class="col-xs-12 col-sm-12 col-md-8 col-lg-9 col-xl-9" id="columna_izquierda__">
			<!--saldos-->
			<%if session("usuario_codigo_empresa")=4 and session("usuario_tipo")<>"GLS PROPIA" then%>
...
mostrar_impresoras = function(cliente_seleccionado){
	$.ajax({
        url: "Impresoras_GLS_Oficinas.asp",
...
configurar_datatable = function() {
	...
	$("#lista_impresoras").DataTable({
		...
		rowId: 'extn',
		deferRender: true,
		scrollY: calcDataTableHeight(75),
		scrollCollapse: true,
		paging: false,
		processing: true,
		searching: true,
   ...
   $('#lista_impresoras').on('init.dt', function () {
		...
		$('#btnDocumentacion').click(function() {
		  window.open("/ruta/al/archivo.pdf");
		});
  });
