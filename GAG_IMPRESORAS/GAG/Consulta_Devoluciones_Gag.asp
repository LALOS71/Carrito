						...
						<div class="card-body">
								<div class="row">
									...
									<div class="col-sm-12 col-md-2 col-lg-2">
										<%cadena_enlace=""
											if empleado_gls="SI" then
												cadena_enlace="Manual_Gestor_Devoluciones_EMPLEADOS_GLS.pdf"
											  else
											  	if session("usuario_codigo_empresa")=4 then
													if session("usuario_tipo")="GLS PROPIA" then
														cadena_enlace="Manual_Gestor_Devoluciones_PROPIAS_GLS.pdf"
													  else
													  	cadena_enlace="Manual_Gestor_Devoluciones_AGENCIAS_GLS.pdf"
													end if
												end if
											end if
										%>
										<a href="manuales/<%=cadena_enlace%>" target="_blank" style="text-decoration:none ">
									  <button type="button" class="btn btn-primary btn-block" id="cmdmanual" name="cmdmanual"
											data-toggle="popover"
											data-placement="top"
											data-trigger="hover"
											data-content="Consultar el Manual de Devoluciones"
											data-original-title=""
											>
			...
			$.ajax({
				...
				success: function(respuesta) {
								...
								if (respuesta.substring(0,13)=='DEVOLUCION###')
									...
									cadena_nombre_empleado=''
									//mostrar_devolucion(id_devolucion, 'SI')
									<%if empleado_gls="SI" then%>
											cadena_nombre_empleado='<%=session("usuario_directorio_activo_nombre")%>'
									<%end if%>
									
									mostrar_detalles_devolucion(id_devolucion, cadena_nombre_empleado, 'si')
...
mostrar_devolucion = function(id_devolucion, nombre_empleado, imprimiendo) {
	console.log('dentro de la funcion mostrar_devolucion.... nombre_empleado: ' + nombre_empleado)
	
	cadena_cabecera='Devoluci&oacute;n ' + id_devolucion
	if (nombre_empleado!='')
		{
		cadena_cabecera+=' - ' + nombre_empleado
		}
	mostrar_detalles_devolucion(id_devolucion, nombre_empleado, imprimiendo)
	$("#dialog_detalles_devolucion").modal("show");
	$("#cabecera_dialog_detalles_devolucion").html(cadena_cabecera);
}
		...
		*/			
        ...
        if (typeof lst_articulos_a_devolver == 'undefined') {
            ...
            lst_articulos_a_devolver = $('#lista_articulos_a_devolver').DataTable({dom:'<"toolbar">Blfrtip',
			  ...
			  lst_articulos_a_devolver.ajax.url('../tojson/consulta_devoluciones_obtener_articulos_a_devolver.asp?' + prm.toString());
		...
		*/			
        ...
        if (typeof lst_devoluciones == 'undefined') {
            ...
            lst_devoluciones = $('#lista_devoluciones').DataTable({dom:'Blfrtip',
                                                          ajax:{url:'../tojson/consulta_devoluciones_obtener_devoluciones.asp?' + prm.toString(),
                                                           type:'POST',
                                                           dataSrc:'ROWSET'},
                                                     order:[],
													 columnDefs: [
				...
				$("#lista_devoluciones tbody").on("dblclick", "tr", function(e) {
					...
					parametro_id=row.ID
					parametro_nombre_empleado=''
					console.log('en el dblclick de la fila de devolucion')
					console.log('abrimos la devolucion ' + row.ID)
					console.log('... que tiene como empleado: ' + row.NOMBRE_EMPLEADO)
					if (row.NOMBRE_EMPLEADO!='')
						{
						parametro_nombre_empleado=row.NOMBRE_EMPLEADO
						}
					...
					$("#ocultonombre_empleado_a_imprimir").val(parametro_nombre_empleado)
				  
				  	mostrar_devolucion(parametro_id, parametro_nombre_empleado, 'NO')
				});              
              ...
              //stf.lst_tra.clear().draw();
			  lst_devoluciones.ajax.url('../tojson/consulta_devoluciones_obtener_devoluciones.asp?' + prm.toString());
              lst_devoluciones.ajax.reload();                  
            }       
  };		


mostrar_detalles_devolucion = function(id_devolucion, nombre_empleado, imprimiendo) {  
													...
													buttons:[{extend:"copy", text:'<i class="far fa-copy"></i>', titleAttr:"Copiar en Portapapeles", 
															 ...
															 {extend: "print"
																		...
																		, title: function () { return ponerle_nombre();}
															 			...
															 			, messageTop: function () { return poner_cabecera_impresion();}
																		//messageTop: 'probando',
...
poner_cabecera_impresion=function(){
	console.log('dentro de poner_cabecera_impresion.... ocultonombreempleado: ' + $("#ocultonombre_empleado_a_imprimir").val())
	cadena='<%=cabecera_para_impresion%>'
	if ($("#ocultonombre_empleado_a_imprimir").val()!='')
		{
		cadena=cadena.replace('<tr><td>Cliente</td><td><b>', '<tr><td>Cliente</td><td><b>' + $("#ocultonombre_empleado_a_imprimir").val() + '<BR>')
		}
	return cadena
}
