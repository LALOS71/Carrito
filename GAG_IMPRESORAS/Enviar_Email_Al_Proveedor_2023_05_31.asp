...
<%
		...
		if not pedido_al_proveedor.eof then
			...
			'response.write("<br>generamos el pdf...")
			...
			anchuras = array(30, 35, 40, 110, 17, 20, 25) 'ancho total 190
			...
			while not detalles_pedido_proveedor.eof
				...
				if plantilla_personalizacion<>"" then
					...
					'response.write("<br>Ruta del fichero json: " & ruta_fichero_json)
			...
			pdf.Ln()
			'response.write("<br>despues de ultimo pdf ln...")
			pdf.Close()
			'response.write("<br>despues del pdf close...")
			
			pdf.Output "TempPDF/Pedido_" & pedido_seleccionado & ".pdf", FALSE
			
			'response.write("<br>terminamos el pdf...")
			...
			'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
				...
				enviar_a="malba@globalia-artesgraficas.com" 	
		...
		''''''''''''''''''''''''''''
		   ...
		   objCDO.Subject  = "Solicitud de Rotulacion GAG - Pedido " & pedido_seleccionado
		   ...
		   'response.write("<br>antes del send.")
		   objCDO.Send
			'response.write("<br>despues del send.")
